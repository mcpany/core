...
def wrap_text(text, width=78): # Reduced width to handle markers safely
    lines = text.split('\n')
    wrapped_lines = []
    in_code_block = False
    in_table = False
    ...
    for line in lines:
        ...
        if match:
            ...
            indent = len(marker)
            # Use a smaller width for the content to account for indentation
