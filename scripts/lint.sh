...
if [[ -x "$GOLANGCI_LINT_BIN" ]]; then
    ...
    GOGC=10 GOMEMLIMIT=1024MiB "$GOLANGCI_LINT_BIN" run --timeout 20m --fix -j 1 ./server/pkg/...
    GOGC=10 GOMEMLIMIT=1024MiB "$GOLANGCI_LINT_BIN" run --timeout 20m --fix -j 1 ./server/tests/...
    GOGC=10 GOMEMLIMIT=1024MiB "$GOLANGCI_LINT_BIN" run --timeout 20m --fix -j 1 ./server/examples/...
