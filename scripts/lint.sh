...
if [[ ! -x "$GOLANGCI_LINT_BIN" ]]; then GOLANGCI_LINT_BIN="$(go env GOPATH)/bin/golangci-lint"; fi
if [[ ! -x "$GOLANGCI_LINT_BIN" ]]; then GOLANGCI_LINT_BIN="$(find_tool golangci-lint)"; fi
