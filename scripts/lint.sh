...
if [[ -x "$GOLANGCI_LINT_BIN" ]]; then
    ...
    cd server
    "$GOLANGCI_LINT_BIN" run --timeout 20m --concurrency 1 ./cmd/...
    "$GOLANGCI_LINT_BIN" run --timeout 20m --concurrency 1 ./pkg/...
    "$GOLANGCI_LINT_BIN" run --timeout 20m --concurrency 1 ./tests/...
    "$GOLANGCI_LINT_BIN" run --timeout 20m --concurrency 1 ./examples/...
    cd ..
