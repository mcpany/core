...
if [[ -x "$GOLANGCI_LINT_BIN" ]]; then
    # Set GOGC to 20 to reduce memory usage during linting in constrained environments.
    # Limit concurrency to 2 to avoid OOM on standard runners.
    GOGC=20 "$GOLANGCI_LINT_BIN" run --timeout 20m --concurrency 2 --fix \
