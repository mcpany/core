...
if [[ -x "$GOLANGCI_LINT_BIN" ]]; then
    ...
    "$GOLANGCI_LINT_BIN" run --timeout 20m --fix --concurrency 1 ./server/cmd/...
    "$GOLANGCI_LINT_BIN" run --timeout 20m --fix --concurrency 1 ./server/pkg/...
    "$GOLANGCI_LINT_BIN" run --timeout 20m --fix --concurrency 1 ./server/tests/...
    "$GOLANGCI_LINT_BIN" run --timeout 20m --fix --concurrency 1 ./server/examples/...
