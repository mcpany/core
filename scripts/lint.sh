...
if [[ -x "$GOLANGCI_LINT_BIN" ]]; then
    ...
    echo "==> Batch: upstream"
    "$GOLANGCI_LINT_BIN" run --timeout 20m --fix --concurrency 1 ./server/pkg/upstream/mcp/...
    "$GOLANGCI_LINT_BIN" run --timeout 20m --fix --concurrency 1 ./server/pkg/upstream/http/... ./server/pkg/upstream/grpc/...
    "$GOLANGCI_LINT_BIN" run --timeout 20m --fix --concurrency 1 \
        ./server/pkg/upstream/command/... ./server/pkg/upstream/filesystem/... ./server/pkg/upstream/openapi/... \
        ./server/pkg/upstream/websocket/... ./server/pkg/upstream/webrtc/...

    echo "==> Batch: tool"
    # pkg/tool is large, split by files if needed, but let's try just one subpkg first
