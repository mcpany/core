...
e2e: build build-examples build-e2e-mocks build-e2e-timeserver-docker
	...
	@GEMINI_API_KEY=$(GEMINI_API_KEY) MCPXY_DEBUG=true CGO_ENABLED=1 USE_SUDO_FOR_DOCKER=$(NEEDS_SUDO_FOR_DOCKER) $(GO_CMD) test -race -count=1 -timeout 600s -tags=e2e $(shell go list ./... | grep 'tests/integration')
