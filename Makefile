...
# Proto TypeScript bindings are no longer generated here; use Bazel:
#   bazel build //proto:ts_proto
#   bazel build //ui:build
.PHONY: all test lint build run clean gen gen-go-proto gen-gateway-proto gen-protoset prepare-proto clean-protos clean-gateway-protos update-screenshots
...
# Resolve bazelisk: prefer the locally installed copy, fall back to PATH.
BAZELISK := $(shell if [ -f $(TOOL_INSTALL_DIR)/bazelisk ]; then echo $(TOOL_INSTALL_DIR)/bazelisk; else command -v bazelisk 2>/dev/null || echo bazelisk; fi)
...
# Build all binaries and the UI via Bazel (single cache, hermetic, reproducible).
all: gen
	$(BAZELISK) build //server/cmd/server //server/cmd/worker //server/cmd/webhooks //ui:build
...
test: gen
	$(MAKE) test-proto
	$(BAZELISK) test //server/... //ui/...
	$(MAKE) -C k8s test

docker-build-all:
	# Build OCI images via Bazel (hermetic, Dockerfile-free where possible).
	$(BAZELISK) run //server/cmd/server:server_tarball
	$(BAZELISK) run //ui:ui_tarball
	# The http-echo-server and other auxiliary images still use Docker:
	$(MAKE) -C server docker-build-http-echo

build-docker:
	# Load the Bazel-built server OCI image into Docker.
	$(BAZELISK) run //server/cmd/server:server_tarball
...
# Build all server binaries and the UI via Bazel.
build:
	$(BAZELISK) build //server/cmd/server //server/cmd/worker //server/cmd/webhooks //ui:build
...
clean-protos: clean-gateway-protos
	@echo "Cleaning generated protobuf files..."

gen: gen-go-proto gen-gateway-proto gen-protoset
	@echo "Proto generation complete. TypeScript bindings are generated by Bazel (bazel build //proto:ts_proto)."

gen-go-proto: gen-gateway-proto gen-protoset
	@export PATH=$(TOOL_INSTALL_DIR):$$PATH; find proto -name "*.proto" -not -path "proto/third_party/*" -not -path "proto/google/*" -exec protoc --proto_path=. --proto_path=$(BUILD_DIR)/grpc-gateway --proto_path=$(BUILD_DIR)/googleapis --go_out=. --go_opt=module=github.com/mcpany/core --go-grpc_out=. --go-grpc_opt=module=github.com/mcpany/core {} +
...
# gen-ts-proto is intentionally removed.
# TypeScript proto bindings are now generated by Bazel:
#   bazel build //proto:ts_proto   (WKT + all package bindings)
#   bazel build //ui:build         (full UI build, includes TS proto gen)
# The generated .ts files are NOT committed to the repository.

update-screenshots:
	$(MAKE) -C ui update-screenshots

# ---------------------------------------------------------------------------
# Convenience alias: build + load both OCI images via Bazel then run
# the screenshot update using Docker Compose.
# ---------------------------------------------------------------------------
bazel-build-images:
	$(BAZELISK) run //server/cmd/server:server_tarball
	$(BAZELISK) run //ui:ui_tarball
