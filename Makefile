...
gen: gen-go gen-gateway-proto gen-protoset
...
gen-go: prepare-proto
	...
	@export PATH=$(TOOL_INSTALL_DIR):$$PATH; \
		find proto -name "*.proto" -not -path "proto/third_party/*" -not -path "proto/google/*" -exec protoc \
			...
			--go_out=. --go_opt=module=github.com/mcpany/core \
			--go-grpc_out=. --go-grpc_opt=module=github.com/mcpany/core \
			{} +

gen-go-proto: gen-go gen-gateway-proto gen-protoset
	@echo "gen-go-proto is now an alias for Go stubs, gateway stubs plus descriptor set generation."
