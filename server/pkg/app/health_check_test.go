...
func (m *mockServiceRegistry) GetServiceHealth(serviceID string) (string, time.Duration, time.Time, bool) {
	err, ok := m.errors[serviceID]
	if ok {
		return err, 0, time.Now(), true
	}
	// Return success for known services if no error mapped
	for _, s := range m.services {
		if s.GetId() == serviceID || s.GetName() == serviceID {
			return "", 10 * time.Millisecond, time.Now(), true
		}
	}
