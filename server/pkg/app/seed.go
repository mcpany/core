...
func (a *Application) handleDebugSeed() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := a.clearData(ctx); err != nil {
		...
		if err := a.seedData(ctx, &req); err != nil {
			...
			// seedData already logs specific error but returns generic error
			// We can improve error handling but for now internal server error is fine
...
func (a *Application) clearData(ctx context.Context) error {
	log := logging.GetLogger()
