...
func (a *Application) handleDebugSeed() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, user := range req.Users {
			// Try Create, if fails (exists), try Update
            // Note: Storage interface errors might not be typed easily to check "exists".
            // We just attempt Create. If it fails, we assume it exists and try Update.
            // This covers both new and existing cases if Reset=false.
			if err := a.Storage.CreateUser(ctx, user); err != nil {
				if updateErr := a.Storage.UpdateUser(ctx, user); updateErr != nil {
                     ...
                     // If we are in Reset mode, Create should have succeeded unless DB error.
                     // We report error if Reset=true and Create fails.
                     if req.Reset {
		...
		for _, c := range req.ServiceCollections {
			if err := a.Storage.SaveServiceCollection(ctx, c); err != nil {
				http.Error(w, "Failed to save collection: "+err.Error(), http.StatusInternalServerError)
				return
			}
		}
		...
		w.Write([]byte(`{"status": "seeded", "message": "Database seeded successfully"}`))
	}
}
