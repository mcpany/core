...
func (a *Application) initializeDatabase(ctx context.Context, store config.Store) error {
	...
	if !ok {
		...
		// If it's not a Storage interface (e.g. file-based only, read-only wrapper),
		// we cannot seed anything.
		return nil
	}

	// 1. Check & Seed Defaults (if empty)
	...
	// 2. Seed Service Templates (Always upsert built-ins)
	log.Info("Seeding service templates...", "count", len(BuiltinServiceTemplates))
	for _, t := range BuiltinServiceTemplates {
		if err := s.SaveServiceTemplate(ctx, t); err != nil {
			log.Warn("Failed to seed template", "id", t.GetId(), "error", err)
		}
	}
