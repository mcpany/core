...
func (a *Application) initializeOfficialCollections(ctx context.Context, store config.Store) error {
	...
	collections := []*configv1.Collection{
		configv1.Collection_builder{
			...
			Tags:        []string{"data", "engineering", "sql", "python"},
		...
		configv1.Collection_builder{
			...
			Author:      proto.String("MCP Any Team"),
			Version:     proto.String("1.0.0"),
			Tags:        []string{"web", "dev", "github"},
			Services:    []*configv1.UpstreamServiceConfig{},
		}.Build(),
	}

	for _, col := range collections {
		// Upsert logic: Always save official collections to ensure they are up to date.
		// Users should clone them if they want to customize.
		if err := s.SaveServiceCollection(ctx, col); err != nil {
			log.Error("Failed to save official collection", "name", col.GetName(), "error", err)
			// Continue to next collection instead of failing hard
			continue
		}
