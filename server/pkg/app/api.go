...
func (a *Application) handleServicesGet(w http.ResponseWriter, r *http.Request, store storage.Storage) {
	...
	for i, svc := range services {
		...
		// Inject runtime info
		if enriched, err := a.enrichServiceJSON(b, svc); err == nil {
			b = enriched
		}
...
func (a *Application) enrichServiceJSON(jsonBytes []byte, svc *configv1.UpstreamServiceConfig) ([]byte, error) {
	...
	if a.ServiceRegistry != nil {
		...
		// Also check sanitize name if ID lookup fails
