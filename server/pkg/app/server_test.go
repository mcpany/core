...
`
	...
	go func() {
		...
		errChan <- app.Run(RunOptions{Ctx: ctx, Fs: fs, Stdio: false, JSONRPCPort: "127.0.0.1:0", GRPCPort: "127.0.0.1:0", ConfigPaths: []string{"/config.yaml"}, APIKey: "", ShutdownTimeout: 5 * time.Second, DBPath: filepath.Join(t.TempDir(), "mcpany.db")})
	}()
...
func TestRun_ConfigLoadError(t *testing.T) {
	...
	err = app.Run(RunOptions{
		...
		APIKey:          "",
		ShutdownTimeout: 5 * time.Second, DBPath: filepath.Join(t.TempDir(), "mcpany.db"),
	})
	require.Error(t, err)
...
func TestRun_BusProviderError(t *testing.T) {
	...
	app := NewApplication()
	err = app.Run(RunOptions{Ctx: ctx, Fs: fs, Stdio: false, JSONRPCPort: "127.0.0.1:0", GRPCPort: "127.0.0.1:0", ConfigPaths: []string{"/config.yaml"}, APIKey: "", ShutdownTimeout: 5 * time.Second, DBPath: filepath.Join(t.TempDir(), "mcpany.db")})
...
func TestRun_StdioMode(t *testing.T) {
	...
	err := app.Run(RunOptions{Ctx: ctx, Fs: fs, Stdio: true, JSONRPCPort: "127.0.0.1:0", GRPCPort: "127.0.0.1:0", ConfigPaths: nil, APIKey: "", ShutdownTimeout: 5 * time.Second, DBPath: filepath.Join(t.TempDir(), "mcpany.db")})
...
func TestRun_NoGrpcServer(t *testing.T) {
	...
	errChan := make(chan error, 1)
	go func() {
		errChan <- app.Run(RunOptions{Ctx: ctx, Fs: fs, Stdio: false, JSONRPCPort: "127.0.0.1:0", GRPCPort: "", ConfigPaths: nil, APIKey: "", ShutdownTimeout: 5 * time.Second, DBPath: filepath.Join(t.TempDir(), "mcpany.db")})
	}()
...
func TestRun_DefaultBindAddress(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		conn, err := net.DialTimeout("tcp", defaultAddr, 500*time.Millisecond)
		if err == nil {
			_ = conn.Close()
			return true
		}
		return false
	}, 2*time.Second, 500*time.Millisecond, "server should be dialable on port %d", int(app.BoundHTTPPort.Load()))
	...
	require.Eventually(t, func() bool {
		...
		return true
	}, 2*time.Second, 500*time.Millisecond, "port should be released after shutdown")
}
...
func TestRun_GrpcPortNumber(t *testing.T) {
	...
	go func() {
		...
		errChan <- app.Run(RunOptions{Ctx: ctx, Fs: fs, Stdio: false, JSONRPCPort: "127.0.0.1:0", GRPCPort: "127.0.0.1:0", ConfigPaths: nil, APIKey: "", ShutdownTimeout: 5 * time.Second, DBPath: filepath.Join(t.TempDir(), "mcpany.db")})
	}()
	...
	require.Eventually(t, func() bool {
		...
		return false
	}, 2*time.Second, 500*time.Millisecond, "gRPC server should be dialable on port %d", int(app.BoundGRPCPort.Load()))
...
func Test_runStdioMode_real(t *testing.T) {
	...
	authManager := auth.NewManager()
	serviceRegistry := serviceregistry.New(nil, toolManager, promptManager, resourceManager, authManager)
	mcpSrv, err := mcpserver.NewServer(ctx, toolManager, promptManager, resourceManager, authManager, serviceRegistry, nil, busProvider, false)
	require.NoError(t, err)
...
func TestRun_CachingMiddleware(t *testing.T) {
	...
	err = app.Run(RunOptions{Ctx: ctx, Fs: fs, Stdio: false, JSONRPCPort: "127.0.0.1:0", GRPCPort: "", ConfigPaths: nil, APIKey: "", ShutdownTimeout: 5 * time.Second, DBPath: filepath.Join(t.TempDir(), "mcpany.db")})
	require.NoError(t, err)
...
`
	...
	time.Sleep(500 * time.Millisecond) // Allow time for publication.
...
func TestGRPCServer_ListenerClosedOnForcedShutdown(t *testing.T) {
	...
	time.Sleep(500 * time.Millisecond) // Allow time for the RPC to start.
...
func waitForServerReady(t *testing.T, addr string) {
	...
	require.Eventually(t, func() bool {
		...
		_ = conn.Close()
		return true
	}, 5*time.Second, 500*time.Millisecond, "server should be ready to accept connections")
}
...
`
	...
	mockStore.On("SaveLog", mock.Anything, mock.Anything).Return(nil)
		...
		errChan := make(chan error, 1)
		go func() {
			errChan <- app.Run(RunOptions{Ctx: ctx, Fs: fs, Stdio: false, JSONRPCPort: addr, GRPCPort: "", ConfigPaths: []string{"/config.yaml"}, APIKey: "", ShutdownTimeout: 5 * time.Second, DBPath: filepath.Join(t.TempDir(), "mcpany.db")})
		}()
...
func TestRunServerMode_Auth(t *testing.T) {
	...
	mcpSrv, err := mcpserver.NewServer(ctx, app.ToolManager, app.PromptManager, app.ResourceManager, authManager, serviceRegistry, nil, busProvider, true)
	require.NoError(t, err)
...
func TestMultiUserHandler_UserAuth(t *testing.T) {
	...
	go func() {
		app.Run(RunOptions{Ctx: ctx, Fs: fs, Stdio: false, JSONRPCPort: addr, GRPCPort: "", ConfigPaths: []string{"/config.yaml"}, APIKey: "", ShutdownTimeout: 5 * time.Second, DBPath: filepath.Join(t.TempDir(), "mcpany.db")})
	}()
...
func TestConfigureUIHandler(t *testing.T) {
	...
	mcpSrv, _ := mcpserver.NewServer(context.Background(), toolManager, prompt.NewManager(), resource.NewManager(), authManager, serviceRegistry, nil, busProvider, false)
...
`
	...
	require.Eventually(t, func() bool { return app.BoundHTTPPort.Load() != 0 }, 5*time.Second, 500*time.Millisecond)
	cancel()
	<-errChan
}
