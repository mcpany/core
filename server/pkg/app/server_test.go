...
func TestRun_ImplicitFileConfig(t *testing.T) {
	...
	// Create a config file defining a service
	configContent := `
upstream_services:
  - name: "implicit-config-service"
    http_service:
      address: "http://127.0.0.1:8080"
`
	err := afero.WriteFile(fs, "/implicit_config.yaml", []byte(configContent), 0o644)
	require.NoError(t, err)
	...
	go func() {
		errChan <- app.Run(RunOptions{
			...
			JSONRPCPort:     "127.0.0.1:0",
			GRPCPort:        "127.0.0.1:0",
			ConfigPaths:     []string{"/implicit_config.yaml"},
			APIKey:          "",
			ShutdownTimeout: 5 * time.Second,
		})
	}()
	...
	// Verify that the service from the config file was loaded.
	// Registration is async, so we use Eventually.
	require.Eventually(t, func() bool {
		// ServiceRegistry is initialized in Run
		...
		for _, s := range services {
			if s.GetName() == "implicit-config-service" {
				return true
			}
		}
		return false
	}, 2*time.Second, 100*time.Millisecond, "Service from implicit config should be registered")
