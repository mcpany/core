...
`
	...
	go func() {
		...
		errChan <- app.Run(ctx, fs, false, "localhost:0", "localhost:0", []string{"/config.yaml"}, "", 100*time.Millisecond, 5*time.Second)
	}()
...
func TestRun_ConfigLoadError(t *testing.T) {
	...
	err = app.Run(ctx, fs, false, "localhost:0", "localhost:0", []string{"/config.yaml"}, "", 100*time.Millisecond, 5*time.Second)
	require.Error(t, err)
...
func TestRun_StdioMode(t *testing.T) {
	...
	err := app.Run(ctx, fs, true, "localhost:0", "localhost:0", nil, "", 100*time.Millisecond, 5*time.Second)
...
func TestRun_NoGrpcServer(t *testing.T) {
	...
	errChan := make(chan error, 1)
	go func() {
		errChan <- app.Run(ctx, fs, false, "localhost:0", "", nil, "", 100*time.Millisecond, 5*time.Second)
	}()
...
func TestRun_ServerStartupErrors(t *testing.T) {
	...
	t.Run("nil_fs_fail", func(t *testing.T) {
		...
		err := app.Run(ctx, nil, false, "0", "0", nil, "", 100*time.Millisecond, 5*time.Second)
		require.Error(t, err)
	...
	t.Run("http_server_fail", func(t *testing.T) {
		...
		err = app.Run(ctx, fs, false, fmt.Sprintf("%d", port), "0", nil, "", 100*time.Millisecond, 5*time.Second)
		require.Error(t, err)
	...
	t.Run("grpc_server_fail", func(t *testing.T) {
		...
		// Attempt to run the server on the occupied port
		err = app.Run(ctx, fs, false, "0", fmt.Sprintf("%d", port), nil, "", 100*time.Millisecond, 5*time.Second)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "failed to start a server")
	})
}
...
func TestRun_ServerStartupError_GracefulShutdown(t *testing.T) {
	...
	runErr := app.Run(ctx, fs, false, fmt.Sprintf("localhost:%d", httpPort), "localhost:0", nil, "", 100*time.Millisecond, 1*time.Second)
...
func TestRun_GrpcPortNumber(t *testing.T) {
	...
	go func() {
		...
		errChan <- app.Run(ctx, fs, false, "localhost:0", fmt.Sprintf("%d", port), nil, "", 100*time.Millisecond, 5*time.Second)
	}()
...
func TestRunServerMode_GracefulShutdownOnContextCancel(t *testing.T) {
	...
	go func() {
		...
		errChan <- app.runServerMode(ctx, mcpSrv, busProvider, "localhost:0", "localhost:0", 1*time.Second, nil, cachingMiddleware, nil, serviceRegistry, nil)
	}()
...
func TestRun_ServerMode_LogsCorrectPort(t *testing.T) {
	...
	go func() {
		errChan <- app.Run(ctx, fs, false, "localhost:0", "localhost:0", nil, "", 100*time.Millisecond, 1*time.Second)
	}()
...
func TestRunServerMode_ContextCancellation(t *testing.T) {
	...
	go func() {
		errChan <- app.runServerMode(ctx, mcpSrv, busProvider, "localhost:0", "localhost:0", 5*time.Second, nil, cachingMiddleware, nil, serviceRegistry, nil)
	}()
...
func TestRun_CachingMiddleware(t *testing.T) {
	...
	err = app.Run(ctx, fs, false, "localhost:0", "", nil, "", 100*time.Millisecond, 5*time.Second)
	require.NoError(t, err)
...
`
	...
	errChan := make(chan error, 1)
	go func() {
		errChan <- app.Run(ctx, fs, false, "localhost:0", "", []string{"/config.yaml"}, "", 100*time.Millisecond, 5*time.Second)
	}()
...
func TestRun_NoConfigDoesNotBlock(t *testing.T) {
	...
	go func() {
		errChan <- app.Run(ctx, fs, false, "localhost:0", "localhost:0", nil, "", 100*time.Millisecond, 5*time.Second)
	}()
...
func TestRun_APIKeyAuthentication(t *testing.T) {
	...
	go func() {
		errChan <- app.Run(ctx, fs, false, addr, "", nil, viper.GetString("api-key"), 100*time.Millisecond, 5*time.Second)
	}()
