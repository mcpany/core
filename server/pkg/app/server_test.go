...
func TestStartGrpcServer_RegistrationServerError(t *testing.T) {
	...
	err = app.runServerMode(ctx, nil, busProvider, "127.0.0.1:0", "127.0.0.1:0", 1*time.Second, nil, nil, nil, nil, nil, nil, "", "", "")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "failed to create API server")
...
func TestGRPCServer_PanicInRegistration(t *testing.T) {
	...
	startGrpcServer(ctx, &wg, errChan, nil, "TestGRPC_NilServer", lis, 1*time.Second, nil)
	wg.Wait()

	select {
	case err := <-errChan:
		t.Fatalf("expected no error for nil server, got: %v", err)
	default:
		// No error — expected behaviour for a nil server.
	}
}
...
func TestStartGrpcServer_PanicHandling(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		srv := gogrpc.NewServer()
		startGrpcServer(ctx, &wg, errChan, nil, fmt.Sprintf("TestGRPC_Multi_%d", i), lis, 100*time.Millisecond, srv)
		...
		select {
		case err := <-errChan:
			require.NoError(t, err, "clean shutdown should not produce an error")
		default:
			// Expected: clean shutdown produces no error.
		}
	}
}
...
func TestStartGrpcServer_PanicInRegistrationRecovers(t *testing.T) {
	...
	closedLis, err := net.Listen("tcp", addr)
	require.NoError(t, err)
	_ = closedLis.Close()

	startGrpcServer(ctx, &wg, errChan, nil, "TestGRPC_Recover", closedLis, 100*time.Millisecond, gogrpc.NewServer())
