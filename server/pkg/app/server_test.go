...
func TestRunServerMode_Auth(t *testing.T) {
	...
	// Run assertions sequentially to avoid race condition on shared server state/logs
	// Note: We deliberately do NOT use t.Run here to avoid creating new goroutines that might outlive the test context.

	// Invalid Path
	resp, err := http.Get(baseURL + "/mcp/u/foo")
	require.NoError(t, err)
	defer resp.Body.Close()
	assert.Equal(t, http.StatusNotFound, resp.StatusCode, "Invalid path should return 404")

	// User Not Found
	resp, err = http.Get(baseURL + "/mcp/u/unknown_user/profile/any")
	require.NoError(t, err)
	defer resp.Body.Close()
	assert.Equal(t, http.StatusNotFound, resp.StatusCode, "Unknown user should return 404")

	// User Auth - Correct Key
	req, _ := http.NewRequest("POST", baseURL+"/mcp/u/user_with_auth/profile/profileA", nil)
	req.Header.Set("X-API-Key", "user-secret")
	resp, err = http.DefaultClient.Do(req)
	require.NoError(t, err)
	defer resp.Body.Close()
	assert.NotEqual(t, http.StatusUnauthorized, resp.StatusCode, "User auth with correct key should not be 401")
	assert.NotEqual(t, http.StatusForbidden, resp.StatusCode, "User auth with correct key should not be 403")

	// User No Auth - Global Correct
	req, _ = http.NewRequest("POST", baseURL+"/mcp/u/user_no_auth/profile/profileB", nil)
	req.Header.Set("X-API-Key", "global-secret")
	resp, err = http.DefaultClient.Do(req)
	require.NoError(t, err)
	defer resp.Body.Close()
	assert.NotEqual(t, http.StatusUnauthorized, resp.StatusCode, "Global auth should work for user with no auth")
