...
func TestRunServerMode_GracefulShutdownOnContextCancel(t *testing.T) {
	...
	errChan <- app.runServerMode(ctx, mcpSrv, busProvider, "localhost:0", "localhost:0", 1*time.Second, nil, nil, nil, nil, nil)
	}()
...
func TestRunServerMode_ContextCancellation(t *testing.T) {
	...
	go func() {
		errChan <- app.runServerMode(ctx, mcpSrv, busProvider, "localhost:0", "localhost:0", 5*time.Second, nil, nil, nil, nil, nil)
	}()
...
func TestRunServerMode_grpcListenErrorHangs(t *testing.T) {
	...
	errChan := make(chan error, 1)
	go func() {
		errChan <- app.runServerMode(ctx, nil, nil, "localhost:0", fmt.Sprintf("localhost:%d", port), 5*time.Second, nil, nil, nil, nil, nil)
	}()
