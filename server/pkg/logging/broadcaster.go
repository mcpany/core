...
func (b *Broadcaster) Broadcast(msg any) {
	...
	// Note: For 'any', we assume the value passed is either a value type (struct) or immutable.
	// If the caller passes a pointer to a mutable object, they should ensure safety.
	// For []byte, we might want to copy? But we moved to 'any' to avoid unnecessary copies of structs.
	// So we trust the caller. If it's a slice, the caller should have copied it if needed.
...
func (b *Broadcaster) Hydrate(messages []any) {
	...
	for _, msg := range messages {
		...
		// Assuming msg is safe to store as is.
