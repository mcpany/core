...
// Summary: Init initializes the application's global logger with a specific log level and output destination. This function is designed to be called only once, typically at the start of the application, to ensure a consistent logging setup.
//
// Parameters:
//   - level (slog.Level): The level parameter.
...
// Summary: GetLogger returns the shared global logger instance. If the logger has not yet been initialized through a call to `Init`, this function will initialize it with default settings: logging to `os.Stderr` at `slog.LevelInfo`.
//
// Parameters:
//   - None.
//
// Returns:
//   - *slog.Logger: The resulting *slog.Logger.
//
// Errors:
//   - None.
//
...
//
// Returns:
//   - slog.Level: The resulting slog.Level.
//
// Errors:
//   - None.
//
