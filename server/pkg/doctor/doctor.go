...
// Summary: RunChecks performs connectivity and health checks on the provided configuration. It iterates through all upstream services defined in the configuration and executes the appropriate check logic for each service type.
//
// Parameters:
...
//
// Returns:
//   - []CheckResult: The resulting []CheckResult.
//
// Errors:
//   - None.
//
...
// Summary: CheckService performs a connectivity check for a single service. It dispatches the check to the specific handler based on the service type (HTTP, gRPC, etc.) and handles upstream authentication checks if configured.
//
// Parameters:
//   - ctx (context.Context): The ctx parameter.
