...
//
// Summary: parses a JSON byte slice and redacts sensitive keys. If the input is not valid JSON object or array, it returns the input as is.
//
// Parameters:
//   - input: []byte
//
...
//
// Summary: redacts the password from a DSN string. Supported formats: postgres://user:password@host...
//
// Parameters:
//   - dsn: string
//
...
//
// Summary: handles redaction of secrets from text. It is optimized to pre-process the list of secrets once and reuse the configuration.
...
//
// Summary: creates a new SecretRedactor with the given secrets. It performs filtering, deduplication, and sorting of secrets to ensure optimal redaction.
//
// Parameters:
//   - secrets: []string
//
