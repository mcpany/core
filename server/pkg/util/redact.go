...
// Summary: RedactJSON parses a JSON byte slice and redacts sensitive keys. If the input is not valid JSON object or array, it returns the input as is. Redacts sensitive keys in JSON data.
//
// Parameters:
//   - input ([]byte): The input parameter.
//
// Returns:
...
// Summary: IsSensitiveKey checks if a key name suggests it contains sensitive information. Checks if a key name implies sensitive data.
//
// Parameters:
...
// Summary: RedactDSN redacts the password from a DSN string. Supported formats: postgres://user:password@host... Redacts passwords from DSN strings.
//
// Parameters:
...
// Summary: SecretRedactor handles redaction of secrets from text. It is optimized to pre-process the list of secrets once and reuse the configuration. Optimized text redactor for known secrets.
//
// Parameters:
//   - None.
//
...
// Summary: NewSecretRedactor creates a new SecretRedactor with the given secrets. It performs filtering, deduplication, and sorting of secrets to ensure optimal redaction. Creates a new SecretRedactor.
//
// Parameters:
//   - secrets ([]string): The secrets parameter.
//
// Returns:
//   - *SecretRedactor: The resulting *SecretRedactor.
//
// Errors:
//   - None.
//
...
// Summary: Redact replaces all occurrences of the configured secrets in the text with [REDACTED]. Redacts secrets from text.
//
// Parameters:
...
// Summary: RedactSecrets replaces all occurrences of the given secrets in the text with [REDACTED]. Convenience function to redact secrets from text.
//
// Parameters:
//   - text (string): The text parameter.
//   - secrets ([]string): The secrets parameter.
