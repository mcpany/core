...
func TestStripSecretsFromService_Coverage(t *testing.T) {
	...
	assert.Nil(t, envVar.GetValue(), "Plain text secret in Env should be stripped")
	...
	assert.Nil(t, secret.GetValue(), "Plain text secret in HttpCall should be stripped")
	...
	assert.Nil(t, wsSvc.GetWebsocketService().GetCalls()["call1"].GetParameters()[0].GetSecret().GetValue())
	...
	assert.Nil(t, webrtcSvc.GetWebrtcService().GetCalls()["call1"].GetParameters()[0].GetSecret().GetValue())
...
func TestHydrateSecrets_Coverage(t *testing.T) {
	secrets := map[string]*configv1.SecretValue{
		"MY_SECRET": configv1.SecretValue_builder{
			PlainText: proto.String("resolved"),
		}.Build(),
	}
	...
	svc := configv1.UpstreamServiceConfig_builder{
		CommandLineService: configv1.CommandLineUpstreamService_builder{
			Env: map[string]*configv1.SecretValue{
				"TARGET": configv1.SecretValue_builder{
					EnvironmentVariable: proto.String("MY_SECRET"),
				}.Build(),
				"MISSING": configv1.SecretValue_builder{
					EnvironmentVariable: proto.String("NOT_FOUND"),
				}.Build(),
				"PLAIN": configv1.SecretValue_builder{
...
func TestStripSecretsFromAuth_Coverage(t *testing.T) {
	...
	assert.Nil(t, auth.GetApiKey().GetValue().GetValue())
	...
	assert.Nil(t, bearer.GetBearerToken().GetToken().GetValue())
	...
	assert.Nil(t, basic.GetBasicAuth().GetPassword().GetValue())
	...
	assert.Nil(t, oauth.GetOauth2().GetClientId().GetValue())
	assert.Nil(t, oauth.GetOauth2().GetClientSecret().GetValue())
}
