...
func TestScanForSensitiveKeys_LongInput(t *testing.T) {
	...
	assert.False(t, scanForSensitiveKeys([]byte(longSafe), false, false))

	// Case 2: Long input with sensitive key at the end
	longSensitive := strings.Repeat("a", 200) + "password"
	assert.True(t, scanForSensitiveKeys([]byte(longSensitive), false, false))
	...
	assert.True(t, scanForSensitiveKeys([]byte(longSensitiveMid), false, false))

    // Case 4: Long input with sensitive key that requires case folding match
    // "Password"
    longSensitiveUpper := strings.Repeat("a", 200) + "Password"
    assert.True(t, scanForSensitiveKeys([]byte(longSensitiveUpper), false, false))
}
...
func TestScanForSensitiveKeys_EdgeCases(t *testing.T) {
    ...
    // input: "author"
    assert.False(t, scanForSensitiveKeys([]byte("author"), false, false))

    // "auth" matches.
    assert.True(t, scanForSensitiveKeys([]byte("auth"), false, false))
    ...
    assert.True(t, scanForSensitiveKeys([]byte("authToken"), false, false))

    // "AUTH" in "AUTHORITY"
    // "AUTH" matches "auth". First char 'A' is upper. Next 'O' is upper.
    // It continues (skips).
    assert.False(t, scanForSensitiveKeys([]byte("AUTHORITY"), false, false))
}
