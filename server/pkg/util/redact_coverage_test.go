...
func TestScanForSensitiveKeys_LongInput(t *testing.T) {
	...
	assert.False(t, scanForSensitiveKeys([]byte(longSafe)))

	// Case 2: Long input with sensitive key at the end
	longSensitive := strings.Repeat("a", 200) + "password"
	assert.True(t, scanForSensitiveKeys([]byte(longSensitive)))
...
func TestScanForSensitiveKeys_EdgeCases(t *testing.T) {
    ...
    // input: "author"
    assert.False(t, scanForSensitiveKeys([]byte("author")))

    // "auth" matches.
    assert.True(t, scanForSensitiveKeys([]byte("auth")))
    ...
    assert.True(t, scanForSensitiveKeys([]byte("authToken")))

    // "AUTH" in "AUTHORITY"
    // "AUTH" matches "auth". First char 'A' is upper. Next 'O' is upper.
    // It continues (skips).
    assert.False(t, scanForSensitiveKeys([]byte("AUTHORITY")))
}
