...
// Summary: WalkJSONStrings visits every string value in the JSON input. Walks through a JSON bytes slice and applies a visitor function to all string values. It supports non-standard JSON with comments (// and /* */).
//
// Parameters:
//   - input ([]byte): The input parameter.
//   - visitor (func(raw []byte) ([]byte, bool)): The visitor parameter.
//
// Returns:
...
// Summary: WalkStandardJSONStrings visits every string value in the JSON input. Optimized JSON walker for standard JSON (no comments). It visits every string value (not keys) and applies the visitor.
//
// Parameters:
//   - input ([]byte): The input parameter.
//   - visitor (func(raw []byte) ([]byte, bool)): The visitor parameter.
