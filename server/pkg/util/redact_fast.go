...
func isKeySensitive(keyContent []byte) bool {
	if bytes.IndexByte(keyContent, '\\') != -1 {
		...
		// Use streaming/chunked scan for keys to avoid allocation of unmarshalled string.
		// scanEscapedKeyForSensitive handles unescaping and checking against sensitive keys on the fly.
