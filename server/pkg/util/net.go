...
// isTrue checks if a string value represents "true", handling optional quotes and case insensitivity.
func isTrue(val string) bool {
	val = strings.Trim(strings.TrimSpace(val), "'\"")
	return strings.ToLower(val) == TrueStr
}
...
//   - MCPANY_DANGEROUS_ALLOW_LOCAL_IPS: Set to "true" to allow both loopback and private.
//
// Returns:
...
func NewSafeHTTPClient() *http.Client {
	...
	// Allow overriding safety checks via environment variables (consistent with CheckConnection)
