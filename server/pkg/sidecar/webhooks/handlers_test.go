...
func TestPaginateHandler_LargeString(t *testing.T) {
	...
	// Page 1, size 100
	body, _ := json.Marshal(event)
	req := httptest.NewRequest(http.MethodPost, "/?page_size=100", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/cloudevents+json")
	w := httptest.NewRecorder()
	...
	err = respEvent.DataAs(&respData)
	require.NoError(t, err)
	repl := respData["replacement_object"].(string)

	assert.Contains(t, repl, "Page 1/")
