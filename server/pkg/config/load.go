...
//
// It orchestrates the reading of the configuration, validates its contents, and returns a sanitized configuration object.
...
//   - error: An error if loading or critical validation fails.
//
// Side Effects:
//   - Logs configuration loading events and validation errors.
...
// LoadResolvedConfig loads key resolved configuration (merging services, setting defaults) without strict validation.
//
// Summary: Loads configuration with defaults and merging but without strict validation.
//
...
//   - error: An error if loading from the store fails.
