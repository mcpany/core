...
//
// Side Effects:
//   - Resolves secrets from environment variables or files to validate them (unless skipped via context).
//   - Performs filesystem checks for file existence (unless skipped via context).
...
//
// Errors:
//   - Returns error if service name is empty or service type is not specified.
//   - Returns error if service-specific validation fails (e.g. invalid URL, missing command).
//
// Side Effects:
//   - Resolves secrets and checks filesystem as part of validation.
