...
// BindRootFlags binds the global and persistent command-line flags to the Viper configuration registry. It establishes the connection between Cobra command flags and Viper's configuration management, enabling flag parsing and environment variable overrides (using the "MCPANY_" prefix).
//
// Summary: binds the global and persistent command-line flags to the Viper configuration registry. It establishes the connection between Cobra command flags and Viper's configuration management, enabling flag parsing and environment variable overrides (using the "MCPANY_" prefix).
//
// Parameters:
//   - cmd: *cobra.Command
...
// BindServerFlags binds server-specific command-line flags to the Viper configuration registry. It defines flags specific to the server operation, such as port configurations, authentication keys, and runtime modes (e.g., stdio).
//
// Summary: binds server-specific command-line flags to the Viper configuration registry. It defines flags specific to the server operation, such as port configurations, authentication keys, and runtime modes (e.g., stdio).
//
// Parameters:
//   - cmd: *cobra.Command
...
// BindFlags binds both root and server-specific command line flags to the Viper configuration registry. This is a helper function that orchestrates the binding of all necessary flags by delegating to BindRootFlags and BindServerFlags.
//
// Summary: binds both root and server-specific command line flags to the Viper configuration registry. This is a helper function that orchestrates the binding of all necessary flags by delegating to BindRootFlags and BindServerFlags.
//
// Parameters:
//   - cmd: *cobra.Command
