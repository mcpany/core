...
func (s *FileStore) Load(ctx context.Context) (*configv1.McpAnyServerConfig, error) {
	...
	for i, path := range filePaths {
		...
		g.Go(func() error {
			cfg, err := s.processConfigFile(ctx, path)
			if err != nil {
				return err
			}
			configs[i] = cfg
...
func (s *FileStore) processConfigFile(ctx context.Context, path string) (*configv1.McpAnyServerConfig, error) {
	var b []byte
	var err error
	if isURL(path) {
...
func resolveEnvValue(root proto.Message, path []string, value string) interface{} {
	...
	for i := 0; i < len(path); i++ {
		...
		if i == len(path)-1 {
			...
			if strings.HasPrefix(trimmed, "{") {
				// Handle single message (not repeated) that might be passed as JSON string in env var
				// This handles cases where a message field is set via ENV_VAR='{"field": "val"}'
				// and not just repeated fields.
				// However, if it's not valid JSON, we fall through to treat it as string or let conversion fail.
				// This branch is for "elseif" lint fix, but logical behavior preservation is key.
				var jsonObj map[string]interface{}
				if json.Unmarshal([]byte(value), &jsonObj) == nil {
					return jsonObj
				}
