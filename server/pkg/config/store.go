...
func (s *FileStore) Load(ctx context.Context) (*configv1.McpAnyServerConfig, error) {
	...
	for i, path := range filePaths {
		...
		g.Go(func() error {
			cfg, err := s.loadSingleConfigFile(ctx, path)
			if err != nil {
				if s.skipErrors {
...
func (s *FileStore) loadSingleConfigFile(ctx context.Context, path string) (*configv1.McpAnyServerConfig, error) {
	var b []byte
	var err error
	if isURL(path) {
	...
	if err := engine.Unmarshal(b, cfg); err != nil {
		return nil, enhanceUnmarshalError(path, b, err)
	}

	return cfg, nil
}

func enhanceUnmarshalError(path string, b []byte, err error) error {
...
func mergeConfigs(configs []*configv1.McpAnyServerConfig) *configv1.McpAnyServerConfig {
