...
func MigrateClaudeConfig(input []byte) (string, error) {
	...
	for name, server := range claudeCfg.MCPServers {
		...
		mcpService := &configpb.UpstreamServiceConfig{
			Name: &name,
			ServiceConfig: &configpb.UpstreamServiceConfig_McpService{
				McpService: &configpb.McpUpstreamService{
					...
					ToolAutoDiscovery: toBoolPtr(true), // Assume we want to discover tools
				},
			},
		}

		mcpAnyConfig.UpstreamServices = append(mcpAnyConfig.UpstreamServices, mcpService)
	}
