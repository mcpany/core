...
func TestSftpProvider_Extended(t *testing.T) {
	...
	t.Run("Stat on file", func(t *testing.T) {
		f := filepath.Join(tmpDir, "stat.txt")
	...
	t.Run("Seek", func(t *testing.T) {
		f := filepath.Join(tmpDir, "seek.txt")
		err := os.WriteFile(f, []byte("0123456789"), 0644)
		require.NoError(t, err)
	...
	t.Run("Truncate", func(t *testing.T) {
		f := filepath.Join(tmpDir, "trunc.txt")
		err := os.WriteFile(f, []byte("longcontent"), 0644)
		require.NoError(t, err)

		// Open for writing
		file, err := fs.OpenFile(f, os.O_RDWR, 0644)
		require.NoError(t, err)
		defer file.Close()
