...
// LocalProvider is a filesystem provider that uses the local OS filesystem.
...
// ResolvePath resolves a virtual path to a real path on the OS filesystem.
...
// Close closes the provider (no-op for local).
