...
func TestHTTPUpstream_QueryMerge_InvalidValueOverride(t *testing.T) {
	testCases := []struct {
		name          string
		address       string
		endpointPath  string
		...
		{
			name:         "base param with invalid value should be overridden by endpoint param",
			address:      "http://example.com/api?foo=bad%",
			endpointPath: "/v1/test?foo=good",
			expectedFqn:  "GET http://example.com/api/v1/test?foo=good",
		},
	}
