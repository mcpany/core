...
func TestHTTPUpstream_URLConstruction_DoubleSlashEncodingBug(t *testing.T) {
	testCases := []struct {
		name          string
		address       string
		endpointPath  string
		...
		{
			name:         "endpoint path with double slash and encoded user info",
			address:      "http://example.com/api",
			// "a%2Fb" should remain "a%2Fb" in the final URL, not become "a%252Fb"
			endpointPath: "//user:a%2Fb@foo/bar",
			expectedFqn:  "GET http://example.com/api//user:a%2Fb@foo/bar",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			configJSON := `{
				"name": "double-slash-encoding-bug-service",
				"http_service": {
					"address": "` + tc.address + `",
