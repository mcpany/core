...
func TestHTTPUpstream_URLConstruction_EncodedDoubleSlashBug(t *testing.T) {
	testCases := []struct {
		name          string
		address       string
		endpointPath  string
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			configJSON := `{
				"name": "encoded-double-slash-service",
				"http_service": {
					"address": "` + tc.address + `",
			...
			// If the bug exists, the tool won't be registered because of url.Parse error
			if !ok {
				t.Fatalf("Tool was not registered, likely due to url.Parse error")
			}
