...
func TestHTTPUpstream_URLConstruction_OrderBug(t *testing.T) {
	testCases := []struct {
		name          string
		address       string
		endpointPath  string
		...
		{
			name:         "invalid query param first should be preserved in order",
			address:      "http://example.com/api",
			endpointPath: "/v1/test?invalid=%&valid=1",
			// We expect the invalid param to stay first.
			expectedFqn:  "GET http://example.com/api/v1/test?invalid=%&valid=1",
		},
	}
