...
func TestHTTPUpstream_URLConstruction_InvalidQueryOverride(t *testing.T) {
	testCases := []struct {
		...
		{
			name:         "endpoint with invalid query param should override base param",
			address:      "http://example.com/api?q=default",
			endpointPath: "/test?q=weird%",
			expectedFqn:  "GET http://example.com/api/test?q=weird%",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			configJSON := `{
				"name": "invalid-query-override-service",
				"http_service": {
					"address": "` + tc.address + `",
