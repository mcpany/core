...
// TestHTTPUpstream_URLConstruction_QueryPreservation ...
// Summary: TestHTTPUpstream_URLConstruction_QueryPreservation
// Parameters:
//   - None.
// Returns:
//   - None.
...
//   - None.
	testCases := []struct {
		...
		{
			name:         "base url with complex encoding should be preserved if no merge needed",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			configJSON := `{
				"name": "query-preservation-service",
				"http_service": {
					"address": "` + tc.address + `",
