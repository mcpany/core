...
func (u *Upstream) createAndRegisterMCPItemsFromBundle(
	...
	manifestFile, err := os.Open(manifestPath) //nolint:gosec // Path is internal and validated
	if err != nil {
...
func unzipBundle(src, dest string) error {
	...
	for _, f := range r.File {
		...
		outFile, err := os.OpenFile(fpath, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, f.Mode()) //nolint:gosec // Zip extraction path validated
		if err != nil {
			return err
		}
