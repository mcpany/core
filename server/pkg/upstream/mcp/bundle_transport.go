...
func setUnexportedID(idPtr interface{}, val interface{}) error {
	...
	f = reflect.NewAt(f.Type(), unsafe.Pointer(f.UnsafeAddr())).Elem() //nolint:gosec // Unsafe required for reflection
...
func fixID(id interface{}) interface{} {
	...
	if f.CanAddr() {
		...
		rf := reflect.NewAt(f.Type(), unsafe.Pointer(f.UnsafeAddr())).Elem() //nolint:gosec // Unsafe required for reflection
