...
func TestGraphQLUpstream_Register_ListHandling(t *testing.T) {
	...
	// Verify users query construction
	usersTool, ok := toolManager.GetTool("test-service.test-service-users")
	require.True(t, ok)
	usersCallable, ok := usersTool.(*tool.CallableTool)
	require.True(t, ok)
	usersQuery := usersCallable.Callable().(*Callable).query
	assert.Contains(t, usersQuery, "query ($ids: [ID!]!)")
	assert.Contains(t, usersQuery, "users(ids: $ids)")

	// Verify addTags mutation construction
	tagsTool, ok := toolManager.GetTool("test-service.test-service-addTags")
	require.True(t, ok)
	tagsCallable, ok := tagsTool.(*tool.CallableTool)
	require.True(t, ok)
	tagsQuery := tagsCallable.Callable().(*Callable).query
	assert.Contains(t, tagsQuery, "mutation ($tags: [String])")
	assert.Contains(t, tagsQuery, "addTags(tags: $tags)")
}
