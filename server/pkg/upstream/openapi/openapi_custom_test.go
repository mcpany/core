...
`
				...
				configv1.PromptDefinition_builder{
					...
					Messages: []*configv1.PromptMessage{
						configv1.PromptMessage_builder{
							...
							// Content oneof: Text, Image
							// Builder for oneof usually exposes fields directly if flattened.
							// If not flattened, check generated code.
							// Assuming flattened based on previous experience:
	...
	mockPromptManager.EXPECT().AddPrompt(gomock.Any()).Do(func(p prompt.Prompt) {
		// p.Prompt() returns *mcp.Prompt (SDK struct). It has .Name field.
	...
	mockResourceManager.EXPECT().AddResource(gomock.Any()).Do(func(r resource.Resource) {
		// r.Resource() returns *mcp.Resource (SDK struct). It has .Name field.
