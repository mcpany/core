...
//
// It initializes a resource that proxies read requests to an underlying tool execution.
//
// Parameters:
//   - def: *configv1.ResourceDefinition. The definition of the resource from configuration.
//   - t: tool.Tool. The tool to execute when the resource is read.
//
// Returns:
...
//
// Returns:
//   - *mcp.Resource: The MCP resource definition including URI, name, and MIME type.
...
//
// It triggers the tool execution with an empty input and converts the result
// into the appropriate MCP resource content format (text or blob).
//
// Parameters:
...
//   - error: An error if the tool execution fails or the result type is unsupported.
