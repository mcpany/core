...
var (
// Summary: ErrPoolClosed is returned when an operation is attempted on a closed pool. Defines ErrPoolClosed.
//
// Parameters:
//   - None.
//
...
// Summary: ErrPoolFull is returned when the pool has reached its maximum capacity and cannot create new clients. Defines the ErrPoolFull constant or variable.
//
// Parameters:
//   - None.
//
...
// Summary: ClosableClient defines the interface for clients that can be managed by the connection pool. Implementations must provide methods for closing the connection and checking its health. Interface for poolable clients.
//
// Parameters:
//   - None.
//
...
// Summary: Pool defines the interface for a generic connection pool. Interface for a connection pool.
//
// Parameters:
//   - None.
//
...
// Summary: New creates a new connection pool with the specified factory and size constraints. Creates a new generic pool.
//
// Parameters:
//   - factory (func(context.Context) (T, error)): The factory parameter.
//   - initialSize (int): The initialSize parameter.
//   - maxIdleSize (int): The maxIdleSize parameter.
//   - maxSize (int): The maxSize parameter.
//   - _ (time.Duration): The _ parameter.
//   - disableHealthCheck (bool): The disableHealthCheck parameter.
//
// Returns:
//   - Pool[T]: The resulting Pool[T].
//   - error: An error if the operation fails.
//
// Errors:
...
// Summary: Get retrieves a client from the pool. Acquires a client, creating one if necessary.
//
// Parameters:
...
// Summary: Put returns a client to the pool for reuse. Returns a client to the pool.
//
// Parameters:
...
// Summary: Close shuts down the pool, closing all idle clients. Closes the pool.
//
// Parameters:
//   - None.
//
...
// Summary: Len returns the current number of idle clients in the pool. Returns idle client count.
//
// Parameters:
//   - None.
//
...
// Summary: UntypedPool defines a non-generic interface for a pool. Interface for untyped pool management.
//
// Parameters:
//   - None.
//
...
// Summary: Manager provides a way to manage multiple named connection pools. Manages a collection of pools.
//
// Parameters:
//   - None.
//
...
// Summary: NewManager creates and returns a new pool Manager. Initializes a new Pool Manager.
//
// Parameters:
//   - None.
//
...
// Summary: Register adds a new pool to the manager under a given name. Registers a pool by name.
//
// Parameters:
...
// Summary: Deregister closes and removes a pool from the manager. Removes a pool by name.
//
// Parameters:
//   - name (string): The name parameter.
//
...
// Summary: Get retrieves a typed pool from the manager by name. Retrieves a pool by name and type.
//
// Parameters:
//   - m (*Manager): The m parameter.
//   - name (string): The name parameter.
//
// Returns:
//   - Pool[T]: The resulting Pool[T].
...
// Summary: CloseAll iterates through all registered pools in the manager and closes them. Summary: Closes all managed pools.
//
// Parameters:
//   - None.
//
