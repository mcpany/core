...
func TestLocalCommandTool_SSRF_Vulnerability(t *testing.T) {
	...
	if err != nil {
		// Can be either "potential SSRF detected" (from checkForSSRF) or "unsafe url argument" (from validateSafePathAndInjection)
		isSSRF := strings.Contains(err.Error(), "potential SSRF detected")
		isUnsafe := strings.Contains(err.Error(), "unsafe url argument")
		assert.True(t, isSSRF || isUnsafe, "Error should indicate SSRF or unsafe URL, got: %s", err.Error())
	}
