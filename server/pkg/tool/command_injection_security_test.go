...
func TestCommandInjection_Advanced(t *testing.T) {
	...
	t.Run("versioned_binary_bypass", func(t *testing.T) {
		...
		_, err := tool.Execute(context.Background(), req)
		assert.Error(t, err)
		// It may return "interpreter injection detected" due to keyword detection (import, os)
		// or "shell injection detected" due to unquoted dangerous chars
		if !strings.Contains(err.Error(), "shell injection detected") && !strings.Contains(err.Error(), "interpreter injection detected") {
			t.Errorf("Expected shell/interpreter injection error, got: %v", err)
		}
	})
