...
func TestLocalCommandTool_SecretInArgs_Override(t *testing.T) {
	...
	if len(args) > 0 {
		t.Logf("Args: %v", args)
		// We assert that the secret value is used, but we expect this to fail currently
		if args[0] != "super-secret-value" {
			t.Logf("VULNERABILITY CONFIRMED: Expected 'super-secret-value', got '%s'", args[0])
		}
