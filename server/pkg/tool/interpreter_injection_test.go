...
func TestSedSandbox_Prevention(t *testing.T) {
	...
	// Payload: 1edate (Execute 'date') - No spaces to avoid shell injection check, testing sed sandbox directly
	req := &ExecutionRequest{
		ToolName: "sed-tool",
		ToolInputs: []byte(`{"script": "1edate"}`),
	}
	...
	if returnCode == 0 {
		t.Errorf("FAIL: sed executed '1edate' successfully (return_code 0). Sandbox failed.")
	}
	...
	// Payload: w/tmp/pwned (Write file) - No spaces
	req = &ExecutionRequest{
		ToolName: "sed-tool",
