...
func TestSedSandbox_Prevention(t *testing.T) {
	...
	// If validation blocks it (due to space), that's ALSO a pass for security.
	// But if it executes, we check return code.
	if err != nil {
		if strings.Contains(err.Error(), "shell injection detected") {
			t.Logf("Success: Shell injection validation blocked dangerous payload: %v", err)
		} else {
