...
func TestZipSecurity(t *testing.T) {
	...
	t.Run("zip_TT_injection", func(t *testing.T) {
		...
		if err == nil {
			...
			// The validation error we want is "shell injection detected" or "interpreter injection detected"
			if !strings.Contains(err.Error(), "shell injection detected") && !strings.Contains(err.Error(), "interpreter injection detected") {
				t.Fatalf("Expected shell or interpreter injection error, got: %v (Vulnerability present: validation passed)", err)
			}
		}
	})
