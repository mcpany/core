...
func TestLocalCommandTool_Perl_Open_Injection(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			payload: `open F, ">hacked"`,
			blocked: true, // Should be blocked by quote check
		},
		{
			name:    "Bypass exec q/ls/",
			payload: `exec q/ls/`,
			blocked: true, // Should be blocked but likely bypassed!
		},
	}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.blocked {
				if err == nil {
					...
					t.Logf("Payload %q blocked with error: %v", tc.payload, err)
...
func TestLocalCommandTool_Perl_Safe_Quoted(t *testing.T) {
	// Regression test: Ensure we don't block valid words inside quotes

	toolDef := v1.Tool_builder{
		Name: proto.String("perl-safe"),
	}.Build()
	...
	callDef := configv1.CommandLineCallDefinition_builder{
		...
		// Template uses quotes!
		Args: []string{"-e", "print '{{msg}}'"},
	}.Build()

	localTool := NewLocalCommandTool(toolDef, service, callDef, nil, "call-id")

	// Payload contains "open" and "system" but is safe inside quotes (treated as string data)
	payload := "System is open"

	req := &ExecutionRequest{
		ToolName: "perl-safe",
		ToolInputs: []byte(`{"msg": "` + payload + `"}`),
	}

	_, err := localTool.Execute(context.Background(), req)

	if err != nil {
		t.Fatalf("Safe quoted payload blocked: %v", err)
	}
}
