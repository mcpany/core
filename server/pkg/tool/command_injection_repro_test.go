...
func TestMyPythonExecInjection(t *testing.T) {
	val := "exec \"print(1)\""
	err := checkForShellInjection(val, "{{val}}", "{{val}}", "python", false)
	if err == nil {
		t.Fatalf("Vulnerability confirmed: 'exec \"print(1)\"' passed validation.")
	} else {
        t.Logf("Blocked with error: %v", err)
    }
}

func TestMyPythonEvalInjection(t *testing.T) {
	val := "eval('__import__(\\'os\\').system(\\'ls\\')')"
	err := checkForShellInjection(val, "{{val}}", "{{val}}", "python", false)
	if err == nil {
		t.Fatalf("Vulnerability confirmed: eval passed validation.")
	} else {
        t.Logf("Blocked with error: %v", err)
    }
}

func TestMyPythonExecfileInjection(t *testing.T) {
...
func TestDoubleQuotesDoubleQuoteBypass2(t *testing.T) {
	val := "\");import os;os.system(\"ls"
	err := checkForShellInjection(val, "\"{{val}}\"", "{{val}}", "python", false)
	if err == nil {
		t.Fatalf("Vulnerability confirmed: bypass via quote passed validation.")
	} else {
        t.Logf("Blocked with error: %v", err)
    }
}
