...
func TestLocalCommandTool_Git_Ext_RCE_Repro(t *testing.T) {
    ...
    if err == nil {
        t.Errorf("Security check failed: Execute should have returned an error for ext:: payload")
    } else {
        if !strings.Contains(err.Error(), "dangerous scheme detected") {
            t.Errorf("Unexpected error: %v (expected 'dangerous scheme detected')", err)
        }
    }
}
