...
func TestLocalCommandTool_SensitiveFileAccess_Blocked(t *testing.T) {
    ...
    // Sentinel Security Update: We expect an error now because .env access is blocked.
    assert.Error(t, err)
    if err != nil {
        assert.Contains(t, err.Error(), "access to .env files is restricted")
    }
    assert.Nil(t, result)
}
