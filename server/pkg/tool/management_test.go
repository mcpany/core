...
func TestToolManager_ListServices(t *testing.T) {
	tm := NewManager(nil)
	service1 := &ServiceInfo{Name: "Service 1"}
	service2 := &ServiceInfo{Name: "Service 2"}

	tm.AddServiceInfo("s1", service1)
	tm.AddServiceInfo("s2", service2)

	services := tm.ListServices()
	assert.Len(t, services, 2)
	// Map iteration order is random, so we can't assert order easily without sorting,
...
func TestToolManager_ProfileFiltering_Advanced(t *testing.T) {
	tm := NewManager(nil)

	// Profile with BOTH tags and properties
	profileDef := &configv1.ProfileDefinition{
		Name: ptr("strict-profile"),
		Selector: &configv1.ProfileSelector{
			Tags: []string{"secure"},
			ToolProperties: map[string]string{
				"idempotent": "true",
	...
	// Profile with nil selector (should match nothing via matchesSelector)
	emptyProfileDef := &configv1.ProfileDefinition{
		Name:     ptr("empty-profile"),
		Selector: nil, // Explicitly nil
	}

	tm.SetProfiles([]string{"strict-profile", "empty-profile"}, []*configv1.ProfileDefinition{profileDef, emptyProfileDef})
	...
	tool2 := &MockTool{
		ToolFunc: func() *v1.Tool {
			return &v1.Tool{
				...
				Tags:      []string{"secure"},
				Annotations: &v1.ToolAnnotations{
					IdempotentHint: proto.Bool(false),
				},
			}
		},
	}

	// Tool 3: Matches only property
	tool3 := &MockTool{
		ToolFunc: func() *v1.Tool {
			return &v1.Tool{
				ServiceId: proto.String("s1"),
	...
	tools := tm.ListTools()
	assert.Len(t, tools, 1)
	assert.Equal(t, "t1", tools[0].Tool().GetName())
}

func TestToolManager_ProfileFiltering_OtherProperties(t *testing.T) {
	tm := NewManager(nil)

	profileDef := &configv1.ProfileDefinition{
		Name: ptr("props-profile"),
		Selector: &configv1.ProfileSelector{
			ToolProperties: map[string]string{
				"destructive": "false",
				"open_world":  "true",
	...
	tm.SetProfiles([]string{"props-profile"}, []*configv1.ProfileDefinition{profileDef})
	...
	// Non-matching tool (wrong destructive)
	tool2 := &MockTool{
		ToolFunc: func() *v1.Tool {
			return &v1.Tool{
				ServiceId: proto.String("s1"),
	...
	// Non-matching tool (wrong open_world)
	tool3 := &MockTool{
		ToolFunc: func() *v1.Tool {
			return &v1.Tool{
				ServiceId: proto.String("s1"),
...
func TestToolManager_ProfileFiltering_UnknownProperty(t *testing.T) {
	tm := NewManager(nil)

	profileDef := &configv1.ProfileDefinition{
		Name: ptr("unknown-prop-profile"),
		Selector: &configv1.ProfileSelector{
			ToolProperties: map[string]string{
				"unknown_key": "true",
			},
	...
	tm.SetProfiles([]string{"unknown-prop-profile"}, []*configv1.ProfileDefinition{profileDef})
