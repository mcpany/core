...
func TestSentinelRCE_PerlArrayInterpolation(t *testing.T) {
	...
	// We use qw() to avoid using double quotes which are blocked by shell injection check
	payload := `@{[system qw(echo pwned)]}`
	...
	assert.Nil(t, res)
	assert.Contains(t, err.Error(), "variable interpolation injection detected", "Error should mention interpolation injection")
}
