...
type HTTPTool struct {
	...
	paramInPath          []bool
	paramInQueryTemplate []bool // True if parameter is present in the query string template
	paramInQuery         []bool // True if explicitly mapped to query (or implicitly if in template)
	paramInHeader        []bool
	paramInCookie        []bool
...
func NewHTTPTool(tool *v1.Tool, poolManager *pool.Manager, serviceID string, authenticator auth.UpstreamAuthenticator, callDefinition *configv1.HttpCallDefinition, cfg *configv1.ResilienceConfig, policies []*configv1.CallPolicy, callID string) *HTTPTool {
	...
	t.paramInQueryTemplate = make([]bool, len(callDefinition.GetParameters()))
	...
	for i, param := range callDefinition.GetParameters() {
		if schema := param.GetSchema(); schema != nil {
			...
			// Check template presence (implicit mapping)
			if strings.Contains(pathStr, placeholder) {
				t.paramInPath[i] = true
			}
			if strings.Contains(queryStr, placeholder) {
				t.paramInQueryTemplate[i] = true
				t.paramInQuery[i] = true
			}

			// Apply explicit location override
			loc := param.GetLocation()
			switch loc {
			case configv1.ParameterLocation_PARAMETER_LOCATION_QUERY:
				t.paramInQuery[i] = true
			case configv1.ParameterLocation_PARAMETER_LOCATION_HEADER:
				t.paramInHeader[i] = true
			case configv1.ParameterLocation_PARAMETER_LOCATION_COOKIE:
				t.paramInCookie[i] = true
			case configv1.ParameterLocation_PARAMETER_LOCATION_PATH:
				t.paramInPath[i] = true
			}
		}
	}
...
func (t *HTTPTool) prepareInputsAndURL(ctx context.Context, req *ExecutionRequest) (map[string]any, string, map[string]string, map[string]string, bool, error) {
	...
	pathReplacements, queryReplacements, extraQueryParams, headerParams, cookieParams, inputsModified, err := t.processParameters(ctx, inputs)
	if err != nil {
	...
	if len(extraQueryParams) > 0 {
		if queryBuf.Len() > 0 && !strings.HasSuffix(queryBuf.String(), "&") && !strings.HasSuffix(queryBuf.String(), "?") {
			queryBuf.WriteString("&")
		}

		// Sort keys for deterministic output
		keys := make([]string, 0, len(extraQueryParams))
		for k := range extraQueryParams {
			keys = append(keys, k)
		}
		sort.Strings(keys)
		...
		for i, k := range keys {
			...
			queryBuf.WriteString("=")
			queryBuf.WriteString(extraQueryParams[k])
		}
	}
	...
	return inputs, urlString, headerParams, cookieParams, inputsModified, nil
}
...
func (t *HTTPTool) processParameters(ctx context.Context, inputs map[string]any) (map[string]string, map[string]string, map[string]string, map[string]string, map[string]string, bool, error) {
	...
	extraQueryParams := make(map[string]string, len(t.parameters))
	headerParams := make(map[string]string, len(t.parameters))
	cookieParams := make(map[string]string, len(t.parameters))
	...
	for i, param := range t.parameters {
		...
		if secret := param.GetSecret(); secret != nil {
			...
			if !ok {
				...
				// If optional and missing, ignore unless it has default value or treat as empty?
				// Logic above treated as empty string.
				val = ""
			} else if t.paramInPath[i] || t.paramInQuery[i] || t.paramInHeader[i] || t.paramInCookie[i] {
				// Only delete from inputs if it was present AND used in explicit mapping (path/query/header/cookie)
				// If it's a body param (implicit), we leave it in inputs.
				delete(inputs, name)
				inputsModified = true
			}
		...
		if isResolved {
			...
			if t.paramInQuery[i] {
				...
				if t.paramInQueryTemplate[i] {
					queryReplacements[name] = encodedVal
				} else {
					// Explicitly mapped to query but NOT in template
					// Note: If we use {{param}} in template, we rely on queryReplacements.
					// If we set location=QUERY, we might want it appended.
					extraQueryParams[name] = encodedVal
				}
			}
	...
	return pathReplacements, queryReplacements, extraQueryParams, headerParams, cookieParams, inputsModified, nil
}
