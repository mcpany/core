...
func checkSQLInjection(val, base string, quoteLevel int) error {
	...
	if base == "sqlite3" {
		if err := checkSQLiteInjection(val); err != nil {
			return err
		}
	}
	...
	if base == "psql" {
		if err := checkPSQLInjection(val); err != nil {
			return err
		}
	}

	return checkUnquotedSQLKeywords(val)
}

func checkSQLiteInjection(val string) error {
...
func checkPSQLInjection(val string) error {
	valTrimmed := strings.TrimSpace(val)
	...
	valLower := strings.ToLower(valTrimmed)
	if strings.HasPrefix(valLower, "\\!") || strings.HasPrefix(valLower, "\\o") || strings.HasPrefix(valLower, "\\copy") {
...
func checkUnquotedSQLKeywords(val string) error {
