...
func checkSQLInjection(val, base string, quoteLevel int) error {
	...
	if base == "psql" {
		if err := checkPsqlInjection(valTrimmed, val); err != nil {
			return err
		}
	}

	// General SQL keyword check
	return checkGeneralSQLKeywords(val)
}
...
func checkPsqlInjection(valTrimmed, val string) error {
...
func checkGeneralSQLKeywords(val string) error {
