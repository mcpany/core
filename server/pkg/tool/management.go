...
// MCPServerProvider defines an interface for components that can provide an instance of an *mcp.Server. Summary: Interface for providing an MCP server instance. This interface is used to decouple the Manager from the concrete server implementation, avoiding circular dependencies.
//
// Summary: MCPServerProvider defines an interface for components that can provide an instance of an *mcp.Server. Summary: Interface for providing an MCP server instance. This interface is used to decouple the Manager from the concrete server implementation, avoiding circular dependencies.
//
// Methods:
//   - Defines the required contract and behavior for implementations of MCPServerProvider.
...
// ManagerInterface defines the contract for a tool manager. Summary: Interface for tool management operations. It outlines the methods required for managing the lifecycle, registration, discovery, and execution of tools within the MCP Any server.
//
// Summary: ManagerInterface defines the contract for a tool manager. Summary: Interface for tool management operations. It outlines the methods required for managing the lifecycle, registration, discovery, and execution of tools within the MCP Any server.
//
// Methods:
//   - Defines the required contract and behavior for implementations of ManagerInterface.
...
// ExecutionMiddleware defines the interface for middleware that intercepts tool execution. Summary: Interface for tool execution middleware.
//
// Summary: ExecutionMiddleware defines the interface for middleware that intercepts tool execution. Summary: Interface for tool execution middleware.
//
// Methods:
//   - Defines the required contract and behavior for implementations of ExecutionMiddleware.
...
// Manager is the central component for managing tools in MCP Any. Summary: Central manager for tool lifecycle and execution. It handles tool registration, retrieval, execution, and profile-based filtering. It is thread-safe and supports efficient lookups via caching and indexing.
//
// Summary: Manager is the central component for managing tools in MCP Any. Summary: Central manager for tool lifecycle and execution. It handles tool registration, retrieval, execution, and profile-based filtering. It is thread-safe and supports efficient lookups via caching and indexing.
//
// Fields:
...
// SetProfiles sets the enabled profiles and their definitions for filtering. Summary: Configures profiles for filtering. This method updates the internal state used for profile-based access control. It pre-computes allowed services for each profile to optimize lookup performance. Parameters: - enabled ([]string): A list of names of the currently active profiles. - defs ([]*configv1.ProfileDefinition): The definitions of all available profiles. Side Effects: - Updates internal profile definitions and caches.
//
// Summary: SetProfiles sets the enabled profiles and their definitions for filtering. Summary: Configures profiles for filtering. This method updates the internal state used for profile-based access control. It pre-computes allowed services for each profile to optimize lookup performance. Parameters: - enabled ([]string): A list of names of the currently active profiles. - defs ([]*configv1.ProfileDefinition): The definitions of all available profiles. Side Effects: - Updates internal profile definitions and caches.
//
// Parameters:
//   - enabled ([]string): The enabled parameter used in the operation.
//   - defs ([]*configv1.ProfileDefinition): The defs parameter used in the operation.
//
// Returns:
//   - None.
//
...
// IsServiceAllowed checks if a service is allowed for a given profile. Summary: Checks if a service is allowed for a profile. Parameters: - serviceID (string): The unique identifier of the service. - profileID (string): The identifier of the profile to check. Returns: - bool: True if the service is allowed, false otherwise.
//
// Summary: IsServiceAllowed checks if a service is allowed for a given profile. Summary: Checks if a service is allowed for a profile. Parameters: - serviceID (string): The unique identifier of the service. - profileID (string): The identifier of the profile to check. Returns: - bool: True if the service is allowed, false otherwise.
//
// Parameters:
...
// ToolMatchesProfile checks if a tool matches a given profile. Summary: Checks if a tool matches a profile. It delegates to the internal toolMatchesProfile method. Parameters: - tool (Tool): The tool definition. - profileID (string): The identifier of the profile. Returns: - bool: True if the tool matches the profile, false otherwise.
//
// Summary: ToolMatchesProfile checks if a tool matches a given profile. Summary: Checks if a tool matches a profile. It delegates to the internal toolMatchesProfile method. Parameters: - tool (Tool): The tool definition. - profileID (string): The identifier of the profile. Returns: - bool: True if the tool matches the profile, false otherwise.
//
// Parameters:
...
// GetAllowedServiceIDs returns a map of allowed service IDs for a given profile. Summary: Retrieves allowed service IDs for a profile. The returned map uses the service ID as the key and a boolean true as the value. Parameters: - profileID (string): The identifier of the profile. Returns: - map[string]bool: A map of allowed service IDs. - bool: True if the profile exists and has cached allowed services.
//
// Summary: GetAllowedServiceIDs returns a map of allowed service IDs for a given profile. Summary: Retrieves allowed service IDs for a profile. The returned map uses the service ID as the key and a boolean true as the value. Parameters: - profileID (string): The identifier of the profile. Returns: - map[string]bool: A map of allowed service IDs. - bool: True if the profile exists and has cached allowed services.
//
// Parameters:
//   - profileID (string): The unique identifier used to reference the profile resource.
//
...
// GetToolCountForService returns the number of tools registered for a given service. Summary: Counts tools for a service. It first checks if the service is healthy; if not, it returns 0. Parameters: - serviceID (string): The unique identifier of the service. Returns: - int: The number of active tools for the service.
//
// Summary: GetToolCountForService returns the number of tools registered for a given service. Summary: Counts tools for a service. It first checks if the service is healthy; if not, it returns 0. Parameters: - serviceID (string): The unique identifier of the service. Returns: - int: The number of active tools for the service.
//
// Parameters:
...
// AddMiddleware adds a middleware to the tool manager. Summary: Adds execution middleware. The middleware will be executed as part of the tool execution chain. Parameters: - middleware (ExecutionMiddleware): The middleware instance to add. Side Effects: - Appends middleware to the internal list.
//
// Summary: AddMiddleware adds a middleware to the tool manager. Summary: Adds execution middleware. The middleware will be executed as part of the tool execution chain. Parameters: - middleware (ExecutionMiddleware): The middleware instance to add. Side Effects: - Appends middleware to the internal list.
//
// Parameters:
//   - middleware (ExecutionMiddleware): The unique identifier used to reference the mdleware resource.
//
// Returns:
//   - None.
//
...
// SetMCPServer stores the mcp server into the persistent storage.
//
// Summary: Stores the mcp server into the persistent storage.
//
// Parameters:
//   - mcpServer (MCPServerProvider): The mcp server parameter used in the operation.
//
// Returns:
//   - None.
//
...
// 3. Running pre-execution hooks and policies.
...
// AddServiceInfo registers metadata about a service. Summary: Registers service metadata. It processes the service configuration to set up hooks and call policies. Parameters: - serviceID (string): The unique identifier for the service. - info (*ServiceInfo): The ServiceInfo struct containing the service's metadata and configuration. Side Effects: - Stores service info in the internal map.
//
// Summary: AddServiceInfo registers metadata about a service. Summary: Registers service metadata. It processes the service configuration to set up hooks and call policies. Parameters: - serviceID (string): The unique identifier for the service. - info (*ServiceInfo): The ServiceInfo struct containing the service's metadata and configuration. Side Effects: - Stores service info in the internal map.
//
// Parameters:
//   - serviceID (string): The unique identifier used to reference the service resource.
//   - info (*ServiceInfo): The info parameter used in the operation.
//
// Returns:
//   - None.
//
...
// GetServiceInfo retrieves the metadata for a registered service. Summary: Retrieves service metadata. It returns a copy of the service info with secrets stripped from the configuration for security. Parameters: - serviceID (string): The unique identifier for the service. Returns: - *ServiceInfo: The service information. - bool: True if the service was found, false otherwise.
//
// Summary: GetServiceInfo retrieves the metadata for a registered service. Summary: Retrieves service metadata. It returns a copy of the service info with secrets stripped from the configuration for security. Parameters: - serviceID (string): The unique identifier for the service. Returns: - *ServiceInfo: The service information. - bool: True if the service was found, false otherwise.
//
// Parameters:
//   - serviceID (string): The unique identifier used to reference the service resource.
//
...
// ListServices returns a list of all currently registered services. Summary: Lists all services. It strips secrets from the service configurations before returning them. Returns: - []*ServiceInfo: A slice of ServiceInfo structs.
//
// Summary: ListServices returns a list of all currently registered services. Summary: Lists all services. It strips secrets from the service configurations before returning them. Returns: - []*ServiceInfo: A slice of ServiceInfo structs.
//
// Parameters:
//   - None.
//
...
// GetTool retrieves a tool from the manager by its fully qualified name. Summary: Retrieves a tool by name. It first attempts to look up the tool by its exact ID (useful if the client sends the ID). If that fails, it looks up the tool by its client-facing name (using the name map). Parameters: - toolName (string): The name or ID of the tool to retrieve. Returns: - Tool: The tool instance. - bool: True if the tool was found, false otherwise.
//
// Summary: GetTool retrieves a tool from the manager by its fully qualified name. Summary: Retrieves a tool by name. It first attempts to look up the tool by its exact ID (useful if the client sends the ID). If that fails, it looks up the tool by its client-facing name (using the name map). Parameters: - toolName (string): The name or ID of the tool to retrieve. Returns: - Tool: The tool instance. - bool: True if the tool was found, false otherwise.
//
// Parameters:
...
// ListTools returns a list of all tools currently registered with the manager. Summary: Lists all tools. It uses a read-through cache to avoid rebuilding the list on every call. Returns: - []Tool: A slice of Tool instances.
//
// Summary: ListTools returns a list of all tools currently registered with the manager. Summary: Lists all tools. It uses a read-through cache to avoid rebuilding the list on every call. Returns: - []Tool: A slice of Tool instances.
//
// Parameters:
//   - None.
//
...
// ListMCPTools returns a slice containing all the tools currently registered with the manager in MCP format. Summary: Lists all tools in MCP format. It maintains a cache of the MCP tool definitions to minimize overhead. Returns: - []*mcp.Tool: A slice of MCP tool definitions. Side Effects: - Updates internal caches if they are invalid.
//
// Summary: ListMCPTools returns a slice containing all the tools currently registered with the manager in MCP format. Summary: Lists all tools in MCP format. It maintains a cache of the MCP tool definitions to minimize overhead. Returns: - []*mcp.Tool: A slice of MCP tool definitions. Side Effects: - Updates internal caches if they are invalid.
//
// Parameters:
//   - None.
//
...
// ClearToolsForService removes all tools associated with a given service ID. Summary: Removes tools for a service. It efficiently cleans up internal indices and caches. Parameters: - serviceID (string): The unique identifier for the service. Side Effects: - Removes entries from the tools map and secondary indices. - Invalidates internal caches.
//
// Summary: ClearToolsForService removes all tools associated with a given service ID. Summary: Removes tools for a service. It efficiently cleans up internal indices and caches. Parameters: - serviceID (string): The unique identifier for the service. Side Effects: - Removes entries from the tools map and secondary indices. - Invalidates internal caches.
//
// Parameters:
//   - serviceID (string): The unique identifier used to reference the service resource.
//
