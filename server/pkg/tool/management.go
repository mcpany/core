...
type ManagerInterface interface {
	...
	//
	// Returns:
	//   - []*ServiceInfo: A slice of all registered services.
...
//
// Parameters:
//   - bus: *bus.Provider. The event bus.
//
// Returns:
...
//
// Parameters:
//   - enabled: []string. The names of enabled profiles.
//   - defs: []*configv1.ProfileDefinition. The profile definitions.
...
// Parameters:
//   - ctx: context.Context. The execution context.
//   - req: *ExecutionRequest. The request details containing tool name and inputs.
//
// Returns:
//   - any: The execution result.
//   - error: An error if the tool is not found, execution denied, or execution fails.
//
// Throws/Errors:
//   - ErrToolNotFound: If the tool does not exist.
//   - fmt.Errorf("service ... is currently unhealthy"): If the service is unhealthy.
//   - fmt.Errorf("tool execution denied..."): If policy or hook denies execution.
...
//
// It generates a unique tool ID and, if an MCP server is configured, registers
// a handler for the tool with the server.
//
// Parameters:
//   - tool: Tool. The tool to be added.
//
// Returns:
//   - error: An error if validation fails or registration with MCP server fails.
//
// Throws/Errors:
//   - fmt.Errorf("tool integrity check failed..."): If integrity check fails.
//   - fmt.Errorf("tool service ID cannot be empty"): If service ID is missing.
//   - fmt.Errorf("failed to sanitize tool name..."): If name sanitization fails.
...
//
// Returns:
//   - []*mcp.Tool: A slice of registered tools in MCP format.
