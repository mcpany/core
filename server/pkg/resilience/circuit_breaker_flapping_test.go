...
func TestCircuitBreaker_Flapping_Race(t *testing.T) {
	...
	// Give goroutines time to enter `Execute` and block on `startWork`
	// However, `Execute` checks state *before* calling `work`.
	// The first one will transition Open -> HalfOpen.
	// The second one will see HalfOpen.
	// Since `HalfOpenRequests` is 2, both should proceed to `work`.

	// We sleep a bit to ensure they are likely blocked on `startWork`
