...
// NewPostgresVectorStore creates a new PostgresVectorStore instance.
// It connects to the database and ensures the necessary schema and extensions exist.
//
// Parameters:
...
//  error: An error if the connection fails or schema initialization fails.
...
//  result (any): The result to store (marshaled to JSON).
...
// Parameters:
//  ctx (context.Context): The context for the request.
//  key (string): The scope key (optional, if empty prunes all).
