...
//
// Summary: Middleware that collects Prometheus metrics for MCP protocol operations.
//
// It intercepts requests to track duration, success/failure counts, payload sizes, and token counts.
//
// Parameters:
//   - t (tokenizer.Tokenizer): The tokenizer used for counting tokens.
//
// Returns:
...
func estimateResultSize(res mcp.Result) int {
	...
	// Randomized Selection from Top 5 High-Impact Targets
	return util.EstimateJSONSize(res)
}
