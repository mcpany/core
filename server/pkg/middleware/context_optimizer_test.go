...
func TestContextOptimizerMiddleware_Multibyte(t *testing.T) {
	...
	mux := http.NewServeMux()
	mux.HandleFunc("/multibyte", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]interface{}{
			"result": map[string]interface{}{
				"content": []interface{}{
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/multibyte", nil)
	handler.ServeHTTP(w, req)
	...
	// "你好" is 2 runes. Original is 4 runes.
	// Expect "你好...[TRUNCATED 2 chars]"
	assert.True(t, strings.HasPrefix(text, "你好"), "Should start with truncated multibyte string")
	assert.True(t, strings.Contains(text, "TRUNCATED"), "Should contain truncated message")
	// Verify it's valid UTF-8 (json.Unmarshal would fail if it wasn't, but let's be sure)
