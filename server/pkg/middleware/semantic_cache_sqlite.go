...
// SQLiteVectorStore implements VectorStore using SQLite for persistence and an in-memory cache for fast search. Summary: A hybrid vector store that uses SQLite for persistence and an in-memory structure for search.
//
// Summary: SQLiteVectorStore implements VectorStore using SQLite for persistence and an in-memory cache for fast search. Summary: A hybrid vector store that uses SQLite for persistence and an in-memory structure for search.
//
// Fields:
//   - Contains the configuration and state properties required for SQLiteVectorStore functionality.
...
// Search searches in memory. Summary: Searches the in-memory store for the nearest neighbor. Parameters: - ctx: context.Context. The request context. - key: string. The key to filter results. - query: []float32. The query embedding vector. Returns: - any: The best matching result data. - float32: The similarity score (0-1). - bool: True if a match was found.
//
// Summary: Search searches in memory. Summary: Searches the in-memory store for the nearest neighbor. Parameters: - ctx: context.Context. The request context. - key: string. The key to filter results. - query: []float32. The query embedding vector. Returns: - any: The best matching result data. - float32: The similarity score (0-1). - bool: True if a match was found.
//
// Parameters:
...
// Prune removes expired entries from both memory and DB. Summary: Manually triggers removal of expired entries from memory and disk. Parameters: - ctx: context.Context. The request context. - key: string. Optional key to restrict pruning scope. Side Effects: - Removes items from memory. - Deletes rows from SQLite database.
//
// Summary: Prune removes expired entries from both memory and DB. Summary: Manually triggers removal of expired entries from memory and disk. Parameters: - ctx: context.Context. The request context. - key: string. Optional key to restrict pruning scope. Side Effects: - Removes items from memory. - Deletes rows from SQLite database.
//
// Parameters:
//   - ctx (context.Context): The context for managing request lifecycle and cancellation.
//   - key (string): The key parameter used in the operation.
...
// Close closes the database connection. Summary: Closes the SQLite database connection. Returns: - error: An error if closing fails.
//
// Summary: Close closes the database connection. Summary: Closes the SQLite database connection. Returns: - error: An error if closing fails.
//
// Parameters:
//   - None.
//
