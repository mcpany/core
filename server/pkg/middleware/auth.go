	...
	return func(next mcp.MethodHandler) mcp.MethodHandler {
		return func(ctx context.Context, method string, req mcp.Request) (mcp.Result, error) {
			...
			if method == consts.MethodToolsCall {
				...
				} else if r, ok := req.(mcp.CallToolRequest); ok && r.Params != nil {
					if before, _, found := strings.Cut(r.Params.Name, "."); found {
						serviceID = before
					}
				}
			...
			if method == consts.MethodPromptsGet {
				...
				} else if r, ok := req.(mcp.GetPromptRequest); ok && r.Params != nil {
					if before, _, found := strings.Cut(r.Params.Name, "."); found {
						serviceID = before
					}
				}
			...
			if method == consts.MethodResourcesRead {
				...
				} else if r, ok := req.(mcp.ReadResourceRequest); ok && r.Params != nil {
					if before, _, found := strings.Cut(r.Params.URI, "://"); found {
						serviceID = before
					}
				}
