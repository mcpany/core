...
func TestVectorMath(t *testing.T) {
	...
	assert.Equal(t, []float32{0.6, 0.8}, normV)

	// normalize zero vector
	zeroV := []float32{0.0, 0.0}
	normZero, nZero := normalize(zeroV)
	assert.Equal(t, float32(0.0), nZero)
	assert.Equal(t, zeroV, normZero)

	// dotProduct (assuming normalized inputs for cosine similarity equivalent)
	v1 := []float32{1.0, 0.0}
	v2 := []float32{1.0, 0.0}
