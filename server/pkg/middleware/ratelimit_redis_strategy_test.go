...
func TestRedisStrategy(t *testing.T) {
	...
	t.Run("Create and Caching", func(t *testing.T) {
		...
		// 4. Different Service ID -> Should REUSE client for same config
		// The code now uses configHash as the primary key.
		// So if serviceID is different but config is same, it SHOULD find it and reuse it.
		limiter4, err := strategy.Create(ctx, "serviceB", "scope", "partition", config1)
		require.NoError(t, err)
		assert.NotNil(t, limiter4)
		assert.Len(t, createdClients, 2, "Should reuse client for same config even if serviceID differs")
	})
