...
// Summary: GzipCompressionMiddleware returns a middleware that compresses HTTP responses using Gzip. Middleware that compresses HTTP responses using Gzip if supported by the client.
//
// Parameters:
...
// Summary: Write writes the data to the connection as part of an HTTP reply. Writes data to the response, buffering until compression threshold is met.
//
// Parameters:
...
// Summary: WriteHeader captures the status code. Captures the status code for later writing.
//
// Parameters:
...
// Summary: Flush ⚡ BOLT: Implemented Flush to support streaming responses (e.g. SSE). Randomized Selection from Top 5 High-Impact Targets Flush implements the http.Flusher interface. Summary: Flushes the compressed stream to the client. Side Effects: - Forces any buffered data to be compressed and sent. - Flushes the underlying response writer.
//
// Parameters:
//   - None.
//
...
// Summary: Close closes the gzip writer and returns it to the pool. Summary: Closes the Gzip writer and releases resources. Side Effects: - Closes the active gzip writer. - Returns buffers to the pool. - Ensures all data is flushed to the client.
//
// Parameters:
//   - None.
//
