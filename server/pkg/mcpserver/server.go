...
//
// Summary: Orchestrates the handling of MCP (Model Context Protocol) requests by managing various components such as tools, prompts, resources, and services.
//
// Fields:
//   - server: The underlying MCP server instance.
//   - router: Internal router for dispatching requests.
//   - toolManager: Manages tool lifecycle and execution.
//   - promptManager: Manages prompt lifecycle and retrieval.
//   - resourceManager: Manages resource lifecycle and access.
//   - authManager: Handles request authentication.
//   - serviceRegistry: Registry of upstream services.
//   - bus: Event bus for asynchronous communication.
type Server struct {
	server          *mcp.Server
	router          *Router
...
//
// Summary: Provides access to the core MCP server functionality.
//
// Returns:
//   - *mcp.Server: The underlying server instance.
//
// Side Effects:
//   - Invokes `AddReceivingMiddlewareHook` if set (used for testing).
...
//
// Summary: Initializes the server, sets up managers, configures the router, and establishes middleware.
//
// Parameters:
...
//
// Errors:
//   - Returns error if internal setup fails (though currently returns nil error).
//
// Side Effects:
//   - Registers handlers for standard MCP methods (prompts/list, tools/call, etc.).
//   - Adds middleware for DLP, routing, and filtering.
//   - Starts listening for resource list changes.
func NewServer(
	_ context.Context,
	toolManager tool.ManagerInterface,
	promptManager prompt.ManagerInterface,
...
//
// Summary: Retrieves the list of available prompts from the PromptManager, converts them to the MCP format, and returns them to the client.
//
// Parameters:
...
//
// Summary: Exposes sampling capability to the server by requesting message creation from the client.
//
// Parameters:
//   - ctx (context.Context): The context for the request.
...
//
// Errors:
//   - Returns "no active session found in context" if the request is not associated with a session.
...
//
// Summary: Retrieves a specific prompt by name, verifies access, and executes it with provided arguments.
//
// Parameters:
//   - ctx (context.Context): The context for the request.
...
//   - "access denied to prompt": If the active profile does not allow access to the prompt's service.
//
// Side Effects:
//   - Logs a warning if access is denied.
...
//
// Summary: Fetches the list of available resources from the ResourceManager.
//
// Parameters:
//   - ctx (context.Context): The context for the request.
//   - req (*mcp.ListResourcesRequest): The request object.
//
// Returns:
...
//
// Summary: Retrieves a specific resource by its URI and returns its content.
//
// Parameters:
//   - ctx (context.Context): The context for the request.
...
//   - "access denied to resource": If the active profile does not allow access to the resource's service.
//
// Side Effects:
//   - Logs a warning if access is denied.
...
//
// Summary: Accessor for the AuthManager.
//
// Returns:
...
//
// Summary: Accessor for the ToolManager.
//
// Returns:
...
//
// Summary: Accessor for the PromptManager.
//
// Returns:
...
//
// Summary: Accessor for the ResourceManager.
//
// Returns:
...
//
// Summary: Accessor for the ServiceRegistry.
//
// Returns:
...
//
// Summary: Registers service metadata with the tool manager.
//
// Parameters:
...
//
// Side Effects:
//   - Updates the tool manager's internal state.
...
//
// Summary: Looks up a tool definition by name.
//
// Parameters:
...
//
// Summary: Returns all registered tools and increments the tool listing metric.
//
// Returns:
//   - []tool.Tool: A slice of all available tools.
//
// Side Effects:
//   - Increments "tools/list/total" metric.
//   - Logs "Listing tools...".
...
//
// Summary: Executes a tool, handling access control, logging, metrics, and error handling.
//
// Parameters:
...
//
// Errors:
//   - "access denied to tool": If the active profile does not allow access.
//   - Returns error from `toolManager.ExecuteTool` if execution fails.
//
// Side Effects:
//   - Increments metrics for tool calls and errors.
//   - Measures latency of tool calls.
//   - Logs start and completion of tool execution.
...
//
// Summary: Injects the MCP server reference into the tool manager.
//
// Parameters:
...
//
// Summary: Adds a tool instance to the server's tool manager.
//
// Parameters:
...
//
// Summary: Looks up service metadata.
//
// Parameters:
//   - serviceID (string): The unique identifier of the service.
//
...
//
// Summary: Removes tools belonging to a given service from the tool manager.
//
// Parameters:
//   - serviceKey (string): The identifier of the service.
//
// Returns:
//   None.
//
// Side Effects:
//   - Modifies the tool manager's internal state (deletes tools).
...
//
// Summary: Triggers the registered reload callback function.
//
// Parameters:
...
//
// Errors:
//   - Returns error from the registered `reloadFunc`.
//
// Side Effects:
//   - Executes the reload function, which may have wide-ranging side effects (reloading config, restarting services, etc.).
