...
func TestHealthCheckIntervalCaching(t *testing.T) {
	// 1. Start a dummy server that we can control
	// We use a flag to control response
	isUp := true
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		if isUp {
			w.WriteHeader(http.StatusOK)
	...
	// 2. Create config with Interval
	upstreamConfig := configv1.UpstreamServiceConfig_builder{
		Name: lo.ToPtr("test-interval-service"),
		HttpService: configv1.HttpUpstreamService_builder{
			Address: lo.ToPtr(server.Listener.Addr().String()),
			HealthCheck: configv1.HttpHealthCheck_builder{
				Url:          lo.ToPtr(server.URL),
				Interval:     durationpb.New(cacheDuration),
				ExpectedCode: lo.ToPtr(int32(http.StatusOK)),
			}.Build(),
		}.Build(),
	}.Build()
	...
	// 3. First check: Server UP -> Status UP
	res := checker.Check(ctx)
	assert.Equal(t, health.StatusUp, res.Status, "Initial check should be UP")

	// 4. Set Server DOWN
	isUp = false

	// 5. Check immediately (should be cached UP)
	res = checker.Check(ctx)
	assert.Equal(t, health.StatusUp, res.Status, "Cached check should be UP even if server is DOWN")

	// 6. Wait for cache to expire
	time.Sleep(cacheDuration + 50*time.Millisecond)

	// 7. Check again (should be fresh check -> DOWN)
	res = checker.Check(ctx)
	assert.Equal(t, health.StatusDown, res.Status, "Expired cache check should be DOWN")
}
