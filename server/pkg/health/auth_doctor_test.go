...
func TestCheckAuth(t *testing.T) {
	// Helper to clear env vars to ensure a clean state for each test
	...
	t.Run("API Keys Present", func(t *testing.T) {
		clearEnv(t)
		t.Setenv("ANTHROPIC_API_KEY", "sk-ant-1234567890")
		results := CheckAuth()

		assert.Equal(t, "ok", results["ANTHROPIC_API_KEY"].Status)
		assert.Equal(t, "Present (...7890)", results["ANTHROPIC_API_KEY"].Message)
	})
