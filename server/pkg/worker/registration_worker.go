...
// Summary: ServiceRegistrationWorker is a background worker responsible for handling service registration requests. It listens for ServiceRegistrationRequest messages on the event bus, processes them using the service registry, and publishes the results as ServiceRegistrationResult messages. Represents a ServiceRegistrationWorker.
//
// Parameters:
//   - None.
//
...
//
// Returns:
//   - *ServiceRegistrationWorker: The resulting *ServiceRegistrationWorker.
//
// Errors:
//   - None.
//
...
// Summary: Start launches the worker in a new goroutine. It subscribes to service registration requests on the event bus and will continue to process them until the provided context is canceled.
//
// Parameters:
...
func (w *ServiceRegistrationWorker) Start(ctx context.Context) {
	...
	unsubscribe := requestBus.Subscribe(ctx, "request", func(req *bus.ServiceRegistrationRequest) {
		...
		go func() {
			...
			if err != nil {
				if errors.Is(err, serviceregistry.ErrServiceAlreadyRegistered) {
					log.Info("Service already registered, skipping retry", "service", req.Config.GetName())
