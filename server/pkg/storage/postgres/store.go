...
func (s *Store) Load(ctx context.Context) (*configv1.McpAnyServerConfig, error) {
	...
	// Previously, these queries were sequential, adding latency to startup/reload.
	...
	g.Go(func() error {
		...
		if err := settingsRow.Scan(&settingsJSON); err == nil {
			...
			if err := opts.Unmarshal(settingsJSON, &s); err == nil {
				...
				return fmt.Errorf("failed to unmarshal global settings: %w", err)
			}
		} else if err != sql.ErrNoRows {
			// If error is not ErrNoRows, return it. If it is ErrNoRows, settings remains nil (valid).
	...
	g.Go(func() error {
		...
		for collectionRows.Next() {
			...
			if err := collectionRows.Scan(&configJSON); err != nil {
				// Continue or fail? Original code continued on scan error?
				// Original: if err := collectionRows.Scan(&configJSON); err != nil { continue }
				// Let's stick to original behavior but logging would be nice.
				// However, if we change behavior to fail, it's safer.
				// Let's match original behavior (continue).
				continue
			}
