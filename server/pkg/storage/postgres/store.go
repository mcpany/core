...
//
// Summary: PostgreSQL-backed storage implementation.
type Store struct {
	db *DB
}
...
//   - error: Returns an error if loading fails.
...
//
// Returns:
//   - error: Returns an error if the user is not found or update fails.
...
//
// Returns:
//   - *configv1.UserToken: The token if found, or nil.
//   - error: Returns an error if the operation fails.
