...
func TestRedisBus_Integration_Subscribe(t *testing.T) {
	if os.Getenv("CI") == "true" {
		t.Skip("Skipping TestRedisBus_Integration_Subscribe in CI due to Docker dependency")
	}
...
func TestRedisBus_Integration_SubscribeOnce(t *testing.T) {
	if os.Getenv("CI") == "true" {
		t.Skip("Skipping TestRedisBus_Integration_SubscribeOnce in CI due to Docker dependency")
	}
	redisAddr, cleanup := StartRedisContainer(t)
	defer cleanup()
...
func TestBusProvider_Integration_Redis(t *testing.T) {
	if os.Getenv("CI") == "true" {
		t.Skip("Skipping TestBusProvider_Integration_Redis in CI due to Docker dependency")
	}
	redisAddr, cleanup := StartRedisContainer(t)
	defer cleanup()
...
func TestRedisBus_Integration_Unsubscribe(t *testing.T) {
	if os.Getenv("CI") == "true" {
		t.Skip("Skipping TestRedisBus_Integration_Unsubscribe in CI due to Docker dependency")
	}
	redisAddr, cleanup := StartRedisContainer(t)
	defer cleanup()
...
func TestRedisBus_Integration_Concurrent(t *testing.T) {
	if os.Getenv("CI") == "true" {
		t.Skip("Skipping TestRedisBus_Integration_Concurrent in CI due to Docker dependency")
	}
	redisAddr, cleanup := StartRedisContainer(t)
	defer cleanup()
