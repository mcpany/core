...
func TestK8sFullStack(t *testing.T) {
	t.Run("HelmDryRunDefault", func(t *testing.T) {
		// Mock implementation or skip if environment not ready
		t.Skip("Skipping Helm dry-run test as it requires a reachable Kubernetes cluster")
	})
	t.Run("HelmDryRunWithOperator", func(t *testing.T) {
		t.Skip("Skipping Helm dry-run test with operator as it requires a reachable Kubernetes cluster")
	})
}
