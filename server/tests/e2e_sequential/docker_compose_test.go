...
func getDockerCmd(t *testing.T) (string, []string) {
...
func TestDockerComposeE2E(t *testing.T) {
	...
	dockerCmd, dockerArgs := getDockerCmd(t)
	runCommand(t, rootDir, dockerCmd, append(dockerArgs, "build", "-t", imageName, "-f", "server/docker/Dockerfile.server", ".")...)
	...
	cleanup := func() {
		...
		if currentComposeFile != "" {
			...
			cmd := exec.Command(dockerCmd, append(dockerArgs, "compose", "-f", currentComposeFile, "logs")...)
		...
		cmd := exec.Command(dockerCmd, append(dockerArgs, "logs", "mcpany-weather-test")...) // Name without random suffix? No, we used random.
		...
		if currentComposeFile != "" {
			cmd = exec.Command(dockerCmd, append(dockerArgs, "compose", "-f", currentComposeFile, "down", "-v", "--remove-orphans")...)
			cmd.Env = os.Environ()
			cmd.Stdout = os.Stdout
			cmd.Stderr = os.Stderr
			_ = cmd.Run()
		...
		cmd = exec.Command(dockerCmd, append(dockerArgs, "compose", "down", "-v", "--remove-orphans")...)
		cmd.Env = os.Environ()
		_ = cmd.Run()
	...
	getServicePort := func(composeFile, service, internalPort string) string {
		dockerCmd, dockerArgs := getDockerCmd(t)
		cmd := exec.Command(dockerCmd, append(dockerArgs, "compose", "-f", composeFile, "port", service, internalPort)...)
		cmd.Env = os.Environ()
		out, err := cmd.Output()
	...
	if _, err := os.Stat(rootCompose); err == nil {
		...
		dockerCmd, dockerArgs := getDockerCmd(t)
		runCommand(t, rootDir, dockerCmd, append(dockerArgs, "compose", "-f", dynamicCompose, "up", "-d", "--wait")...)
		...
		dockerCmd, dockerArgs = getDockerCmd(t)
		runCommand(t, rootDir, dockerCmd, append(dockerArgs, "compose", "-f", dynamicCompose, "down")...)
	} else {
...
func testFunctionalWeather(t *testing.T, rootDir string) {
	...
	dockerCmd, dockerArgs := getDockerCmd(t)
	args := append(dockerArgs, "run", "-d", "--name", containerName,
	...
	defer func() {
		dockerCmd, dockerArgs := getDockerCmd(t)
		_ = exec.Command(dockerCmd, append(dockerArgs, "rm", "-f", containerName)...).Run()
	}()

	// Get assigned port
	dockerCmd, dockerArgs = getDockerCmd(t)
	out, err := exec.Command(dockerCmd, append(dockerArgs, "port", containerName, "50050/tcp")...).Output()
