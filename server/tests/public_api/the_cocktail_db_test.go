...
func TestUpstreamService_TheCocktailDB(t *testing.T) {
	...
	if _, ok := theCocktailDBResponse["drinks"].(string); ok {
		// drinks can be null or string "null" sometimes in some APIs, but TheCocktailDB usually returns null if not found
	}
	if theCocktailDBResponse["drinks"] == nil {
		// Handle empty response gracefully if needed
	}
