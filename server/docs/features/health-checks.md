...
Health checks are performed periodically in the background (globally every 30s by default) to update the service status. The status is logged and can be monitored via the server API or metrics.
