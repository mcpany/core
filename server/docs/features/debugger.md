...
The Agent Debugger is a middleware that monitors and records HTTP traffic for
inspection. It captures detailed information about requests and responses,
including headers and bodies, allowing developers to debug agent interactions
and "replay" tool calls.
...
- **Truncation**: Automatically truncates large bodies (default 10KB) to prevent
  memory issues.
...
The debugger exposes an endpoint (typically `/debug/entries`) to retrieve the
recorded traffic logs.
