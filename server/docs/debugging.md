...
MCP Any provides a debug mode that can be used to inspect the full request and
response of each tool call. This is useful for troubleshooting your
configurations and understanding the flow of data through the server.

## Enabling Debug Mode

You can enable debug mode by passing the `--debug` flag when starting the
server:
...
When debug mode is enabled, the server will log the full JSON-RPC request and
response for each tool call. The logs will be printed to standard output or to
the file specified by the `--logfile` flag.

## Example Log Output

Here is an example of the log output when debug mode is enabled (using
structured logging):
...
By inspecting the log output, you can identify any issues with your
configuration and ensure that the server is behaving as expected.

## System Health & Diagnostics (Doctor)

For troubleshooting system startup, configuration issues, and upstream
connectivity, MCP Any includes a built-in `doctor` command.
