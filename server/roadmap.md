...
- [x] **System Health Dashboard**: Added a new "System Health" widget to the main dashboard that visualizes uptime, active connections, version info, and security warnings (e.g. unconfigured API keys), providing real-time visibility into server state. (Experience Crafter)
...
- [x] **User-Friendly Config Validation**: Implemented explicit check for configuration file existence with helpful error messages and tips. Added smart suggestions (Did you mean...?) for common configuration field aliases (e.g., `url` -> `address`) and improved "unknown field" error reporting. (Friction Fighter)
...
| 49   | **Secure Defaults Enforcer**                  | **Security**: A mode that warns or errors if insecure defaults (like `0.0.0.0` binding without auth) are detected, guiding users to secure configurations.                                                        | Medium     |
| 50   | **Connection Draining**                       | **Ops**: Gracefully handle server shutdown by waiting for active connections (tracked by the new metrics) to close before terminating the process.                                                                 | High       |
...
| 56   | **Tool Usage Analytics**                      | **Observability**: Track and visualize invocation counts, success rates, and latency per tool in the dashboard.                                                                                                    | Medium     |
