...
- [x] **Configuration Migration Tool**: Added `mcpany config migrate` command to convert `claude_desktop_config.json` to `mcpany` config format. (Friction Fighter)
...
| 19 | **Interactive Config Builder** | **UX**: A TUI or Web-based wizard that asks users what services they want and generates the config for them. | Medium |
| 20 | **Secret Rotation Manager** | **Security**: Automate rotation of secrets for upstream services, integrating with Vault/AWS. | High |
