...
- [x] **Startup Connectivity Doctor**: Runs diagnostics on startup to verify connectivity to all configured upstream services. Prints clear warnings to stderr if services are unreachable, preventing "silent failures" where a server starts but tools are broken. (Friction Fighter)
...
| 49 | **Interactive Doctor Mode** | **DevX**: `mcpany doctor --interactive` that allows users to interactively fix issues found during the check (e.g. prompt for missing API keys). | Medium |
| 50 | **Service Dependency Check** | **Reliability**: Validate that services depending on other services (if we add such feature) are ordered correctly or warn about cycles. | High |
