...
| 49   | **Service Retry Logic**                       | **Resilience**: Add configurable retry logic for service registration connectivity checks to handle transient network issues during startup.                                                                        | Medium     |
| 50   | **Doctor JSON Output**                        | **DevX**: Add `--json` flag to `mcpany doctor` for machine-readable output.                                                                                                                                        | Low        |
