...
- [x] **Startup Connectivity Check**: Validate connectivity to upstream services (HTTP, gRPC, etc.) during server startup and configuration reload to prevent silent failures. (Friction Fighter)
...
| 41 | **Parallel Connectivity Checks** | **Performance**: Refactor the startup connectivity checks to run in parallel to reduce startup time when many services are configured. | Low |
| 42 | **Detailed Diagnostic Report** | **Observability**: Generate a structured report (JSON/HTML) of connectivity status for all services, accessible via CLI or UI, instead of just logging errors. | Medium |
