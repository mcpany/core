...
- [x] **Frontend Config Status Banner**: Implemented a global alert banner in the UI that monitors the `/doctor` endpoint and warns users if the server configuration reload failed, preventing "silent failures" where users think the server is up-to-date but is actually stale. (Experience Crafter)
...
| 32 | **Configuration Diffing API** | **Observability**: An API endpoint to compare the currently active configuration with the previous version or the file on disk, helping users understand what changed during a reload. | Medium |
| 33 | **Config Version History** | **Ops**: Maintain a history of loaded configurations in the database (or git integration) to allow for one-click rollbacks of bad config changes directly from the UI. | High |
| 34 | **Interactive Troubleshooting Wizard** | **UX**: A guided "Help Me Fix It" flow in the UI that activates when errors are detected (like config failures), suggesting specific steps based on the error message. | Medium |
