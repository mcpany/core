...
- **Auto-Discovery Status API**
  - **Description**: Expose the status of auto-discovery providers (Last run, Error, Success) via API to the UI, so users know why local tools (like Ollama) are missing.
...
| 78 | **Discovery Provider Diagnostics** | **Observability**: UI features to manually trigger auto-discovery and view detailed logs/errors for each provider to debug connection issues. | Medium |
| 79 | **Service Grouping** | **UX**: Organize services in the dashboard by tags or source (config vs discovery) for better manageability in large deployments. | Low |
