...
*   **[P0] Neural Shard Attestation (NSA) Broker**: Authoritative model validation service facilitating hardware-attested proofs of model weight integrity for local agents. (Added: 2026-07-25)
*   **[P0] Kernel-Mediated Budget Pinning (KMBP)**: Advanced resource governance layer utilizing OS-level primitives (cgroups/sandbox-exec) to enforce hardware-attested budgets. (Added: 2026-07-25)
*   **[P0] Continuous State Re-validation (CSR)**: Security middleware for shared memory shards that re-validates fragments upon mission-state transitions to detect dormant "Ghost Fragments." (Added: 2026-07-25)
*   **[P0] Declarative GC-Immunity Middleware**: Policy engine allowing users to define declarative rules for pinning reasoning anchors against aggressive context-window pruning. (Added: 2026-07-25)
