...
- **Windows Path Handling Fix**: ✅ Implemented correct `file://` URI normalization for Windows in `server/pkg/upstream/filesystem`.
...
| 70   | **Path Validation Diagnostics**               | **DevX**: Expose a CLI command to test path validation logic against specific inputs to debug configuration issues.                                                                                                | Low        |
| 71   | **Filesystem Provider Windows Tests**         | **Technical Debt**: Add dedicated CI jobs running on Windows to ensure filesystem provider compatibility.                                                                                                          | High       |
