...
- [x] **Strict Syntax Validation**: Server now fails fast if configuration files are malformed (YAML/JSON syntax errors), preventing silent startup failures where services would just be missing. (Friction Fighter)
...
| 24 | **Interactive Diagnostic Tool** | **Support**: A "Diagnose" button in the UI for failed tools that runs a battery of checks (connectivity, DNS, Auth) and suggests fixes in plain English. | Medium |
| 25 | **Environment Variable Generator** | **DevX**: CLI command to scan configuration files and generate a `.env.example` template with all required variables and their default values/descriptions. | Low |
