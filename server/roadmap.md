...
- **Secret Validation Pre-flight**
    - **Description**: Validate all secrets (files, env vars, remote) before starting the server to ensure all credentials are accessible. This prevents silent failures where the server starts but tools fail later due to missing credentials.
    - **Status**: Completed.
...
| :--- | :-------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------- |
| 1    | **Duplicate Tool Detection**                  | **Safety**: Detect if two services expose tools with the same name (before sanitization) and warn about potential conflicts or shadowing.                                                                          | Low        |
| 2    | **Environment Variable Wizard**               | **DevX**: A UI helper to identify used environment variables in a config and prompt the user to fill them if missing during startup/testing.                                                                       | Low        |
