...
sequences with zero host access by default. (Added: 2026-03-09 - Promoted to P0
on 2026-03-10)
...
with OpenClaw's pluggable context management. (Added: 2026-03-13 - Promoted to
P0 on 2026-03-14)
...
and normalizing conversation-feedback for RL-driven agents (e.g., OpenClaw-RL).
(Added: 2026-03-19)
...
that use `$ref` to remote URLs by configuring a custom schema loader with HTTP
support. | Medium |
...
startup and suggests using `--json-rpc-port` or `--grpc-port` flags to resolve
the conflict.
- **Whitespace URL Validation**
  - **Description**: Detects and warns about hidden whitespace in URL
configurations (HTTP, WebSocket, OpenAPI, etc.) which often occurs when copying
from external sources, providing actionable fixes.
- **gRPC Health Checks**
...
with TPM/Secure Enclave for zero-latency mission validation. (Added: 2026-03-29)
...
Validator to provide signed environment manifests for deterministic agent boot.
(Added: 2026-04-10)
...
local transport channels to hardware-attested Reasoning Session Tokens. (Added:
2026-05-17)
...
Provider adding monotonic timestamps to reasoning fragments. (Added: 2026-05-21)
...
service for tracking capability lineage and enforcing immediate revocation upon
sub-intent termination.
...
anchoring stylometric profiles to multi-modal trace history (Added: 2026-06-17).
