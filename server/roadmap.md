...
- [x] **Startup Service Diagnostics**: Server now waits for initial service registration on startup and prints a status summary table to stderr, preventing silent failures. (Friction Fighter)
...
| 31 | **Parallel Service Registration** | **Performance**: Optimize startup by running service registrations in parallel (already async, but can be further optimized) while maintaining the result wait logic to reduce total startup time. | Medium |
...
| 41 | **Interactive Startup Troubleshooting** | **UX**: If startup fails, provide an interactive CLI prompt to view logs or retry specific services without restarting the whole process. | High |
