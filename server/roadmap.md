...
- **Startup Doctor Checks**
  - **Description**: Automatically validates upstream service connectivity on startup (in background) and provides actionable hints for errors (e.g. DNS, Auth, Timeout).
...
| 80 | **Service Health History** | **Observability**: Persist doctor check results over time (sqlite) to show reliability trends for upstream services in the dashboard. | Medium |
| 81 | **Doctor Auto-Fix Integration** | **DevX**: Add a `--fix` flag to `mcpany doctor` to automatically resolve simple issues like file permissions or missing directories. | Medium |
