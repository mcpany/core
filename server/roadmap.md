...
### Completed Features
- **Command Line Env Validation**: Added strict validation for `env` variables in `command_line_service` to prevent silent failures on startup.
...
| 2    | **Interactive OAuth Handler**                 | **UX/Auth**: Solve the "copy-paste token" friction. Allow users to click "Login" in the UI to authenticate tools like GitHub/Google.                                                                               | High       |
...
| 33   | **Startup Diagnostics & Doctor UI**           | **UX**: [COMPLETED] Enhanced CLI diagnostics (`mcpany doctor`) with visual feedback. Next: Add frontend banner for config status.                                                                                  | Low        |
...
| 70   | **Strict Env Var Validation for All Services**| **Security**: Extend the `validateSecretMap` check to all service types that support env vars (e.g., HTTP headers, connection params) to ensure fail-fast behavior.                                                | Medium     |
| 71   | **Secrets Management UI**                     | **UX**: A UI to manage/view defined secrets and their status (set/unset), aiding in debugging configuration issues.                                                                                                | High       |
