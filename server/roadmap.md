...
- [x] **Validation CLI Command**: **DevX**: Enhance `mcpany config validate` to run deep checks, including connecting to upstream services (dry-run) to verify connectivity and auth. (Friction Fighter)
...
| 23 | **Service Reachability Probing** | **Observability**: Automatically probe upstream services periodically and report reachability metrics to Prometheus/UI. | Medium |
...
| 41 | **Interactive Config Debugger** | **DevX**: A TUI (Terminal UI) mode for `mcpany config validate` that allows interactive fixing of validation errors. | High |
