...
- [x] **Interactive Config Generator**: `mcpany init` wizard that asks questions and generates a valid `config.yaml` with best practices (secure defaults, comments). (Friction Fighter)
...
| 43 | **Web-Based Initial Setup Wizard** | **UX**: A web-based setup wizard that launches on first run if no config is found, guiding users through configuration visually. | High |
| 44 | **Tool Usage Analytics Dashboard** | **Observability**: A dedicated dashboard view showing tool usage frequency, success rates, and token consumption per tool. | Medium |
