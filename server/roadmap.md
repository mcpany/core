...
- [x] **Upstream Service Connectivity Check**: Added a "Test Connection" button in the UI and a backend HealthCheck API to allow users to verify upstream service connectivity (HTTP, MCP) on demand. (Experience Crafter)
...
| 41 | **Auto-Retry on Health Check Failure** | **Resilience**: If a manual health check fails, optionally trigger an automatic re-registration attempt in the background. | Medium |
| 42 | **Detailed Connectivity Report** | **Observability**: Expand the "Test Connection" feature to return detailed diagnostics (latency, SSL certificate expiry, supported methods) beyond just success/failure. | Medium |
