...
| 49   | **Config Template Wizard**                    | **DevX**: An interactive CLI wizard (`mcpany config wizard`) that asks user questions (e.g. "Do you want to add a Postgres service?") and generates a valid `config.yaml` from templates.                          | Medium     |
| 50   | **Config Validation Webhook**                 | **Ops**: A webhook endpoint that CI/CD pipelines can POST a candidate config to for validation before deployment.                                                                                                  | Low        |
