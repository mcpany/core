...
| 49   | **Validation Error Code Support**             | **DevX**: Return structured error codes (e.g., `ERR_CONFIG_INVALID_PATH`) alongside text messages to allow programmatic handling of configuration errors.                                                          | Medium     |
| 50   | **Validator Test Refactoring**                | **Maintainability**: Split `validator.go` tests into separate files by service type (e.g. `validator_mcp_test.go`, `validator_http_test.go`) to manage growing complexity.                                         | Low        |
