...
- **Config Schema Validation with Line Numbers**
  - **Description**: Extended line number reporting to schema validation errors and protojson type mismatches by mapping errors back to YAML AST nodes.
...
| 80   | **Config Schema Visualization CLI**           | **DevX**: A command `mcpany config tree` that prints the configuration structure with resolved values and source file locations (line numbers) for debugging inheritance/overrides.                              | Low        |
| 81   | **LSP Server for Config**                     | **DevX**: Implement a Language Server Protocol (LSP) server for `config.yaml` that provides autocomplete, validation (with my new line numbers), and hover docs based on the proto schema.                       | High       |
