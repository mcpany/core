...
def check_file(filepath):
    ...
    for i, line in enumerate(lines):
        line = line.rstrip()
        symbol = None; kind = None; params_str = ""; returns_str = ""
        ...
        if m_func: symbol = m_func.group(1); kind = "func"; params_str = m_func.group(2); returns_str = m_func.group(3)
        elif m_method: symbol = m_method.group(1); kind = "method"; params_str = m_method.group(2); returns_str = m_method.group(3)
        ...
        if symbol:
            ...
            while j >= 0:
                prev = lines[j].strip()
                if prev.startswith('//'):
                    if not prev.startswith('//go:') and not prev.startswith('// +build'): doc_lines.insert(0, prev)
                elif prev == '': break
                else: break
                j -= 1
            reasons = []
