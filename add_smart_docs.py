...
def split_camel_case(s):
    # e.g. ListServices -> List Services
    ...
    while i < len(lines):
        ...
        if symbol:
            ...
            if not is_compliant:
                ...
                # Check for "Empty Calories" check from prompt: "Sets the ID." is bad.
                # Must be meaningful.
                if not old_summary or len(old_summary.split()) <= 3 or "Sets the" in old_summary or old_summary.startswith(f"{symbol} represents"):
                    old_summary = generate_summary(symbol, kind)

                if not old_summary.endswith('.'):
                    old_summary += '.'

                # Ensure it starts with the symbol name for go doc compliance
