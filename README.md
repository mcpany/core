...
We enforce the use of `bazelisk` and `make` for all common development tasks.
...
- **Environment Variables:** Set any secret values in environment variables (e.g., `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`) and reference them dynamically in your config files to ensure security boundaries are maintained. Do not hardcode secrets in your configuration files.
