...
**Note:** By default, the server prioritizes database configuration. To load configuration from files (and enable read-only mode for those settings), you can simply provide the `--config-path` flag. If you need to force file configuration even without the flag (e.g. relying on default paths), you can set the `MCPANY_ENABLE_FILE_CONFIG` environment variable to `true`.
