...
func TestConvertMcpOperationsToTools_WithExamplesAndDefaults(t *testing.T) {
	spec := `
{
  "openapi": "3.0.0",
  "info": { "title": "Test", "version": "1.0" },
  "paths": {
	...
	paramWithDefault := props["param_with_default"].GetStructValue().GetFields()
	assert.Equal(t, "defaultParam", paramWithDefault["default"].GetStringValue())

	// Check property with example
	propWithExample := props["prop_with_example"].GetStructValue().GetFields()
	assert.Equal(t, "exampleValue", propWithExample["example"].GetStringValue())

	// Check property with default
	propWithDefault := props["prop_with_default"].GetStructValue().GetFields()
	assert.Equal(t, float64(123), propWithDefault["default"].GetNumberValue()) // JSON numbers are float64
}
