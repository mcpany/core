...
func startGrpcServer(ctx context.Context, wg *sync.WaitGroup, errChan chan<- error, name, port string, register func(*gogrpc.Server), shutdownTimeout time.Duration) {
	...
	go func() {
		...
		go func() {
			...
			// Use a channel to signal when GracefulStop has completed.
			...
			// Wait for either the shutdown to complete or the timeout to be reached.
			select {
			case <-shutdownCtx.Done():
				// If the timeout is reached, force the server to stop.
				serverLog.Warn("Graceful shutdown timed out, forcing stop.")
				grpcServer.Stop()
			case <-stopped:
				// If the shutdown completes successfully, log it.
