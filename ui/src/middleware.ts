...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/v1/ws/')) {
    // Skip middleware rewrite for WebSockets; handled by next.config.ts rewrites
  } else if (pathname.startsWith('/api/v1') || pathname.startsWith('/mcpany.api.v1.') || pathname.startsWith('/doctor') || pathname.startsWith('/v1/') || pathname.startsWith('/auth/oauth/') || pathname === '/auth/login' || pathname.startsWith('/debug/') || pathname.startsWith('/sse') || pathname.startsWith('/messages') || pathname.startsWith('/mcp/')) {
