...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/v1') || pathname.startsWith('/mcpany.api.v1.') || pathname.startsWith('/doctor') || pathname.startsWith('/v1/') || pathname.startsWith('/auth/oauth/') || pathname === '/auth/login' || pathname.startsWith('/debug/') || pathname.startsWith('/sse') || pathname.startsWith('/messages') || pathname.startsWith('/mcp/')) {
    ...
    // Use 127.0.0.1 by default to avoid IPv6 (::1) issues with Node/Docker on localhost
    const backendUrl = process.env.BACKEND_URL || 'http://127.0.0.1:50050';
  ...
  const csp = [
    ...
    "connect-src 'self' https://cdn.jsdelivr.net https://raw.githubusercontent.com", // Added cdn.jsdelivr.net and raw.githubusercontent.com
