...
function SchemaField({
  ...
  if (schema.oneOf || schema.anyOf) {
    ...
    return (
      <div
        className={cn(
          "border rounded-md p-3 bg-muted/10",
  ...
  if (type === "string") {
    ...
    const isMultiline =
      ...
      (label && /description|content/i.test(label));
  ...
  if (type === "object") {
    ...
    const renderDynamicProperties = () => {
      ...
      const dynamicKeys = Object.keys(objectValue).filter(
        (k) => !(k in properties),
      );
      ...
      return (
        <div className="space-y-2 mt-4">
          {dynamicKeys.length > 0 && (
            <div className="space-y-2 border-t pt-4 border-dashed">
              ...
              {dynamicKeys.map((key) => (
                <div
                  key={key}
                  className="flex gap-2 items-start group relative pl-2"
                >
                  ...
                  <div className="w-1/3 min-w-[120px]">
                    <Input
                      value={key}
                      onChange={(e) =>
                        handleDynamicKeyChange(key, e.target.value)
                      }
          ...
          <div
            className={dynamicKeys.length === 0 && !hasProperties ? "" : "pt-2"}
          >
            <Button
              type="button"
              variant="outline"
