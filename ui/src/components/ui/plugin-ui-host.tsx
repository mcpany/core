...
export function PluginUIHost({
  ...
  const handleError = () => {
    setLoading(false);
    setError(
      "Failed to load plugin UI. This could be due to a connection issue or security policy.",
    );
  };
  ...
  return (
    <div
      className={cn(
        "relative w-full h-full min-h-[400px] border rounded-lg overflow-hidden bg-background",
        className,
      )}
    >
      {loading && (
