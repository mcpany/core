...
const getTableData = (data: unknown, smartTable: boolean) => {
  ...
  if (Array.isArray(content) && content.length > 0) {
    const isListOfObjects = content.every(
      (item) =>
...
export function JsonView({
  ...
  const isImage =
    typeof parsedData === "string" && parsedData.startsWith("data:image/");
  ...
  const renderRaw = () => (
    <div
      className={cn("relative group/code rounded-md bg-[#1e1e1e]", className)}
    >
      <div
        className={cn(
      ...
      >
        <Suspense
          fallback={
            <pre className="p-4 text-xs text-muted-foreground">Loading…</pre>
          }
        >
          <SyntaxHighlighter
            language="json"
    ...
    return (
      <div className={cn("rounded-md border bg-[#1e1e1e]", className)}>
        <div
          ...
          style={{
            maxHeight:
              showCollapse && !isExpanded ? `${maxHeight}px` : undefined,
          }}
        >
          <JsonTree
            data={parsedData}
  ...
  const renderImage = () => {
    return (
      <div
        className={cn(
          "rounded-md border bg-black/50 p-4 flex justify-center",
          className,
        )}
      >
        <img
  ...
  const renderSmart = () => {
    ...
    return (
      <div
        className={cn("rounded-md border overflow-hidden bg-card", className)}
      >
        <div
          className={cn(
            "overflow-auto",
            ...
            <TableHeader className="bg-muted/50 sticky top-0 z-10">
              <TableRow>
                {columns.map((col) => (
                  <TableHead
                    key={col}
                    className="whitespace-nowrap font-medium text-xs px-2 py-1 h-8 bg-muted/50"
                  >
                    {col}
                  </TableHead>
                ))}
                ...
                <TableRow key={idx} className="hover:bg-muted/50">
                  {columns.map((col) => {
                    ...
                    return (
                      <TableCell
                        key={col}
                        className="px-2 py-1 text-xs max-w-[200px] truncate"
