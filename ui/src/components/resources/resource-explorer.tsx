...
const MemoizedResourceListItem = React.memo(
  ({
    res,
    isSelected,
    onSelect,
...
const MemoizedResourceGridItem = React.memo(
  ({
    res,
    isSelected,
    onSelect,
          ...
          >
            <CardContent className="p-3 flex flex-col items-center text-center gap-2">
              ...
              <div className="w-full">
                <div className="font-medium text-xs truncate" title={res.name}>
                  {res.name}
                </div>
                <div className="text-[10px] text-muted-foreground truncate mt-0.5">
                  {res.mimeType || "unknown"}
                </div>
              </div>
            </CardContent>
          </Card>
...
export function ResourceExplorer({
  ...
  const handleDownload = useCallback(
    async (uri: string) => {
      ...
      try {
        toast({
          title: "Downloading...",
          description: "Fetching resource content.",
        });
        const res = await apiClient.readResource(uri);
        if (!res.contents || res.contents.length === 0) {
          toast({
            title: "Error",
            description: "No content found for resource.",
            variant: "destructive",
          });
          return;
        }
  ...
  const handleCopyUri = useCallback(
    (uri: string) => {
      ...
      toast({
        title: "Copied",
        description: "Resource URI copied to clipboard.",
      });
    },
    [toast],
  );
  ...
  const handleCopyName = useCallback(
    (name: string) => {
      ...
      toast({
        title: "Copied",
        description: "Resource name copied to clipboard.",
      });
    },
    [toast],
  );
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent, res: ResourceDefinition) => {
    ...
    >
      ...
      <div className="flex items-center justify-between p-2 px-4 border-b bg-muted/20 h-14 shrink-0">
        <div className="flex items-center gap-2 flex-1 max-w-md">
          ...
          >
            <SearchCode
              className={cn("h-4 w-4", isDeepSearch && "text-primary")}
            />
          </Button>
        </div>
            ...
            <>
              <div className="flex items-center justify-between p-3 border-b bg-muted/5 h-12 shrink-0">
                <div className="flex items-center gap-2 overflow-hidden">
                  ...
                  >
                    {resourceContent?.mimeType || "loading..."}
                  </Badge>
                </div>
                  ...
                  <div className="flex items-center gap-1 mr-2 px-1 bg-muted/50 rounded-md">
                    <Button
                      ...
                      disabled={
                        ...
                        filteredResources[0].uri === selectedUri
                      }
                      title="Previous"
                    >
                    ...
                    <Button
                      ...
                      disabled={
                        filteredResources.length <= 1 ||
                        filteredResources[filteredResources.length - 1].uri ===
                          selectedUri
                      }
                      title="Next"
                    >
      ...
      <ResourcePreviewModal
        ...
        initialContent={
          previewResource?.uri === selectedUri ? resourceContent : undefined
        }
      />
    </div>
  );
