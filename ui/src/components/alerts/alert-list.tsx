...
export function AlertList() {
  ...
  const handleSelectAll = useCallback(
    (checked: boolean, filteredAlerts: Alert[]) => {
      if (checked) {
  ...
  const isAllSelected =
    filteredAlerts.length > 0 && selected.size === filteredAlerts.length;
              ...
              <TableHead className="w-[30px] pr-0">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleSelectAll(!!checked, filteredAlerts)
                  }
            ...
            {loading && alerts.length === 0 ? (
              ...
              filteredAlerts.map((alert) => (
                <TableRow
                  ...
                  className={cn(
                    "group",
                    selected.has(alert.id) ? "bg-muted/50" : "",
                  )}
                >
