    ...
    return logs.filter((log) => {
      ...
      if (traceId) {
        ...
        if (traceStartTime && traceEndTime) {
          ...
          if (
            logTime >= traceStartTime - 500 &&
            logTime <= traceEndTime + 500
          ) {
            return true;
          }
        }
    ...
    <div className="flex flex-col h-full gap-4">
      ...
      <Card
        className={cn(
          ...
          isEmbedded && "border-0 shadow-none bg-transparent",
        )}
      >
        <CardHeader
          className={cn(
            "p-4 border-b bg-muted/20",
            isEmbedded && "p-2 bg-transparent border-b",
          )}
        >
          <div className="flex flex-col md:flex-row gap-4 justify-between">
            ...
            {isEmbedded && (
              <div className="text-[10px] text-muted-foreground flex items-center justify-end px-2 italic">
                Note: Showing live logs only. Historical logs may not be
                available.
              </div>
            )}
            {!isEmbedded && (
