...
const HighlightText = React.memo(
  ({ text, regex }: { text: string; regex: RegExp | null }) => {
      ...
      <>
        {parts.map((part, i) =>
          i % 2 === 1 ? (
            <mark
              key={i}
              className="bg-yellow-500/40 text-inherit rounded-sm px-0.5 -mx-0.5"
            >
              {part}
            </mark>
          ) : (
...
const LogRow = React.memo(
  ({
    ...
    const isPotentialJson = React.useMemo(
      () => isLikelyJson(log.message),
      [log.message],
    );
      ...
      >
        <div className="flex flex-row w-full items-start gap-1 sm:gap-3">
          <div className="flex items-center gap-2 sm:contents">
            ...
            <span
              className={cn(
                "font-bold w-12 text-[10px] sm:text-xs sm:mt-0.5",
            ...
            {log.source && (
              <span
                ...
                style={
                  {
                    "--source-hue": getSourceHue(log.source),
                  } as React.CSSProperties
                }
                title={log.source}
              >
