...
export function SecretsManager() {
    ...
    const loadSecrets = async () => {
        ...
        try {
            ...
            setSecrets(data);
            setSelectedSecrets(new Set()); // Reset selection on reload
        } catch (error) {
    ...
    const handleBulkDelete = async () => {
        if (!confirm(`Are you sure you want to delete ${selectedSecrets.size} secrets?`)) return;

        try {
            // Process deletions sequentially to avoid SQLite locking issues
            ...
            toast({
                title: "Success",
                description: `${selectedSecrets.size} secrets deleted successfully.`,
            });
            loadSecrets();
        } catch (e) {
            ...
            loadSecrets(); // Refresh to see what's left
        }
    };
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    {selectedSecrets.size > 0 && (
                        <Button variant="destructive" onClick={handleBulkDelete} className="animate-in fade-in slide-in-from-right-4 duration-300">
    ...
    return (
        <TableRow className="group">
            <TableCell>
                <Checkbox
                    checked={isSelected}
                    onCheckedChange={(c) => onSelect(secret.id, !!c)}
            ...
            <TableCell>
                 <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <Badge variant="outline" className="text-[10px] h-4 font-mono px-1 py-0">
                            {secret.provider}
                        </Badge>
                    </div>
                 </div>
