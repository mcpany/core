...
const settingsSchema = z.object({
  ...
  alerts_webhook_url: z.string().optional(),
});
      ...
      try {
        ...
        if (settings) {
          ...
          form.reset({
            ...
            merge_strategy_upstream:
              (settings.merge_strategy?.upstream_service_list as
                | "extend"
                | "replace") || "extend",
            merge_strategy_profile:
              (settings.merge_strategy?.profile_list as "extend" | "replace") ||
              "extend",
            alerts_enabled: settings.alerts?.enabled || false,
            alerts_webhook_url: settings.alerts?.webhook_url || "",
          });
    ...
    try {
      ...
      const payload = {
        ...
        alerts: {
          ...
          webhook_url: data.alerts_webhook_url,
        },
      };
      ...
      <CardContent className="space-y-6">
        {isReadOnly && (
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-500 p-4 text-yellow-700 dark:text-yellow-400">
            ...
            <p className="text-sm">
              Additional configuration can be found in the file configuration.
              This configuration cannot be edited via the UI.
            </p>
          </div>
        )}
        <Form {...form}>
                ...
                <div className="flex flex-col space-y-4 rounded-lg border p-4">
                  <div className="flex flex-row items-center justify-between">
                    <div className="space-y-0.5">
                      ...
                      <FormDescription>
                        Enable health status change alerts.
                      </FormDescription>
                    </div>
                    <FormField
                      control={form.control}
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            ...
                            {...field}
                            disabled={
                              !form.watch("alerts_enabled") || isReadOnly
                            }
                          />
                        </FormControl>
                        <FormDescription>
