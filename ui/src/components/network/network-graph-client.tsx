...
const NodeIcon = React.memo(({ type }: { type: NodeType }) => {
  switch (type) {
    case "NODE_TYPE_CORE":
      return <Cpu className="h-4 w-4 text-blue-500 dark:text-blue-400" />;
    ...
    case "NODE_TYPE_WEBHOOK":
      return <Webhook className="h-4 w-4 text-pink-500 dark:text-pink-400" />;
    case "NODE_TYPE_RESOURCE":
      return <Database className="h-4 w-4 text-cyan-500 dark:text-cyan-400" />;
 ...
 * @returns The rendered component.
 */
export function NetworkGraphFlow({
  widgetMode = false,
}: NetworkGraphFlowProps) {
  ...
  const selectedNode = useMemo(() => {
    ...
    return node ? (node as Node & { data: NodeData }) : null;
  ...
  const filteredNodes = useMemo(() => {
    return nodes.filter((n) => {
      ...
      if (
        !showSystem &&
        (type === "NODE_TYPE_MIDDLEWARE" || type === "NODE_TYPE_CORE")
      )
        return false;
      if (
        !showTools &&
  ...
  const getStatusBadgeVariant = (status: NodeStatus) => {
    switch (status) {
      case "NODE_STATUS_ACTIVE":
        return "default"; // primary color usually indicates active/good
      case "NODE_STATUS_INACTIVE":
        return "secondary";
      case "NODE_STATUS_ERROR":
        return "destructive";
  ...
  return (
    <div
      className={cn(
        "h-full w-full relative bg-muted/5 group",
        widgetMode && "rounded-lg overflow-hidden",
      )}
    >
      {!widgetMode && (
        <div
          className={cn(
            "absolute top-4 left-4 right-4 z-10 space-y-4 pointer-events-none flex flex-col gap-2 transition-all",
            ...
            >
              <CardTitle className="text-lg flex items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={cn(
                      isControlsExpanded ? "block" : "hidden sm:block",
                    )}
                  >
                    Network Graph
                  </span>
                  ...
                  <div className="mt-2 space-y-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="show-system"
                        checked={showSystem}
                        onCheckedChange={(c) => setShowSystem(!!c)}
                      />
                      <Label
                        htmlFor="show-system"
                        className="text-xs font-normal cursor-pointer"
                      >
                        Show System (Core/Middleware)
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="show-tools"
                        checked={showTools}
                        onCheckedChange={(c) => setShowTools(!!c)}
                      />
                      <Label
                        htmlFor="show-tools"
                        className="text-xs font-normal cursor-pointer"
                      >
                        Show Capability Details (Tools)
                      </Label>
                    </div>
      ...
      >
        {!widgetMode && (
          <Controls
            ...
            className="bg-background/80 backdrop-blur border-muted shadow-sm dark:bg-slate-900/80 dark:border-slate-800 dark:text-slate-200 [&>button]:!border-muted [&>button]:!bg-transparent hover:[&>button]:!bg-muted"
          />
        )}
          ...
          <Panel position="bottom-center" className="mb-8">
            <div className="bg-background/90 p-2 px-4 rounded-full border shadow-lg backdrop-blur text-[10px] text-muted-foreground flex gap-4 items-center">
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-blue-500"></div> Core
              </div>
              <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-green-500"></div> Client
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-amber-500"></div> Tool
              </div>
              <div className="flex items-center gap-1.5">
          ...
          <div className="py-6 space-y-6">
            ...
            <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg border">
              ...
              <Badge
                variant={
                  selectedNode
                    ? getStatusBadgeVariant(selectedNode.data.status)
                    : "outline"
                }
                className="flex items-center gap-1.5 px-3 py-1"
              >
                {selectedNode && (
                  <StatusIcon status={selectedNode.data.status} />
                )}
            ...
            {selectedNode?.data.metrics && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <MetricCard
                    label="QPS"
                    value={selectedNode.data.metrics.qps?.toFixed(2)}
                    unit="req/s"
                  />
                  <MetricCard
                    label="Latency"
                    value={selectedNode.data.metrics.latencyMs?.toFixed(0)}
                    unit="ms"
                  />
                  <MetricCard
                    label="Error Rate"
                    value={(
                      (selectedNode.data.metrics.errorRate || 0) * 100
                    ).toFixed(2)}
                    unit="%"
                    intent={
                      (selectedNode.data.metrics.errorRate || 0) > 0.05
                        ? "danger"
                        : "neutral"
                    }
                  />
            ...
            {selectedNode?.data.metadata &&
              Object.keys(selectedNode.data.metadata).length > 0 && (
                <div className="space-y-3">
                  <h4 className="font-medium text-sm flex items-center gap-2">
                  ...
                  <div className="bg-black/90 rounded-lg p-3 overflow-hidden border border-slate-800">
                    <table className="w-full text-xs font-mono">
                      <tbody>
                        {Object.entries(selectedNode.data.metadata).map(
                          ([k, v]) => (
                            <tr
                              key={k}
                              className="border-b border-white/10 last:border-0"
                            >
                              <td className="py-2 pr-4 text-slate-400 select-none w-1/3">
                                {k}
                              </td>
                              <td className="py-2 text-slate-100 break-all">
                                {String(v)}
                              </td>
                            </tr>
                          ),
...
const MetricCard = React.memo(
  ({
    ...
    return (
      <Card
        className={`p-3 bg-card/50 ${intent === "danger" ? "border-red-200 bg-red-50 dark:bg-red-900/10 dark:border-red-900" : ""}`}
      >
        <div className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">
          {label}
        </div>
        <div
          className={`text-xl font-bold mt-1 ${intent === "danger" ? "text-red-600 dark:text-red-400" : ""}`}
        >
          {value || "-"}{" "}
