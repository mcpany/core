  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-popover text-popover-foreground border rounded-md shadow-md p-2 text-sm">
        ...
        {data.serviceId && (
          <p className="text-xs text-muted-foreground">{data.serviceId}</p>
        )}
        <p className="font-mono mt-1">{formatTokenCount(data.size)} tokens</p>
      </div>
    );
  }
  return null;
...
export function ContextTreemap() {
  const { tools, getToolCost, loading, disabledToolIds } =
    useRecursiveContext();
