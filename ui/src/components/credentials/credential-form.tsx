...
export function CredentialForm({
  ...
  if (initialData?.authentication?.bearerToken)
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      apiKeyParamName:
        initialData?.authentication?.apiKey?.paramName || "Authorization",
      apiKeyLocation:
        initialData?.authentication?.apiKey?.in?.toString() ||
        APIKeyAuth_Location.HEADER.toString(),
      apiKeyValue: initialData?.authentication?.apiKey?.value?.plainText || "",
      bearerToken:
        initialData?.authentication?.bearerToken?.token?.plainText || "",
      basicUsername: initialData?.authentication?.basicAuth?.username || "",
      basicPassword:
        initialData?.authentication?.basicAuth?.password?.plainText || "",
      oauthClientId:
        initialData?.authentication?.oauth2?.clientId?.plainText || "",
      oauthClientSecret:
        initialData?.authentication?.oauth2?.clientSecret?.plainText || "",
    ...
    try {
      ...
      if (values.authType === "api_key") {
        ...
        auth.basic_auth = {
          ...
          password: {
            plain_text: values.basicPassword || "",
            validation_regex: "",
          },
          password_hash: "",
        };
        ...
        auth.oauth2 = {
          client_id: {
            plain_text: values.oauthClientId || "",
            validation_regex: "",
          },
          client_secret: {
            plain_text: values.oauthClientSecret || "",
            validation_regex: "",
          },
      ...
      toast({
        variant: "destructive",
        description: "Failed to save credential: " + error.message,
      });
    }
  }
  ...
  async function handleTest() {
    ...
    try {
      ...
      if (values.authType === "api_key") {
        ...
        auth.bearerToken = {
          token: { plainText: values.bearerToken || "", validationRegex: "" },
        };
      } else if (values.authType === "basic_auth") {
        auth.basicAuth = {
          username: values.basicUsername || "",
          password: {
            plainText: values.basicPassword || "",
            validationRegex: "",
          },
          passwordHash: "",
        };
        ...
        if (initialData?.token?.accessToken) {
          ...
          auth.oauth2 = {
            clientId: {
              plainText: values.oauthClientId || "",
              validationRegex: "",
            },
            clientSecret: {
              plainText: values.oauthClientSecret || "",
              validationRegex: "",
            },
          ...
          toast({
            description:
              "OAuth 2.0 requires connecting (getting a token) before testing.",
          });
          return;
        }
      }
      ...
      if (res.status >= 200 && res.status < 300) {
        ...
        toast({
          variant: "destructive",
          description: `Test returned: ${res.status} ${res.status_text}`,
        });
      }
    } catch (error: any) {
      toast({
  ...
  async function handleConnect() {
    if (!initialData?.id) {
      toast({
        variant: "destructive",
        description: "Please save the credential first.",
      });
      return;
    }
    try {
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem
                          value={APIKeyAuth_Location.HEADER.toString()}
                        >
                          Header
                        </SelectItem>
                        <SelectItem
                          value={APIKeyAuth_Location.QUERY.toString()}
                        >
                          Query
                        </SelectItem>
                        <SelectItem
                          value={APIKeyAuth_Location.COOKIE.toString()}
                        >
                          Cookie
                        </SelectItem>
                      </SelectContent>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="scope1 scope2" {...field} />
                  </FormControl>
