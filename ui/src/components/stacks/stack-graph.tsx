...
import { Database, Terminal, Cpu, Box, AlertTriangle } from "lucide-react";
      ...
      <CardContent className="p-2 space-y-1">
        {data.image && (
          <div className="flex items-center gap-1 text-[10px] text-muted-foreground truncate">
            ...
            <span className="truncate" title={data.image}>
              {data.image}
            </span>
          </div>
        )}
  ...
  const { services, error } = useMemo(() => {
    try {
      ...
      const processService = (key: string, val: any): ParsedService => {
        ...
        const envCount = Array.isArray(env)
          ? env.length
      ...
      if (Array.isArray(rawServices)) {
        serviceList = rawServices.map((val: any) =>
          processService(val.name || "unknown", val),
        );
      } else {
        serviceList = Object.entries(rawServices).map(
          ([key, val]: [string, any]) => processService(key, val),
        );
      }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-4 gap-2 bg-red-50/10">
        ...
        <p className="text-[10px] font-mono opacity-75 max-w-[200px] break-all text-center">
          {error}
        </p>
      </div>
    );
    ...
    <div className="h-full w-full bg-muted/5">
      ...
      >
        ...
        <Controls
          ...
          className="bg-background/80 backdrop-blur border-muted shadow-sm"
        />
      </ReactFlow>
    </div>
  );
