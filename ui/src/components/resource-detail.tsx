    ...
    const fetchDetails = async () => {
      ...
      try {
        const { service: serviceDetails } =
          await apiClient.getService(serviceId);
        ...
        const foundResource = (serviceData as any)?.resources?.find(
        ...
        if (foundResource) {
          ...
          throw new Error(
            `Resource "${resourceName}" not found in service "${serviceDetails.name}".`,
          );
        }
      } catch (e: any) {
