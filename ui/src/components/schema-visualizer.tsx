...
const TypeBadge = ({ type }: { type: string }) => {
  const colorClass = TYPE_COLORS[type] || "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300";
...
const SchemaNode = memo(({ name, schema, required, level = 0 }: SchemaNodeProps) => {
  // ⚡ BOLT: Collapse deep nodes (>1 level) by default to prevent massive initial DOM rendering.
