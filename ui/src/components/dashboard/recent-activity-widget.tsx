        ...
        {loading && traces.length === 0 ? (
            ...
            <div className="relative z-10 p-4 space-y-4">
                {traces.map((trace, index) => {
                    ...
                    // The API can return these inside attributes, or directly on input/output depending on the trace source.
                    const attrs = trace.rootSpan.attributes || {};
                    const input = trace.rootSpan.input || {};
                    const output = trace.rootSpan.output || {};
                    const errorMsg = attrs['error.message'] || trace.rootSpan.errorMessage || trace.errorMessage;
                    const responseDiff = attrs['mcp.response_diff'] || input['mcp.response_diff'] || output['mcp.response_diff'];
                    const hasResponseDiff = responseDiff !== undefined;
                    const requestPayload = attrs['mcp.request_payload'] || input;
                    const responsePayload = attrs['mcp.response_payload'] || output;
                    const serviceId = attrs['mcp.service_id'] || trace.rootSpan.serviceName || 'unknown-service';
                                ...
                                <div className="flex-1 min-w-0 flex flex-col justify-center">
                                    ...
                                    <div className={cn(
                                        ...
                                        <div className="overflow-hidden space-y-3">
                                            ...
                                            {requestPayload && Object.keys(requestPayload).length > 0 && (
                                                <div className="space-y-1.5">
                                                    ...
                                                    <div className="bg-muted/50 rounded-md border border-border/50 overflow-hidden">
                                                        <JsonView data={safeParsePayload(requestPayload)} maxHeight={400} smartTable={true} />
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {!hasResponseDiff && responsePayload && Object.keys(responsePayload).length > 0 && !isError && (
                                                <div className="space-y-1.5">
                                                    ...
                                                    <div className="bg-emerald-500/5 rounded-md border border-emerald-500/20 overflow-hidden">
                                                        <JsonView data={safeParsePayload(responsePayload)} maxHeight={400} smartTable={true} />
                                                    </div>
                                                </div>
                                            )}
