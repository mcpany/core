...
import {
  ...
  ServiceHealth,
  HealthHistoryPoint,
} from "@/hooks/use-service-health-history";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
...
const getStatusColor = (status: string) => {
  switch (status) {
    ...
    case "unhealthy":
      return "border-red-200 bg-red-50 text-red-700 dark:border-red-900/30 dark:bg-red-900/20 dark:text-red-400";
    case "inactive":
      return "border-muted bg-muted/50 text-muted-foreground dark:bg-muted/80 dark:text-muted-foreground/80";
    default:
      return "border-gray-200 bg-gray-50 text-gray-700 dark:border-gray-800 dark:bg-gray-800/50 dark:text-gray-400";
  }
};
 ...
 */
const ServiceDiagnosisPopover = memo(function ServiceDiagnosisPopover({
  message,
}: {
  message: string;
}) {
 ...
 */
const HealthTimeline = memo(function HealthTimeline({
  history,
}: {
        ...
        return (
          <Tooltip key={point.timestamp} delayDuration={0}>
            <TooltipTrigger asChild>
              <div
                className={cn(
                  "w-1.5 h-full rounded-[1px] transition-all cursor-crosshair",
                  colorClass,
                )}
              />
            </TooltipTrigger>
 ...
 */
const ServiceHealthItem = memo(function ServiceHealthItem({
  service,
  history,
}: {
  ...
  return (
    <div className="group flex items-center justify-between p-3 hover:bg-muted/50 rounded-lg transition-colors">
      <div className="flex items-center space-x-4 flex-1 min-w-0">
        <div
          className={cn(
            "p-2 rounded-full bg-background shadow-sm border shrink-0",
  ...
  if (sortedServices.length === 0) {
    return (
      <Card className="col-span-4 backdrop-blur-xl bg-background/60 border border-white/20 shadow-sm">
        <CardHeader>
          <CardTitle>System Health</CardTitle>
          <CardDescription>No services connected.</CardDescription>
        </CardHeader>
        <CardContent>
