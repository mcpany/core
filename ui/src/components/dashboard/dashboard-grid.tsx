...
export function DashboardGrid() {
    ...
    useEffect(() => {
        ...
        const loadLayout = async () => {
            try {
                ...
                if (settings?.dashboard?.layout_json) {
                    const parsed = JSON.parse(settings.dashboard.layout_json);
                ...
                console.error("Failed to load dashboard layout from backend, falling back to local storage", e);
            }

            // Fallback to local storage (legacy or offline)
            ...
            if (saved) {
                try {
                    ...
                    if (parsed.length > 0 && !parsed[0].instanceId) {
                        // ... Legacy migration omitted for brevity as it's less relevant for new backend ...
                        // Assuming new format for backend persistence
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                // Persist to Local Storage (Backup/Fast)
