 ...
 */
export const MemoizedWidgetRenderer = React.memo(
  function MemoizedWidgetRenderer({ widget }: { widget: WidgetInstance }) {
  ...
  const migrateLayout = (parsed: any): WidgetInstance[] => {
    ...
    if (Array.isArray(parsed) && parsed.length > 0 && !parsed[0].instanceId) {
      ...
      const migrated: WidgetInstance[] = parsed.map((w: LegacyWidget) => ({
        ...
        title:
          WIDGET_DEFINITIONS.find((d) => d.type === w.id)?.title || w.title,
        size: (["full", "half", "third", "two-thirds"].includes(w.type)
          ? w.type
          : "third") as WidgetSize,
    ...
    const loadLayout = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.warn(
            "Failed to fetch preferences, falling back to local/default",
          );
...
>>>>>>> origin/main
  ...
  const toggleWidgetVisibility = (instanceId: string) => {
    const updated = widgets.map((w) =>
      w.instanceId === instanceId ? { ...w, hidden: !w.hidden } : w,
    );
    saveWidgets(updated);
  };
              ...
              {widgets.map((widget) => (
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={() =>
                      toggleWidgetVisibility(widget.instanceId)
                    }
                  />
                  <Label
                    htmlFor={`show-${widget.instanceId}`}
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuSub>
                              ...
                              <DropdownMenuSubContent>
                                ...
                                >
                                  <DropdownMenuRadioItem value="full">
                                    <LayoutGrid className="mr-2 h-4 w-4" /> Full
                                    Width
                                  </DropdownMenuRadioItem>
                                  <DropdownMenuRadioItem value="two-thirds">
                                    <Columns className="mr-2 h-4 w-4" /> 2/3
                                    Width
                                  </DropdownMenuRadioItem>
                                  <DropdownMenuRadioItem value="half">
                                    <Columns className="mr-2 h-4 w-4" /> 1/2
                                    Width
                                  </DropdownMenuRadioItem>
                                  <DropdownMenuRadioItem value="third">
                                    <Columns className="mr-2 h-4 w-4" /> 1/3
                                    Width
                                  </DropdownMenuRadioItem>
                                </DropdownMenuRadioGroup>
                              </DropdownMenuSubContent>
