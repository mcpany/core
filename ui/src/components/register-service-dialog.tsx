...
export function RegisterServiceDialog({
  ...
  const [view, setView] = useState<"templates" | "template-config" | "form">(
    "templates",
  );
  const [selectedTemplate, setSelectedTemplate] =
    useState<ServiceTemplate | null>(null);
  const [templateConfigValues, setTemplateConfigValues] = useState<
  ...
  const [pendingConfig, setPendingConfig] =
    useState<UpstreamServiceConfig | null>(null);
  ...
  const defaultValues = serviceToEdit
    ? {
        ...
        type: (serviceToEdit.grpcService
          ? "grpc"
          : serviceToEdit.httpService
            ? "http"
            : serviceToEdit.commandLineService
              ? "command_line"
              : serviceToEdit.openapiService
                ? "openapi"
                : "other") as
          | "grpc"
          | "http"
          ...
          | "other",
        address:
          serviceToEdit.grpcService?.address ||
          serviceToEdit.httpService?.address ||
          serviceToEdit.openapiService?.address ||
          "",
  ...
  const handleCredentialSelect = (credId: string) => {
    ...
    if (cred && cred.authentication) {
      ...
      toast({
        ...
        description: `Applied auth from '${cred.name}'`,
      });
    }
  };
  ...
  useEffect(() => {
    const hasSecrets =
      detectSensitiveData(watchedAddress || "") ||
  ...
  const populateFormFromTemplate = (
      ...
      template.fields.forEach((field) => {
        ...
        if (field.replaceToken) {
          ...
          if (
            ...
            config.commandLineService.command = (
              config.commandLineService.command ?? ""
            ).replace(field.replaceToken, value);
          }
          // Add other paths as needed
        } else {
          // Direct value assignment
          if (
            field.key.startsWith("commandLineService.env.") &&
            config.commandLineService
          ) {
            const envKey = field.key.split(".").pop();
            if (envKey) {
              if (!config.commandLineService.env)
                config.commandLineService.env = {};
              config.commandLineService.env[envKey] = value as any;
            }
          } else if (
            field.key === "httpService.address" &&
            config.httpService
          ) {
    ...
    if (config.httpService?.address)
      form.setValue("address", config.httpService.address);
    if (config.grpcService?.address)
      form.setValue("address", config.grpcService.address);
    if (config.openapiService?.address)
      form.setValue("address", config.openapiService.address);
    if (config.commandLineService?.command)
      form.setValue("command", config.commandLineService.command);
  ...
  const constructConfig = (
    ...
    if (values.type === "other") {
      if (!values.configJson)
        throw new Error("Config JSON is required for 'Other' type");
    ...
    try {
      if (isEditing) {
        ...
        toast({
          ...
          description: `${config.name} registered successfully.`,
        });
      }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent
        className={cn(
          "max-h-[85vh] overflow-y-auto transition-all duration-200",
          showDiff ? "sm:max-w-[900px]" : "sm:max-w-[700px]",
        )}
      >
        <DialogHeader>
          <DialogTitle>
          ...
          <DialogDescription>
            {showDiff
              ...
              : view === "templates"
                ? "Choose a template to quickly configure a popular service, or start from scratch."
                : view === "template-config"
                  ? "Fill in the required information to configure this service."
        ...
        {showDiff && serviceToEdit && pendingConfig ? (
          <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={() => performSave(pendingConfig)}>
                Confirm & Save
              </Button>
            </div>
          </div>
              ...
              >
                <TabsContent value="basic" className="space-y-4">
                  {hasPotentialSecrets && (
                    <Alert
                      variant="destructive"
                      className="bg-yellow-50 border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-900 text-yellow-800 dark:text-yellow-200"
                    >
                      <AlertTriangle className="h-4 w-4" />
                      <AlertTitle>Potential Sensitive Data Detected</AlertTitle>
                      <AlertDescription>
                        Your configuration appears to contain secrets (keys,
                        tokens, or passwords) in non-secret fields. Consider
                        using the <strong>Authentication</strong> tab or
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="prod, external, db (comma separated)"
                            {...field}
                          />
                        </FormControl>
                        <FormDescription>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            The command to run the MCP server. Ensure all
                  ...
                  {selectedType === "other" && (
                    <div className="text-sm text-muted-foreground">
                      Please switch to the Advanced tab to configure other
                      service types using JSON.
                    </div>
                  )}

                  {/* Hint for Env Vars if using a template that needs them */}
                  {selectedTemplate &&
                    selectedTemplate.config.commandLineService?.env &&
                    Object.keys(selectedTemplate.config.commandLineService.env)
                      .length > 0 && (
                      <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-900 rounded-md text-sm text-yellow-800 dark:text-yellow-200">
                        <strong>Note:</strong> This template uses environment
                        variables (e.g., keys). You may need to configure them
                        in the "Advanced (JSON)" tab or ensure they are set in
                ...
                <TabsContent value="auth" className="space-y-4">
                  <div className="space-y-4 border p-4 rounded-md bg-muted/50">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Select a saved credential to apply its authentication
                      configuration to this service.
                    </p>
                  ...
                  <div className="space-y-2">
                    ...
                    {form.watch("upstreamAuth") ? (
                      <div className="text-sm border p-2 rounded">
                        ...
                        <Button
                          ...
                          className="mt-2"
                          onClick={() =>
                            form.setValue("upstreamAuth", undefined)
                          }
                        >
                    ...
                    {(form.watch("upstreamAuth")?.oauth2 ||
                      selectedTemplate?.config?.upstreamAuth?.oauth2) && (
                      <div className="pt-2">
                        <Button
                          type="button"
                          variant="secondary"
                          ...
                          onClick={async () => {
                            ...
                            if (!oauthConfig) {
                              toast({
                                title: "No OAuth Config",
                                description: "Please configure OAuth2 first.",
                                variant: "destructive",
                              });
                              return;
                            }
                            ...
                            const state = Math.random()
                              .toString(36)
                              .substring(7); // Temporary client-side state for session key?
                            ...
                            try {
                              ...
                              if (
                                !serviceToEdit?.id &&
                                !form.getValues("upstreamAuth")
                              ) {
                                toast({
                                  title: "Save Service First",
                                  description:
                                    "Please register the service before authenticating.",
                                  variant: "default",
                                });
                                return;
                              }
                              ...
                              if (!serviceId) {
                                toast({
                                  title: "Save Required",
                                  description:
                                    "Please save the service before authenticating.",
                                  variant: "destructive",
                                });
                                return;
                              }
                ...
                {validationResult && (
                  <div
                    className={`p-3 rounded-md text-sm mb-4 ${validationResult.valid ? "bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-300" : "bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300"}`}
                  >
                    <div className="flex items-center gap-2">
                ...
                <DialogFooter className="flex justify-between sm:justify-between w-full">
                  ...
                  <Button type="submit">
                    {isEditing ? "Review Changes" : "Register Service"}
                  </Button>
                </DialogFooter>
              </form>
            </Form>
