...
export function StepServiceType() {
          ...
          if (sc.commandLineService && sc.commandLineService.env) {
            for (const [key, val] of Object.entries(
              ...
              if (typeof val === "string") {
                ...
                params[key] =
                  (val as any).plainText || (val as any).plain_text || "";
              } else {
                // Fallback
                params[key] = "";
              }
  ...
  const handleTemplateChange = (val: string) => {
    ...
    if (template) {
      ...
      const newName =
        config.name && config.name !== MANUAL_TEMPLATE.name
          ? config.name
          : template.name;
      updateConfig({
        ...template.config,
        ...
        <Card className="mt-2 bg-muted/50">
          <CardHeader>
            <CardTitle className="text-base">
              {
                templates.find((t) => t.id === (selectedTemplateId || "manual"))
                  ?.name
              }
            </CardTitle>
            <CardDescription>
