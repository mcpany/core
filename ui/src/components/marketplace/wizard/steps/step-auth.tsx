  ...
  const testConnection = async () => {
    ...
    try {
      ...
      setTestResult({
        ...
        message:
          res.message ||
          (res.success ? "Connection successful" : "Connection failed"),
      });
    } catch (e) {
      console.error("Auth test error:", e);
      setTestResult({
        success: false,
        message: "Connection failed (Network/Server Error)",
      });
    } finally {
      setTesting(false);
    }
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      sessionStorage.setItem(
        "oauth_service_id",
        getServiceType(config) === "unknown" ? "" : getServiceType(config),
      );
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <AlertDescription className="text-amber-700 dark:text-amber-400">
          Authentication selected here is used{" "}
          <strong>only to test connectivity</strong> during this configuration.
          The actual credential binding for the running service must be
          configured in the <strong>Upstream Services</strong> page after
          instantiation.
        </AlertDescription>
      </Alert>
      ...
      <div className="space-y-4">
        ...
        {testResult && (
          ...
          >
            <AlertDescription
              className={
                testResult.success ? "text-green-800 dark:text-green-300" : ""
              }
            >
              {testResult.message}
            </AlertDescription>
