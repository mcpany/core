    ...
    const handleSchemaChange = (newParams: Record<string, string>) => {
        ...
        if (config.commandLineService) {
            ...
            Object.entries(newParams).forEach(([k, v]) => {
                let regex = "";
                // Try to find pattern in schema
                if (schema && schema.properties && schema.properties[k] && schema.properties[k].pattern) {
                     regex = schema.properties[k].pattern;
                }

                env[k] = { plainText: v, validationRegex: regex };
            });
            updateConfig({
                commandLineService: {
