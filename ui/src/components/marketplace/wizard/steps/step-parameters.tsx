...
export function StepParameters() {
    ...
    const schema = useMemo(() => {
        try {
            return config.configurationSchema ? JSON.parse(config.configurationSchema) : null;
        } catch {
            return null;
        }
             ...
             <div className="space-y-4 pt-4 border-t">
                 ...
                 <div className="grid gap-2">
                     ...
                     <p className="text-xs text-muted-foreground">
                        Most arguments are passed via environment variables (configured above) or directly in the command string.
                     </p>
                 </div>
             </div>
        </div>
