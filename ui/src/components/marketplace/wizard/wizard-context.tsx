...
export function WizardProvider({ children }: { children: ReactNode }) {
  ...
  const validateStep = (
    ...
    switch (step) {
      case WizardStep.SERVICE_TYPE:
        ...
        if (
          !state.config.commandLineService &&
          !state.config.httpService &&
          !state.config.grpcService &&
          !state.config.mcpService &&
          !state.config.openapiService
        ) {
      ...
      case WizardStep.PARAMETERS:
        if (state.config.openapiService) {
          if (
            !state.config.openapiService.specUrl &&
            !state.config.openapiService.specContent
          ) {
  ...
  const nextStep = () => {
    ...
    setState((prev) => {
      ...
      return {
        ...prev,
        currentStep: next > WizardStep.REVIEW ? WizardStep.REVIEW : next,
      };
    });
  };
