...
const TypeBadge = ({
  ...
  return (
    <span
      className={cn(
        "text-[10px] px-1.5 py-0.5 rounded border font-mono uppercase tracking-wider select-none",
        getTypeColor(type),
      )}
    >
      {displayLabel}
...
export function SchemaViewer({
  ...
  return (
    <div
      className={cn(
        "font-mono text-sm",
        depth > 0 && "ml-3 border-l pl-3 border-border/50",
      )}
    >
        ...
        {hasChildren ? (
          ...
          >
            ...
            <CollapsibleContent>
              <div className="pt-1">
                {isObject &&
                  properties.map(([key, propSchema], idx) => (
                    <SchemaViewer
                      key={key}
                      schema={propSchema}
                      name={key}
                ...
                {isArray && items && (
                  <div className="mt-1">
                    <span className="text-xs text-muted-foreground mb-1 block pl-4">
                      Items:
                    </span>
