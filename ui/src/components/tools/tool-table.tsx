      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead title="Estimated context size when tool is defined">
              Est. Context
            </TableHead>
              ...
              <TableCell colSpan={10} className="h-24 text-center">
                <div className="flex flex-col items-center justify-center text-muted-foreground">
                  ...
                  <p className="text-sm opacity-70">
                    No tools found for this service or matching your search.
                  </p>
                </div>
              </TableCell>
            </TableRow>
            ...
            tools.map((tool) => (
              <TableRow
                ...
                className={cn(
                  "group",
                  isCompact ? "h-8" : "",
                  selected.has(tool.name) ? "bg-muted/50" : "",
                )}
              >
                <TableCell className={cn("pr-0", isCompact ? "py-0 px-2" : "")}>
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleSelectOne(tool.name, !!checked)
                    }
                ...
                <TableCell className={isCompact ? "py-0 px-2" : ""}>
                  ...
                  >
                    <Star
                      className={`h-4 w-4 ${isPinned(tool.name) ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"}`}
                    />
                  </Button>
                </TableCell>
                <TableCell
                ...
                <TableCell className={isCompact ? "py-0 px-2" : ""}>
                  <Badge
                    variant="outline"
                    className={isCompact ? "h-5 text-[10px] px-1" : ""}
                  >
                    {tool.serviceId}
                  </Badge>
                </TableCell>
                ...
                <TableCell
                  className={cn("text-right", isCompact ? "py-0 px-2" : "")}
                >
                  {(() => {
                    const stats =
                      usageStats?.[`${tool.name}@${tool.serviceId}`];
                ...
                <TableCell className={isCompact ? "py-0 px-2" : ""}>
                  <div
                    ...
                    title={`${estimateTokens(JSON.stringify(tool))} tokens`}
                  >
                ...
                <TableCell className={isCompact ? "py-0 px-2" : ""}>
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={() =>
                        toggleTool(tool.name, !tool.disable)
                      }
                    ...
                    <span
                      className={cn(
                        ...
                        isCompact ? "text-[10px] w-12" : "text-sm w-16",
                      )}
                    >
                ...
                >
                  <Button
                    ...
                    className={isCompact ? "h-6 px-2 text-[10px]" : ""}
                  >
                    <Play
                      className={cn("mr-1", isCompact ? "h-2 w-2" : "h-3 w-3")}
                    />{" "}
                    Inspect
                  </Button>
                </TableCell>
