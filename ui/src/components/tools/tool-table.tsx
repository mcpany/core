...
interface ToolTableProps {
  ...
  onSelectTool: (name: string, checked: boolean) => void;
...
export const ToolTable = memo(function ToolTable({
  ...
  const someSelected = tools.some(t => selectedTools.has(t.name));
    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          <TableHead className="w-[30px] pl-2"></TableHead>
          ...
          return (
            <TableRow
                ...
                className={cn(
                    ...
                    isCompact ? "h-8" : "",
                    isSelected ? "bg-muted/50 data-[state=selected]:bg-muted" : ""
                )}
            ...
            >
                <TableCell className={cn("pr-0", isCompact ? "py-0 px-2" : "")}>
                    <Checkbox
                        checked={isSelected}
                        onCheckedChange={(checked) => onSelectTool(tool.name, !!checked)}
                        aria-label={`Select ${tool.name}`}
                    />
                </TableCell>
                <TableCell className={cn("pl-2", isCompact ? "py-0 px-2" : "")}>
                    <Button
                    variant="ghost"
                    size="icon"
                    className="h-6 w-6"
