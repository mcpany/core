...
interface ToolCellsProps {
  ...
  openInspector: (tool: ToolDefinition) => void;
  usageStats?: Record<string, ToolAnalytics>;
  handleSelectOne: (name: string, checked: boolean) => void;
}

const ToolCells = memo(function ToolCells({
  tool,
  isCompact,
  isSelected,
  isPinned,
...
const ToolTableToolbar = memo(function ToolTableToolbar({
  selectedCount,
  onBulkToggle,
  onBulkPin,
  getSelected,
  clearSelection
}: ToolTableToolbarProps) {
  ...
  return (
    <div className="flex items-center gap-2 p-2 bg-muted/40 rounded-md animate-in fade-in slide-in-from-top-1 duration-200 sticky top-0 z-10 backdrop-blur-md border">
      <span className="text-sm text-muted-foreground mr-2 font-medium px-2">{selectedCount} selected</span>
      <div className="h-4 w-px bg-border mx-1" />
      {onBulkToggle && (
        <>
          <Button size="sm" variant="ghost" onClick={() => {
            onBulkToggle(getSelected(), true);
            clearSelection();
          ...
          <Button size="sm" variant="ghost" onClick={() => {
            onBulkToggle(getSelected(), false);
            clearSelection();
...
const TableHeaderContent = memo(function TableHeaderContent({ isAllSelected, handleSelectAll }: TableHeaderContentProps) {
  return (
...
export const ToolTable = memo(function ToolTable({
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Fallback to standard Table for small lists (e.g. tests or small deployments) */}
      {tools.length < 50 ? (
        <Table>
          <TableHeader>
            <TableHeaderContent isAllSelected={isAllSelected} handleSelectAll={handleSelectAll} />
          </TableHeader>
          <TableBody>
            {tools.map((tool) => (
        ...
        /* ⚡ BOLT: Replaced standard Table with TableVirtuoso for large lists */
