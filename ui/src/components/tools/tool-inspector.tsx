...
import { BarChart3, Activity, History as HistoryIcon, RefreshCw, Zap } from "lucide-react";
...
export function ToolInspector({ tool, open, onOpenChange }: ToolInspectorProps) {
  ...
  useEffect(() => {
      if (open && tool) {
          fetchMetrics();
          setOutput(null); // Clear output on open
      }
  }, [open, tool]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <Tabs defaultValue="testing" className="w-full flex-1 flex flex-col overflow-hidden">
            ...
            <TabsContent value="testing" className="space-y-4 flex-1 overflow-hidden mt-0">
                <ToolForm
                    tool={tool}
                    onSubmit={handleExecute}
                    onCancel={() => onOpenChange(false)}
                    submitLabel={loading ? "Executing..." : "Execute"}
                    ...
                    {output && (
                         <div className="grid gap-2 mb-2">
                            ...
                            <ScrollArea className="h-[150px] w-full rounded-md border p-4 bg-muted/50">
                                <pre className="text-xs text-green-600 dark:text-green-400 font-mono whitespace-pre-wrap break-all">{output}</pre>
                            </ScrollArea>
                        </div>
                    )}
                </ToolForm>
            ...
            <TabsContent value="metrics" className="space-y-6 flex-1 overflow-y-auto mt-0">
