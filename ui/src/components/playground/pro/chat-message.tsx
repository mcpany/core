...
const _SyntaxHighlighterLazy = lazy(() =>
  import("react-syntax-highlighter/dist/esm/light").then(async (mod) => {
    ...
    const jsLang =
      await import("react-syntax-highlighter/dist/esm/languages/hljs/javascript");
    const pythonLang =
      await import("react-syntax-highlighter/dist/esm/languages/hljs/python");
    const bashLang =
      await import("react-syntax-highlighter/dist/esm/languages/hljs/bash");
...
const SyntaxHighlighter = (props: any) => (
  <Suspense
    fallback={
      <div className="p-4 bg-[rgba(0,0,0,0.4)] h-12 animate-pulse rounded" />
    }
  >
...
const _DiffEditorLazy = lazy(() =>
  import("@monaco-editor/react").then((mod) => ({ default: mod.DiffEditor })),
);
const DiffEditor = (props: any) => (
  <Suspense
    fallback={
      <div className="h-full w-full bg-[#1e1e1e] animate-pulse rounded-md" />
    }
  >
...
export function ChatMessage({ message, onReplay, onRetry }: ChatMessageProps) {
      ...
      <div className="flex justify-end gap-3 pl-10 group">
        <div className="flex flex-col items-end gap-1 max-w-full">
          <div className="bg-primary text-primary-foreground rounded-2xl rounded-tr-sm px-4 py-2 shadow-sm max-w-full overflow-hidden">
            <p className="whitespace-pre-wrap font-mono text-sm break-words">
              {message.content}
            </p>
          </div>
        ...
        <Card className="w-full border border-primary/20 bg-primary/5 shadow-none overflow-hidden">
          <CardHeader className="p-3 pb-2 flex flex-row items-center gap-2 space-y-0 border-b border-primary/10 bg-primary/10">
            ...
            <span
              ...
              title="Estimated token usage for arguments"
            >
              {formatTokenCount(
                estimateTokens(JSON.stringify(message.toolArgs || {})),
              )}{" "}
              tokens
            </span>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    onClick={() =>
                      onReplay(message.toolName!, message.toolArgs || {})
                    }
          ...
          <CardContent className="p-0">
            <div className="relative group/code">
              ...
              <Button
                ...
                onClick={() =>
                  copyToClipboard(JSON.stringify(message.toolArgs, null, 2))
                }
              >
                {copied ? (
  ...
  if (message.type === "tool-result") {
    const prevUnwrapped =
      message.previousResult !== undefined
        ? deepParseJson(unwrapMcpResult(message.previousResult))
        : undefined;
    ...
    return (
      <>
        <div className="flex justify-start gap-3 pl-11 w-full pr-4 md:pr-10 my-2">
          <Collapsible
            className="w-full group rounded-lg border shadow-sm overflow-hidden bg-card"
            defaultOpen
          >
            <div className="flex items-center justify-between bg-muted/30 px-3 py-2 text-xs border-b">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span
                  ...
                  title="Estimated token usage for result"
                >
                  (
                  {formatTokenCount(
                    estimateTokens(JSON.stringify(message.toolResult)),
                  )}{" "}
                  tokens)
                </span>
              </div>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    onClick={() =>
                      onRetry(message.toolName!, message.toolArgs || {})
                    }
