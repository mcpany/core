...
export function SmartResultRenderer({ result }: SmartResultRendererProps) {
  ...
  const mcpContent = useMemo<McpContent[] | null>(() => {
    if (Array.isArray(unwrappedContent) && unwrappedContent.length > 0) {
      const isMcp = unwrappedContent.every(
        (item: any) =>
          typeof item === "object" &&
          item !== null &&
          (item.type === "text" ||
  ...
  const tableData = useMemo(() => {
    ...
    if (Array.isArray(fullyUnwrapped) && fullyUnwrapped.length > 0) {
      const isTable = fullyUnwrapped.every(
    ...
    if (Array.isArray(unwrappedContent) && unwrappedContent.length > 0) {
      const isTable = unwrappedContent.every(
        (item: any) => typeof item === "object" && item !== null,
      );
  ...
  const renderRaw = () => <JsonView data={result} maxHeight={400} />;
          ...
          <TableHeader className="bg-muted/50 sticky top-0">
            <TableRow>
              {columns.map((col) => (
                <TableHead
                  key={col}
                  className="whitespace-nowrap font-medium text-xs px-2 py-1 h-8"
                >
                  {col}
                </TableHead>
              ))}
