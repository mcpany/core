...
import { Search, Command, ArrowRight, Zap, Filter, Library } from "lucide-react";
 ...
 */
export function ToolSidebar({ tools, onSelectTool, onRunRequest, className }: ToolSidebarProps) {
        ...
        <div className={cn("flex flex-col h-full bg-muted/10 border-r", className)}>
            <Tabs defaultValue="tools" className="flex flex-col h-full">
                <div className="p-2 border-b bg-background">
                    <TabsList className="grid w-full grid-cols-2">
                        <TabsTrigger value="tools" className="text-xs">
                            <Command className="h-3 w-3 mr-2" /> Tools
                        </TabsTrigger>
                ...
                <TabsContent value="tools" className="flex-1 flex flex-col min-h-0 m-0">
                    ...
                    <div className="p-3 border-t bg-muted/5 text-[10px] text-muted-foreground text-center shrink-0">
                        {tools.length} available tools
                    </div>
                </TabsContent>

                <TabsContent value="library" className="flex-1 min-h-0 m-0 overflow-hidden">
                    <LibraryView onRunRequest={onRunRequest || (() => {})} />
                </TabsContent>
            </Tabs>
        </div>
    );
