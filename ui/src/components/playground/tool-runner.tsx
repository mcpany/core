...
import {
  UniversalSchemaForm as SchemaForm,
  Schema,
} from "@/components/shared/universal-schema-form";
...
export function ToolRunner({ tool, onClose }: ToolRunnerProps) {
  ...
  // Real data state
  const [historicalStats, setHistoricalStats] = useState<ToolAnalytics | null>(
    null,
  );
  ...
  const recentStats = useMemo(() => {
    ...
    const avgLatency =
      total > 0
        ? Math.round(
            auditLogs.reduce((acc, curr) => acc + curr.durationMs, 0) / total,
          )
        : 0;
  ...
  const fetchMetrics = async () => {
    ...
    try {
      ...
      const stats = usage.find(
        (u) => u.name === tool.name && u.serviceId === tool.serviceId,
      );
  ...
  const handleCopyCode = (type: "curl" | "python") => {
    ...
    toast({
      ...
      description: `${type === "curl" ? "Curl command" : "Python code"} copied.`,
    });
  };
      ...
      >
        <div className="border-b px-6 bg-muted/5">
          <TabsList className="h-10 bg-transparent p-0">
            <TabsTrigger
              value="builder"
              className="data-[state=active]:bg-transparent data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:shadow-none rounded-none h-10 px-4"
            >
              Request Builder
            </TabsTrigger>
            <TabsTrigger
        ...
        <TabsContent
          value="builder"
          className="flex-1 overflow-hidden p-0 m-0 flex flex-col md:flex-row"
        >
          {/* Left/Top: Input */}
                ...
                <TabsContent value="form" className="mt-0">
                  {parsedInput === undefined ? (
                    <div className="text-destructive text-sm p-4 border border-destructive/50 rounded bg-destructive/10">
                      Invalid JSON. Please fix errors in JSON view to use the
                      Form builder.
                    </div>
                  ) : (
                    <SchemaForm
            ...
            <div className="p-3 border-b bg-muted/10 flex justify-between items-center">
              ...
              {output && (
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    onClick={() => {
                      ...
                      toast({
                        ...
                        description: `Saved as ${tool.name}-result.json`,
                      });
                    }}
                  ...
                  <Badge
                    variant={
                      output.isError || output.error ? "destructive" : "outline"
                    }
                    className={cn(
                      "text-[10px]",
                      output.isError || output.error
                        ? ""
        ...
        <TabsContent value="metrics" className="flex-1 overflow-y-auto p-6 m-0">
          <div className="flex justify-end mb-4">
            ...
            >
              <RefreshCw
                className={cn("h-3 w-3", metricsLoading && "animate-spin")}
              />
              Refresh
            </Button>
          </div>
          <div className="grid grid-cols-4 gap-4 mb-8">
            <div className="space-y-1 p-4 border rounded-lg bg-card">
              ...
              <p className="text-2xl font-bold">
                {historicalStats?.totalCalls ?? recentStats.total ?? 0}
              </p>
            </div>
            <div className="space-y-1 p-4 border rounded-lg bg-card">
              <p className="text-[10px] uppercase text-muted-foreground font-semibold">
              ...
              <p className="text-2xl font-bold text-green-500">
                {historicalStats?.successRate ?? 100}%
              </p>
            </div>
            <div className="space-y-1 p-4 border rounded-lg bg-card">
              <p className="text-[10px] uppercase text-muted-foreground font-semibold">
              ...
              <div className="rounded-md border divide-y">
                ...
                {[...recentStats.chartData]
                  ...
                  .map((h, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between text-sm p-3 hover:bg-muted/50 transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={cn(
        ...
        <TabsContent value="schema" className="flex-1 overflow-hidden p-6 m-0">
          <Tabs defaultValue="visual" className="w-full h-full flex flex-col">
            ...
            <TabsContent
              value="visual"
              className="flex-1 overflow-hidden rounded-md border bg-muted/20 mt-0"
            >
            ...
            <TabsContent
              value="json"
              className="flex-1 overflow-hidden rounded-md border bg-muted/50 mt-0"
            >
              <ScrollArea className="h-full w-full p-4">
