...
import { Plus, Trash2, Edit, Loader2, Layers, Wrench, FileArchive } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {skills.map((skill) => (
          <Card key={skill.name} className="backdrop-blur-sm bg-background/50 border-muted-foreground/20 hover:border-primary/50 transition-all duration-300">
            <CardHeader>
            ...
            <CardContent>
              <div className="flex flex-wrap items-center gap-2 pt-2">
                ...
                <Link to={`/skills/${skill.name}`} className="ml-auto text-xs text-primary hover:underline font-medium">
                  View Details
                </Link>
              </div>
            </CardContent>
        ...
        {skills.length === 0 && (
          <div className="col-span-full flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6 max-w-sm">
              Skills are reusable capabilities combining tools and prompts. Create one to get started!
            </p>
            <Link to="/skills/create">
              <Button>
                <Plus className="mr-2 h-4 w-4" /> Create Skill
      ...
      <AlertDialog open={!!skillToDelete} onOpenChange={(open) => !open && setSkillToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete the skill "{skillToDelete}"? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
