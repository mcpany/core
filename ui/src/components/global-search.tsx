...
export function GlobalSearch() {
  ...
  const fetchData = React.useCallback(async () => {
    setLoading(true);
    try {
      const [servicesData, toolsData, resourcesData, promptsData] =
        await Promise.all([
  ...
  const restartService = React.useCallback(
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          ...
          <CommandGroup heading="Suggestions">
            ...
            <CommandItem
              onSelect={() => runCommand(() => router("/upstream-services"))}
            >
            ...
            <CommandItem
              onSelect={() => runCommand(() => router("/resources"))}
            >
            ...
            <CommandItem
              onSelect={() => runCommand(() => router("/playground"))}
            >
          ...
          <CommandGroup heading="System Actions">
            <CommandItem
              ...
              onSelect={() => runCommand(() => setShowShortcuts(true))}
            >
            ...
            <CommandItem
              ...
              onSelect={() =>
                copyToClipboard(window.location.href, "Current URL")
              }
            >
            ...
            <CommandGroup heading="Services">
              {services.map((service) => (
                <CommandItem
                  ...
                  onSelect={() =>
                    runCommand(() =>
                      router(`/upstream-services?id=${service.id}`),
                    )
                  }
                >
              ...
              {query.length > 0 &&
                services.map((service) => (
                  <CommandItem
                    ...
                    value={`restart service ${service.name}`}
                    onSelect={() => restartService(service.name)}
                  >
            ...
            <CommandGroup heading="Tools">
              {tools.map((tool) => (
                <CommandItem
                  ...
                  onSelect={() =>
                    runCommand(() => router(`/tools?name=${tool.name}`))
                  }
                >
          ...
          {resources.length > 0 && (
            <CommandGroup heading="Resources">
              {resources.map((resource) => (
                <CommandItem
                  ...
                  onSelect={() =>
                    runCommand(() =>
                      router(
                        `/resources?uri=${encodeURIComponent(resource.uri)}`,
                      ),
                    )
                  }
                >
              ...
              {query.length > 0 &&
                resources.map((resource) => (
                  <CommandItem
                    key={`copy-${resource.uri}`}
                    value={`copy uri ${resource.name}`}
                    onSelect={() =>
                      copyToClipboard(resource.uri, "Resource URI")
                    }
                  >
            ...
            <CommandGroup heading="Prompts">
              {prompts.map((prompt) => (
                <CommandItem
                  ...
                  onSelect={() =>
                    runCommand(() => router(`/prompts?name=${prompt.name}`))
                  }
                >
                  <MessageSquare className="mr-2 h-4 w-4" />
