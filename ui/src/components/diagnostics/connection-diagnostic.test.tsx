...
describe("ConnectionDiagnosticDialog", () => {
  ...
  it("opens the dialog and starts diagnostics", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Client-Side Configuration Check"),
      ).toBeInTheDocument();
    });
  ...
  it("detects HTTP service and adds browser check step", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Browser Connectivity Check"),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Successfully connected to HTTP server from browser/),
      ).toBeInTheDocument();
    });
  });
  ...
  it("displays context-aware suggestion for 404 error", async () => {
    ...
    expect(
      screen.getAllByText(/Check the URL path in your configuration/).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("displays context-aware suggestion for Connection Refused", async () => {
    ...
    // Use getAllByText because it appears in logs and the card
    expect(
      screen.getAllByText(/Check if the upstream service is running/).length,
    ).toBeGreaterThan(0);
  });
    ...
    await waitFor(() => {
      expect(
        screen.getByText((content) =>
          content.includes("WARNING: You are using 'localhost'"),
        ),
      ).toBeInTheDocument();
    });
    expect(
      screen.getByText((content) => content.includes("host.docker.internal")),
    ).toBeInTheDocument();
