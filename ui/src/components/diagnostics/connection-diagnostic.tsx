...
export function ConnectionDiagnosticDialog({
  service,
  trigger,
}: ConnectionDiagnosticDialogProps) {
  ...
  const resetSteps = () => {
    const initialSteps: DiagnosticStep[] = [
      {
        id: "config",
        name: "Client-Side Configuration Check",
        status: "pending",
        logs: [],
      },
      {
    ...
    if (service.websocketService || service.httpService) {
      initialSteps.splice(1, 0, {
        id: "browser_connectivity",
        name: "Browser Connectivity Check",
        status: "pending",
        logs: [],
      });
    }
  ...
  const runDiagnostics = async () => {
    ...
    if (service.httpService) {
      ...
      if (!url.startsWith("http")) {
        addLog(
          "config",
          "Error: HTTP address must start with http:// or https://",
        );
        isValid = false;
      } else {
      ...
      addLog(
        "config",
        `Validating Command: ${service.commandLineService.command}`,
      );
      ...
      if (!url.startsWith("ws://") && !url.startsWith("wss://")) {
        addLog(
          "config",
          "Error: WebSocket address must start with ws:// or wss://",
        );
        isValid = false;
      }
    }
    ...
    if (service.websocketService) {
      ...
      addLog(
        ...
        `Attempting to connect to ${url} from browser...`,
      );
      ...
      try {
        ...
        addLog(
          "browser_connectivity",
          "Successfully connected to WebSocket server from browser.",
        );
        updateStep("browser_connectivity", {
          status: "success",
          detail: "Accessible",
        });
      } catch (error: unknown) {
        ...
        addLog(
          "browser_connectivity",
          `Failed to connect from browser: ${msg}`,
        );
        addLog(
          "browser_connectivity",
          "Note: This is expected if the server is internal or behind a firewall not accessible from your browser.",
        );
        updateStep("browser_connectivity", {
          status: "failure",
          detail: "Not Accessible",
        });
      ...
      addLog(
        "browser_connectivity",
        `Attempting to connect to ${httpUrl} from browser...`,
      );
      ...
      try {
        ...
        addLog(
          "browser_connectivity",
          "Successfully connected to HTTP server from browser.",
        );
        addLog(
          "browser_connectivity",
          "Note: 'no-cors' mode used. We can reach the server, but cannot read the response due to CORS policy. This confirms network connectivity.",
        );
        updateStep("browser_connectivity", {
          status: "success",
          detail: "Accessible",
        ...
        if (
          httpUrl.includes("localhost") ||
          httpUrl.includes("127.0.0.1") ||
          httpUrl.includes("0.0.0.0")
        ) {
          addLog(
            "browser_connectivity",
            "⚠️ WARNING: You are using 'localhost' or loopback address.",
          );
          addLog(
            "browser_connectivity",
            "If MCP Any is running in Docker, it cannot access 'localhost' of your host machine directly.",
          );
          addLog(
            "browser_connectivity",
            "Try using 'host.docker.internal' or your LAN IP address.",
          );
        }

        addLog(
          "browser_connectivity",
          "Possible causes: Server down, blocked by firewall, invalid SSL cert, Mixed Content blocking, or CSP (Content Security Policy) restrictions.",
        );
        updateStep("browser_connectivity", {
          status: "failure",
          detail: "Not Accessible",
    ...
    try {
      ...
      if (!serviceStatus) {
        addLog(
          "backend_health",
          "Warning: Service not found in backend registry.",
        );
        addLog(
          "backend_health",
          "This might happen if the service was just added or backend is restarting.",
        );
        updateStep("backend_health", {
          status: "failure",
        ...
        addLog(
          "backend_health",
          `Backend reports status: ${serviceStatus.status.toUpperCase()}`,
        );
        ...
        if (serviceStatus.status === "healthy") {
          ...
          addLog(
            "backend_health",
            `Error: ${serviceStatus.message || "Unknown error"}`,
          );
          ...
          if (diagnosis.category !== "unknown") {
            addLog(
              "backend_health",
              `Analysis: ${diagnosis.title} - ${diagnosis.description}`,
            );
    ...
    try {
      ...
      if (s) {
        ...
        if (s.lastError) {
          ...
          updateStep("operational", {
            status: "failure",
            detail: "Operational Error",
          });
        } else if ((s.toolCount ?? 0) === 0) {
          addLog(
            "operational",
            "Warning: Service is healthy but exposed 0 tools.",
          );
          addLog(
            "operational",
            "Check if the service requires specific configuration to expose tools (e.g., allowed paths, exposed schemas).",
          );
          setDiagnosticResult({
            category: "configuration",
            title: "No Tools Discovered",
            description:
              "The service connected successfully but did not register any tools.",
            suggestion:
              "1. Check upstream service configuration (e.g. allowed paths for filesystem).\n2. Verify the upstream service actually exposes tools (some might only expose resources).\n3. Check logs for silent failures.",
            severity: "warning",
          });
          updateStep("operational", {
            status: "success",
            detail: "No Tools (Warning)",
          }); // Green but warned in logs
        } else {
          updateStep("operational", {
            status: "success",
            detail: "Fully Operational",
          });
        }
      } else {
