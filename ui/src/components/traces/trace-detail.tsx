...
import { StructuredDataViewer } from "@/components/ui/structured-data-viewer";
...
function SpanIcon({ type }: { type: Span["type"] }) {
  switch (type) {
    case "tool":
      return <Terminal className="h-3 w-3 text-amber-500" />;
    case "service":
  ...
  return (
    <div className="group">
      ...
      >
        {/* Tree Column */}
        <div
          className="flex-1 flex items-center gap-2 min-w-[200px] overflow-hidden"
        ...
        >
          <button
            ...
            className={cn(
              ...
              !span.children?.length && "invisible",
            )}
          >
            {expanded ? (
          ...
          <span className="font-medium truncate" title={span.name}>
            {span.name}
          </span>
          {span.status === "error" && (
              ...
              <Tabs defaultValue="input" className="w-full">
                <div className="flex items-center justify-between mb-2">
                  <TabsList className="h-8">
                    <TabsTrigger value="input" className="text-xs px-3 h-6">
                      <Code className="h-3 w-3 mr-1" /> Input
                    </TabsTrigger>
                    <TabsTrigger value="output" className="text-xs px-3 h-6">
...
export function TraceDetail({ trace }: { trace: Trace | null }) {
  ...
  const handleExportJSON = () => {
    ...
    const dataStr =
      ...
      encodeURIComponent(JSON.stringify(trace, null, 2));
  ...
  return (
    <div className="h-full flex flex-col bg-background">
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            <div className="flex items-center gap-1">
              <Clock className="h-3 w-3" /> {trace.totalDuration}ms
            </div>
            <div className="flex items-center gap-1">
            ...
            <div
              ...
              title={`Input: ${rootInputTokens} | Output: ${rootOutputTokens}`}
            >
            ...
            <div className="flex items-center gap-1 font-mono text-xs bg-muted px-1 rounded">
              {trace.id}
            </div>
          </div>
        </div>
          ...
          {trace.rootSpan.type === "tool" && (
            <>
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  handleReplay(trace.rootSpan.name, trace.rootSpan.input)
                }
                className="gap-2"
              >
      ...
      >
        ...
        <TabsContent
          value="overview"
          className="flex-1 p-0 overflow-hidden m-0"
        >
          <ScrollArea className="h-full p-6">
                ...
                <CardContent className="space-y-4">
                  {diagnostics.map((diag, i) => (
                    <Alert
                      key={i}
                      variant={
                        diag.type === "error" ? "destructive" : "default"
                      }
                    >
              ...
              <CardContent className="pl-2 pr-6">
                <div className="w-full border rounded-md bg-background/50 overflow-hidden">
                  ...
                  <div className="flex text-xs font-medium text-muted-foreground border-b p-2 bg-muted/20">
                    ...
                    <div className="w-[40%] md:w-[50%] pl-4 border-l">
                      Timeline
                    </div>
                  </div>
                  <WaterfallItem
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-medium flex items-center gap-2">
                    <Code className="h-4 w-4" /> Root Input
                  </CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-medium flex items-center gap-2">
                    <Terminal className="h-4 w-4" /> Root Output
                  </CardTitle>
                </CardHeader>
                <CardContent>
