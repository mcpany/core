 ...
 */
export function ReplayDiffDialog({
  open,
  onOpenChange,
  trace,
}: ReplayDiffDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Re-executing <strong>{trace.rootSpan.name}</strong> with original
            inputs.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 min-h-0 py-4 flex flex-col gap-4">
          ...
          {error && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                The replay execution encountered an error. The diff below
                compares the original output with the error message.
              </AlertDescription>
            </Alert>
          )}
