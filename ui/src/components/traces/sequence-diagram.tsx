...
export function SequenceDiagram({ trace }: SequenceDiagramProps) {
  ...
  const { actors, messages } = useMemo(() => {
    ...
    const traverse = (span: Span, callerId: string) => {
      ...
      const requestLabel =
        span.type === "service" ? `Access ${span.name}` : `Call ${span.name}`;
  ...
  const height =
    messages.length * MESSAGE_HEIGHT + PADDING_TOP + PADDING_BOTTOM;

  const getActorX = (index: number) =>
    PADDING_X + index * ACTOR_GAP + ACTOR_WIDTH / 2;
        ...
        {messages.map((msg, i) => {
          ...
          const color =
            msg.error || msg.status === "error"
              ...
              : "text-primary";
          const strokeColor =
            msg.error || msg.status === "error" ? "#ef4444" : "currentColor";
              ...
              {isSelf ? (
                <>
                  ...
                  <text
                    x={x1 + 35}
                    y={y + 10}
                    className={cn("text-[10px] fill-current", color)}
                  >
                    {msg.label}
                  </text>
                </>
                ...
                <>
                  ...
                  <text
                    ...
                    textAnchor="middle"
                    className={cn(
                      "text-[10px] font-medium fill-current select-none",
                      color,
                    )}
                  >
                    {msg.label}
