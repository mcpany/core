...
export function PromptWorkbench({ initialPrompts = [] }: PromptWorkbenchProps) {
  ...
  const [selectedPrompt, setSelectedPrompt] = useState<PromptDefinition | null>(
    null,
  );
  ...
  const [editingPrompt, setEditingPrompt] = useState<PromptDefinition | null>(
    null,
  );
    ...
    apiClient
      ...
      .then((data) => {
        ...
        const list = Array.isArray(data)
          ? data
          : data && Array.isArray(data.prompts)
            ? data.prompts
            : [];
        setPrompts(list);
        ...
        if (selectedPrompt) {
          const updated = list.find(
            (p: any) =>
              ...
              (p as any).serviceId === (selectedPrompt as any).serviceId,
          );
  ...
  const handleExecute = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await apiClient.executePrompt(
        selectedPrompt.name,
        argumentValues,
      );
  ...
  const handleSavePrompt = async (
    ...
    try {
      ...
      const existingIdx = currentPrompts.findIndex(
        (p: any) => p.name === newPrompt.name,
      );
      ...
      toast({
        title: "Prompt Saved",
        description: `Prompt ${newPrompt.name} saved to service ${service.service.name}`,
      });
  ...
  const handleDeletePrompt = async (prompt: PromptDefinition) => {
    ...
    if (!serviceId) {
      toast({
        title: "Error",
        description: "Cannot delete prompt: Service ID unknown",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const updatedPrompts = (service.service.prompts || []).filter(
        (p: any) => p.name !== prompt.name,
      );
      ...
      toast({
        ...
        description: `Prompt ${prompt.name} removed.`,
      });
  ...
  const togglePromptStatus = async (prompt: PromptDefinition) => {
    ...
    setPrompts(
      prompts.map((p) =>
        p.name === prompt.name ? { ...p, disable: newDisable } : p,
      ),
    );
    if (selectedPrompt?.name === prompt.name) {
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {(getArguments(prompt).length || 0) > 0 && (
                    <span className="text-[10px] text-muted-foreground flex items-center gap-0.5">
                      ...
                      {getArguments(prompt).length} args
                    </span>
                  )}
                </div>
              </button>
        ...
        {selectedPrompt ? (
                        ...
                        {getArguments(selectedPrompt).length > 0 ? (
                          getArguments(selectedPrompt).map((arg) => (
                            <div key={arg.name} className="space-y-1.5">
                              <Label
                                htmlFor={arg.name}
                                className="flex items-center gap-1 text-xs font-mono uppercase text-muted-foreground"
                              >
                                {arg.name}
                                {arg.required && (
                    ...
                    <CardContent className="flex-1 p-0 overflow-auto">
                      {executionResult ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-sm p-8 text-center">
                          ...
                          <p>
                            Configure arguments and click Generate to see the
                            prompt result.
                          </p>
                        </div>
                      )}
          ...
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-8">
            ...
            <p className="max-w-xs text-center mt-2">
              Select a prompt from the library to view details, configure
              arguments, and test execution.
            </p>
