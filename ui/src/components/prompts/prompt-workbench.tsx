...
export function PromptWorkbench({ initialPrompts = [] }: PromptWorkbenchProps) {
  ...
  const handleBulkToggle = async (names: string[], enabled: boolean) => {
      // Optimistic update
      setPrompts(prompts.map(p => names.includes(p.name) ? { ...p, disable: !enabled } : p));

      try {
          await Promise.all(names.map(name => apiClient.setPromptStatus(name, !enabled)));
          toast({
              title: enabled ? "Prompts Enabled" : "Prompts Disabled",
              description: `${names.length} prompts have been ${enabled ? "enabled" : "disabled"}.`
          });
      } catch (e) {
          console.error("Failed to bulk toggle prompts", e);
          loadPrompts(); // Revert
          toast({
              variant: "destructive",
              title: "Error",
              description: "Failed to update some prompts."
          });
      }
  };

  const handleBulkDelete = async (names: string[]) => {
      if (!confirm(`Are you sure you want to delete ${names.length} prompts?`)) return;
      try {
          // Iterate over names and find the prompt to get its serviceId
          const promptsToDelete = prompts.filter(p => names.includes(p.name));
          ...
          toast({ title: "Prompts Deleted", description: `${names.length} prompts removed.` });
          if (selectedPrompt && names.includes(selectedPrompt.name)) {
              setSelectedPrompt(null);
          }
          loadPrompts();
      } catch (e) {
          console.error("Failed to delete prompts", e);
          toast({ title: "Failed to delete prompts", description: String(e), variant: "destructive" });
      }
  };
                  ...
                  <div className="flex items-center space-x-4">
                      <div className="relative w-64">
                          ...
                          <Input
                              ...
                              className="pl-8 h-9 text-sm backdrop-blur-sm bg-background/50"
                          />
                      </div>
                      <Button
                          variant="ghost"
              ...
              <Card className="backdrop-blur-sm bg-background/50">
                  <CardContent className="pt-6">
                      <PromptTable
                          ...
                          isCompact={isCompact}
                          togglePrompt={togglePromptStatus}
                          openInspector={handleSelectPrompt}
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] w-full rounded-md border bg-background shadow-sm overflow-hidden">
      {/* Workbench Full View for Selected Prompt */}
      <div className="flex-1 flex flex-col min-w-0 bg-background">
        <div className="flex flex-col h-full">
            ...
            <div className="p-6 border-b pb-4">
                <div className="flex items-center justify-between">
                    <div>
                        <Button variant="ghost" className="mb-2 -ml-2 text-muted-foreground" onClick={() => setSelectedPrompt(null)}>
                                        ...
                                        {(executionResult?.messages || []).map((msg: any, idx: number) => (
                                            <div key={idx} className="space-y-1">
                                                 ...
                                                 <div className="bg-background border rounded-md p-3 text-sm">
                                                    <SmartResultRenderer result={msg.content} />
                                                 </div>
                                            </div>
                                        ))}
