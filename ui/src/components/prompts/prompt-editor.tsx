...
export function PromptEditor({
  ...
  const defaultServiceId =
    (prompt as any)?.serviceId || (services.length > 0 ? services[0].name : "");
      ...
      if (prompt) {
        form.reset({
          ...
          messages: prompt.messages?.map((m: any) => ({
            ...
            content:
              m.content?.text?.text ||
              (typeof m.content === "string" ? m.content : ""), // Handle content structure
          })) || [{ role: "user", content: "" }],
        });
      } else {
        form.reset({
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                {fields.map((field, index) => (
                  <div
                    key={field.id}
                    className="flex gap-2 items-start p-3 bg-muted/20 rounded-md border"
                  >
