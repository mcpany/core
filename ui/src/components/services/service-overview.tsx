...
export function ServiceOverview({
  ...
  const isHealthy =
    !service.lastError && (!status?.status || status.status === "OK");
  ...
  const displayErrorRate =
    metrics.error_rate !== undefined ? `${metrics.error_rate}%` : "N/A";
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center gap-2">
            <div
              className={`h-3 w-3 rounded-full ${isHealthy ? "bg-green-500" : "bg-red-500"} animate-pulse`}
            />
