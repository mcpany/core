 ...
 * Simulates and previews the HTTP request generated by the tool configuration.
 * Allows executing the tool against the backend.
 *
      ...
      call.parameters.forEach((p: HttpParameterMapping) => {
        if (p.schema?.name) {
          if (p.schema.type === ParameterType.STRING)
            defaults[p.schema.name] = "value";
          else if (
            p.schema.type === ParameterType.NUMBER ||
            p.schema.type === ParameterType.INTEGER
          )
            defaults[p.schema.name] = 0;
          else if (p.schema.type === ParameterType.BOOLEAN)
            defaults[p.schema.name] = false;
        }
      });
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
        <div className="flex flex-col gap-4">
          ...
          <Card>
            ...
            <CardContent
              ...
              data-testid="request-preview-content"
            >
              {preview ? (
                <>
                  ...
                  <div className="text-xs text-muted-foreground">
                    ...
                    client-side simulation. Actual headers and transformation
                    may vary.
                  </div>
                </>
              ) : (
                <div className="text-sm text-muted-foreground italic">
                  Fix JSON arguments to see preview.
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <div
          className="flex flex-col h-full overflow-hidden border rounded-md bg-muted/10"
        ...
        >
          <div className="p-2 border-b bg-muted/20 text-xs font-medium flex justify-between items-center">
            ...
            {executionResult && (
              <Badge
                variant={executionResult.isError ? "destructive" : "secondary"}
