...
export function HttpToolManager({ service, onChange }: HttpToolManagerProps) {
  ...
  const handleToolChange = (
    ...
    updatedCall: HttpCallDefinition,
  ) => {
            ...
            <Card key={index} className="flex items-center justify-between p-4">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className={getMethodColor(call.method ?? 0)}
                  >
          ...
          <SheetHeader>
            <SheetTitle>
              {editingToolIndex !== null && tools[editingToolIndex]
                ? `Edit ${tools[editingToolIndex].name}`
                : "Edit Tool"}
            </SheetTitle>
            <SheetDescription>
              Configure the tool definition and its mapping to the HTTP
              endpoint.
            </SheetDescription>
          </SheetHeader>
          <div className="mt-6">
