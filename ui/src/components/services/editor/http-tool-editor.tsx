 ...
 */
export function HttpToolEditor({
  tool,
  call,
  serviceName,
  onChange,
  ...
  const handleExecute = async (args: any) => {
    if (!serviceName) {
      toast({
        title: "Error",
        description: "Service name missing context. Cannot execute.",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem
                  value={HttpCallDefinition_HttpMethod.HTTP_METHOD_GET.toString()}
                >
                  GET
                </SelectItem>
                <SelectItem
                  value={HttpCallDefinition_HttpMethod.HTTP_METHOD_POST.toString()}
                >
                  POST
                </SelectItem>
                <SelectItem
                  value={HttpCallDefinition_HttpMethod.HTTP_METHOD_PUT.toString()}
                >
                  PUT
                </SelectItem>
                <SelectItem
                  value={HttpCallDefinition_HttpMethod.HTTP_METHOD_DELETE.toString()}
                >
                  DELETE
                </SelectItem>
                <SelectItem
                  value={HttpCallDefinition_HttpMethod.HTTP_METHOD_PATCH.toString()}
                >
                  PATCH
                </SelectItem>
              </SelectContent>
          ...
          <div className="col-span-2 space-y-2">
            ...
            <p className="text-[10px] text-muted-foreground">
              Use <code>{"{paramName}"}</code> for path parameters.
            </p>
          </div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="request" className="space-y-4 mt-4">
          ...
          {localCall.parameters?.map(
            (param: HttpParameterMapping, index: number) => (
                ...
                <CardContent className="p-4 grid grid-cols-12 gap-4">
                  <div className="col-span-3 space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateParameterSchema(index, { name: e.target.value })
                      }
                      placeholder="userId"
                    />
                  </div>
                  <div className="col-span-3 space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(val) =>
                        updateParameterSchema(index, { type: parseInt(val) })
                      }
                    >
                      ...
                      <SelectContent>
                        <SelectItem value={ParameterType.STRING.toString()}>
                          String
                        </SelectItem>
                        <SelectItem value={ParameterType.NUMBER.toString()}>
                          Number
                        </SelectItem>
                        <SelectItem value={ParameterType.INTEGER.toString()}>
                          Integer
                        </SelectItem>
                        <SelectItem value={ParameterType.BOOLEAN.toString()}>
                          Boolean
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  ...
                  <div className="col-span-2 flex items-center justify-center pt-8 space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        updateParameterSchema(index, { isRequired: checked })
                      }
                    />
