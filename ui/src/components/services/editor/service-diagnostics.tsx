...
export function ServiceDiagnostics({ service }: ServiceDiagnosticsProps) {
  ...
  const runDiagnostics = async () => {
    ...
    const configCheck: DiagnosticResult = {
    ...
    if (service.name && service.id) {
      // 2. Runtime Status
      const statusCheck: DiagnosticResult = {
        name: "Runtime Status",
        status: "running",
      };
      ...
      // 3. Tool Discovery
      const toolCheck: DiagnosticResult = {
        name: "Tool Discovery",
        status: "running",
      };
      ...
      try {
        ...
        const tools = toolsResponse.tools.filter(
          (t: any) =>
            t.serviceId === service.name || t.serviceId === service.id,
        );
        ...
        if (tools.length > 0) {
          ...
          toolCheck.details =
            "The service might be running but not exposing any tools, or discovery failed.";
        }
      } catch (e: any) {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Run checks to verify configuration, connectivity, and capability
                discovery.
              </CardDescription>
            </div>
