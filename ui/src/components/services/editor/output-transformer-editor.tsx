 ...
 */
export function OutputTransformerEditor({
  transformer,
  onChange,
}: OutputTransformerEditorProps) {
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value={OutputTransformer_OutputFormat.JSON.toString()}>
              JSON (JSONPath)
            </SelectItem>
            <SelectItem value={OutputTransformer_OutputFormat.XML.toString()}>
              XML (XPath)
            </SelectItem>
            <SelectItem value={OutputTransformer_OutputFormat.TEXT.toString()}>
              Text (Regex)
            </SelectItem>
            <SelectItem value={OutputTransformer_OutputFormat.JQ.toString()}>
              JQ Query
            </SelectItem>
            <SelectItem
              value={OutputTransformer_OutputFormat.RAW_BYTES.toString()}
            >
              Raw Bytes
            </SelectItem>
          </SelectContent>
      ...
      {(localTransformer.format === OutputTransformer_OutputFormat.JSON ||
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground mb-2">
            Map field names to extraction expressions (JSONPath, XPath, or
            Regex).
          </p>
          <KeyValueEditor
