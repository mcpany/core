...
export function ServiceInspector({ service }: ServiceInspectorProps) {
  ...
  const filteredTraces = traces.filter(
    (t) =>
      t.rootSpan.serviceName === service.name ||
      t.rootSpan.serviceName === service.id,
  );
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-lg flex items-center gap-2">
              ...
              <Badge
                ...
                className="font-mono text-xs gap-1 ml-2"
              >
                {isConnected ? (
                  <>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <RefreshCcw
                className={`h-4 w-4 ${loading && !isConnected ? "animate-spin" : ""}`}
              />
            </Button>
          </div>
        </div>
      ...
      <CardContent className="flex-1 min-h-0 p-0 flex flex-col">
        <div className="border-t flex-1 min-h-0">
          <InspectorTable
            ...
            loading={loading && filteredTraces.length === 0}
          />
        </div>
      </CardContent>
    </Card>
