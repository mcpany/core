      ...
      if (inputType === "url") {
        ...
        if (data.openapi || data.swagger) {
          parsedServices = [
            {
              name:
                data.info?.title?.toLowerCase().replace(/\s+/g, "-") ||
                "openapi-service",
              openapiService: {
                address: importUrl,
                specUrl: importUrl,
        ...
        try {
          ...
          if (res.valid) {
            ...
            const isConnectionError =
              res.error?.toLowerCase().includes("reachability") ||
              res.error?.toLowerCase().includes("failed to connect");

            validatedItems[index].validationStatus = isConnectionError
              ? "warning"
              : "invalid";
        ...
        >
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="json">
              <FileJson className="mr-2 h-4 w-4" /> JSON / YAML
            </TabsTrigger>
            <TabsTrigger value="file">
              <Upload className="mr-2 h-4 w-4" /> File Upload
            </TabsTrigger>
            <TabsTrigger value="url">
              <LinkIcon className="mr-2 h-4 w-4" /> URL Import
            </TabsTrigger>
          </TabsList>
            ...
            {inputType === "file" && (
              <div className="flex flex-col items-center justify-center border-2 border-dashed rounded-lg p-12 text-center hover:bg-muted/50 transition-colors">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Drag and drop your JSON/YAML config file here, or click to
                  browse.
                </p>
                <Input
                  type="file"
            ...
            {(inputType === "json" || inputType === "file") && (
              <div className={inputType === "file" ? "hidden" : "block"}>
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Paste a JSON array of service configurations or a single
                  service object.
                </p>
              </div>
            )}
              ...
              <div className="space-y-4 py-8">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Enter a URL to a JSON configuration file or OpenAPI
                    specification.
                  </p>
                </div>
              </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Found {items.length} services. {validCount} valid, {warningCount}{" "}
              warnings.
            </p>
          </div>
        </div>
          ...
          <Table>
                ...
                <TableHead className="w-[50px]">
                  <Checkbox
                    checked={
                      selectedCount > 0 &&
                      selectedCount ===
                        items.filter((i) => i.validationStatus !== "invalid")
                          .length
                    }
            ...
            <TableBody>
              {items.map((item, idx) => (
                <TableRow
                  key={idx}
                  className={!item.selected ? "opacity-60" : ""}
                >
                  <TableCell>
                    <Checkbox
                    ...
                    {item.validationStatus === "warning" && (
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            {item.validationMessage || "Warning"}
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    )}
                    {item.validationStatus === "invalid" && (
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            {item.validationMessage || "Invalid configuration"}
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    )}
                  ...
                  <TableCell className="font-medium">
                    {item.config.name || "Unnamed"}
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline">
                      {item.config.httpService
                        ? "HTTP"
                        : item.config.grpcService
                          ? "gRPC"
                          : item.config.commandLineService
                            ? "CLI"
                            : item.config.mcpService
                              ? "MCP"
                              : "Other"}
                    </Badge>
                  </TableCell>
          ...
          <div className="space-y-4">
            {importSummary.failed > 0 && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  {importSummary.failed} services failed to import. Check the
