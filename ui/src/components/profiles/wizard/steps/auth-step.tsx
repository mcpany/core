...
export function AuthStep({ services, onNext, onBack }: AuthStepProps) {
  const [authServices, setAuthServices] = useState<WizardService[]>([
    ...services,
  ]);
  ...
  const handleTokenInput = (index: number, token: string) => {
    ...
    if (next[index].templateId === "github") {
      next[index].config.upstreamAuth.bearerToken = {
  ...
  return (
    <div className="space-y-6">
        ...
        {authServices.map((svc, idx) => (
          <Card key={idx}>
            ...
            <CardContent className="py-3">
              {svc.templateId === "google-calendar" ||
              svc.templateId === "linear" ? (
                <div className="flex items-center gap-4">
                  <Button
                    variant="outline"
                    onClick={() => initiateOAuth(idx)}
                    disabled={
                      loadingAuth[svc.instanceName] || svc.isAuthenticated
                    }
                  >
                    {loadingAuth[svc.instanceName] && (
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    defaultValue={
                      svc.config.upstreamAuth?.bearerToken?.token?.plainText ||
                      ""
                    }
                  />
                </div>
                ...
                <p className="text-sm">
                  No authentication required (or not supported in wizard yet).
                </p>
              )}
            </CardContent>
          </Card>
      ...
      <div className="flex justify-between">
        ...
        <div>
          <Button variant="ghost" onClick={handleNext} className="mr-2">
            Skip / Finish Later
          </Button>
