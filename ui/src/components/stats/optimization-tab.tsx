...
export function OptimizationTab({
  ...
  const analysis = useMemo(() => {
    ...
    tools.forEach((tool) => {
      ...
      const isGhost =
        (tokens > 500 && calls === 0) || (tokens > 2000 && calls < 5);
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            These tools are consuming significant context window but are rarely
            or never used. Disabling them will save tokens and costs.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {analysis.ghostTools.length === 0 ? (
              ...
              <TableBody>
                {analysis.ghostTools.map(({ tool, tokens }) => (
                  <TableRow key={tool.name}>
                    ...
                    <TableCell>
                      {toolUsage[`${tool.name}@${tool.serviceId}`]
                        ?.totalCalls || 0}
                    </TableCell>
