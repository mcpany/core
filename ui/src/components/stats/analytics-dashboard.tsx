...
export function AnalyticsDashboard() {
  ...
  const [toolUsageMap, setToolUsageMap] = useState<
    Record<string, ToolAnalytics>
  >({});
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        const [traffic, topTools, toolsResponse, toolUsageStats] =
          await Promise.all([
        ...
        const formattedTools = (topTools || []).map(
          (t: any, index: number) => ({
            name: t.name,
            value: t.count,
        ...
        // Top heaviest tools
        setHeaviestTools(
          toolTokens.sort((a, b) => b.tokens - a.tokens).slice(0, 10),
        );
      } catch (error) {
  ...
  const { totalRequests, avgLatency, errorCount, errorRate, avgRps } =
    useMemo(() => {
      // ⚡ BOLT: Memoized traffic stats calculation to prevent re-render waste.
      // Randomized Selection from Top 5 High-Impact Targets
      const totalRequests = trafficData.reduce(
        (acc, cur) => acc + (cur.requests || cur.total || 0),
        0,
      );
      const avgLatency = trafficData.length
        ? Math.floor(
            trafficData.reduce((acc, cur) => acc + (cur.latency || 0), 0) /
              trafficData.length,
          )
        : 0;
      const errorCount = trafficData.reduce(
        (acc, cur) => acc + (cur.errors || 0),
        0,
      );
      ...
      const avgRps =
        durationMinutes && totalRequests
          ? (totalRequests / (durationMinutes * 60)).toFixed(2)
          : "0.00";
      ...
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-muted-foreground">
            Real-time insights into your MCP infrastructure.
          </p>
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="col-span-2">
              <CardHeader>
                ...
                <CardDescription>
                  Number of failed requests over time.
                </CardDescription>
              </CardHeader>
              <CardContent className="pl-2">
                <div className="h-[300px]">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatTokenCount(contextTotal)} tokens
                </div>
                <p className="text-xs text-muted-foreground">
