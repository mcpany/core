...
describe("ToolsPage", () => {
  const mockTools = [
    ...
    {
      name: "special-tool",
      description: "A very special tool",
      serviceId: "service1-id",
      disable: false,
    },
  ];
  ...
  it("filters tools by service", async () => {
    ...
    const select = screen
      .getAllByTestId("select-native")
      .find((el) => (el as HTMLSelectElement).value === "all");
  ...
  it("groups tools by category", async () => {
    ...
    (apiClient.listTools as Mock).mockResolvedValue({
      tools: mockToolsWithTags,
    });
