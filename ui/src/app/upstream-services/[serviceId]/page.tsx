...
export default function UpstreamServiceDetailPage() {
  ...
  const fetchServiceData = async () => {
    ...
    try {
      ...
      try {
        const statusData = await apiClient.getServiceStatus(
          loadedService.name || serviceId,
        );
        setStatus(statusData);
      } catch (err) {
        console.warn(
          "Failed to fetch status (service might be down/disabled):",
          err,
        );
        setStatus(null);
      }
      ...
      toast({
        ...
        description: "Service not found or error occurred.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleUnregister = async () => {
    if (
      !confirm(
        "Are you sure you want to unregister this service? This action cannot be undone.",
      )
    )
      return;
    try {
  ...
  if (!service) {
    return (
      <div className="p-8 text-center">
        ...
        <Button variant="link" onClick={() => navigate("/upstream-services")}>
          Back to Services
        </Button>
      </div>
    );
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-background">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight flex items-center gap-3">
              {service.name}
              <Badge
                variant={service.disable ? "secondary" : "default"}
                className={
                  service.disable
      ...
      >
        ...
        <div className="flex-1 overflow-hidden bg-muted/5">
          <TabsContent
            value="overview"
            className="h-full p-6 overflow-y-auto m-0"
          >
            <ServiceOverview
              service={service}
              status={status}
