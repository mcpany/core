...
export default function ServicesPage() {
  ...
  const [configuringTemplate, setConfiguringTemplate] =
  ...
  const toggleService = useCallback(
    async (name: string, enabled: boolean) => {
      // Optimistic update
      setServices((prev) =>
        prev.map((s) => (s.name === name ? { ...s, disable: !enabled } : s)),
      );
  ...
  const bulkToggleService = useCallback(
    async (names: string[], enabled: boolean) => {
      // Optimistic update
      setServices((prev) =>
        prev.map((s) =>
          names.includes(s.name) ? { ...s, disable: !enabled } : s,
        ),
      );

      try {
        await Promise.all(
          names.map((name) => apiClient.setServiceStatus(name, !enabled)),
        );
        toast({
  ...
  const bulkDeleteService = useCallback(
    async (names: string[]) => {
      if (!confirm(`Are you sure you want to delete ${names.length} services?`))
        return;

      try {
        await Promise.all(
          names.map((name) => apiClient.unregisterService(name)),
        );
  ...
  const handleBulkEdit = useCallback(
    async (names: string[], updates: { tags?: string[] }) => {
      try {
        ...
        await Promise.all(
          servicesToUpdate.map((service) => {
            ...
            if (updates.tags) {
              updated.tags = [
                ...new Set([...(service.tags || []), ...updates.tags]),
              ];
            }
  ...
  const handleDuplicate = useCallback(
    (service: UpstreamServiceConfig) => {
      ...
      toast({
        ...
        description:
          "A copy of the service has been created. Please review and save.",
      });
    },
    [toast],
  );
  ...
  const handleLogin = useCallback(
    async (service: UpstreamServiceConfig) => {
      try {
    ...
    try {
      if (selectedService.id) {
        ...
        toast({
          ...
          description: "New service registered successfully.",
        });
      }
  ...
  const getSheetDescription = () => {
    ...
    if (configuringTemplate)
      return "Enter the required information to set up this service.";
    ...
    <div className="flex-1 space-y-4 p-8 pt-6">
      ...
      <Card className="backdrop-blur-sm bg-background/50">
        <CardHeader>
          ...
          <CardDescription>
            Manage your connected upstream services.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ServiceList
