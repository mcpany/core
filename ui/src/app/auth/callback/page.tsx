  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      try {
        ...
        const credentialID =
          sessionStorage.getItem("oauth_credential_id") || "";
        ...
        if (!serviceID && !credentialID) {
          ...
          throw new Error(
            "No service ID or credential ID found in session. Please start the flow again.",
          );
        }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-background">
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          {status === "loading" && (
            <p>Please wait while we complete the secure connection.</p>
          )}
          {status === "success" && (
