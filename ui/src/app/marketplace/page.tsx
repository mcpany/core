...
export default function MarketplacePage() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<
    UpstreamServiceConfig | undefined
  >(undefined);
  ...
  const loadData = async () => {
    try {
      const [cols, markets, templates, community, catalogServices] =
        await Promise.all([
  ...
  const handleImportCollection = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Saved ${col.name} to Local Marketplace`,
      });
  ...
  const handleWizardComplete = async (config: UpstreamServiceConfig) => {
    try {
      ...
      toast({
        title: "Config Saved",
        description: `${config.name} saved to Backend Templates.`,
      });
  ...
  const deleteTemplate = async (templateSvc: UpstreamServiceConfig) => {
    ...
    try {
      ...
      const idToDelete =
        templateSvc.templateId || templateSvc.id || templateSvc.name;
  ...
  // ⚡ BOLT: [Render/Memory] Memoized filtered community servers and hoisted string allocations to avoid O(N) operations on every render.
  // Randomized Selection from Top 5 High-Impact Targets
  const filteredCommunityServers = useMemo(() => {
    const query = searchQuery.toLowerCase();
    return communityServers.filter(
      (s) =>
              ...
              {popularServices.map((service) => (
                ...
                >
                  ...
                  <CardFooter>
                    <Button
                      className="w-full"
                      onClick={() => openInstantiate(service)}
                    >
                      Configure
                    </Button>
                  </CardFooter>
                ...
                <Card key={idx} className="flex flex-col">
                  ...
                  <CardFooter>
                    <Button
                      className="w-full"
                      variant="outline"
                      onClick={() => openCollectionDetails(col)}
                    >
                      View Details
                    </Button>
                  </CardFooter>
                ...
                <Card key={idx} className="flex flex-col border-dashed">
                  ...
                  <CardFooter className="gap-2">
                    <Button
                      className="flex-1"
                      onClick={() =>
                        col.services[0] && openInstantiate(col.services[0])
                      }
                    >
                      Instantiate
                    </Button>
                    <Button
                      ...
                      size="icon"
                      onClick={() =>
                        col.services[0] && deleteTemplate(col.services[0])
                      }
                    >
          ...
          <section>
                ...
                <Card key={idx} className="flex flex-col">
                  ...
                  <CardContent className="flex-1">
                    <div className="flex flex-wrap gap-1 mb-2">
                      ...
                      {server.tags.map((tag, i) => (
                        <span key={i} className="text-sm" title="Tag">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <CardFooter>
                    <Button
                      className="w-full"
                      variant="secondary"
                      onClick={() => openInstantiateCommunity(server)}
                    >
                      Install
                    </Button>
                  </CardFooter>
            ...
            {filteredCommunityServers.length > 100 && (
              <div className="text-center mt-8 text-muted-foreground text-sm">
                Showing first 100 of {filteredCommunityServers.length} results.
