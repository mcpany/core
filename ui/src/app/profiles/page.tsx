  ...
  const fetchProfiles = async () => {
    ...
    try {
      ...
      const mapped: Profile[] = data.map((p: any) => {
        ...
        const type =
          (allTags.find((t: string) => ["dev", "prod", "debug"].includes(t)) as
            | "dev"
            | "prod"
            | "debug") || "dev";
        const additionalTags = allTags.filter(
          (t: string) => !["dev", "prod", "debug"].includes(t),
        );

        const disabledTools: Record<string, string[]> = {};
        if (p.serviceConfig) {
          Object.entries(p.serviceConfig).forEach(
            ([svcName, cfg]: [string, any]) => {
              if (cfg.tools) {
  ...
  return (
    <div className="flex-1 space-y-4 p-8 pt-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {isLoading && (
          <div className="col-span-3 text-center p-4">Loading profiles...</div>
        )}
        ...
        {profiles.map((profile) => (
          <Card
            key={profile.id}
            className="backdrop-blur-sm bg-background/50 hover:shadow-md transition-all cursor-pointer group"
