...
const ServiceHealthContext = createContext<
  ServiceHealthContextType | undefined
>(undefined);
const TopologyContext = createContext<TopologyContextType | undefined>(
  undefined,
);
...
export function ServiceHealthProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      const url =
        typeof window !== "undefined"
          ? "/api/v1/topology"
          : "http://localhost/api/v1/topology";
  ...
  const getServiceCurrentHealth = useCallback(
    (serviceId: string) => {
...
export function useServiceHealth() {
  ...
  if (!context) {
    throw new Error(
      "useServiceHealth must be used within a ServiceHealthProvider",
    );
  }
  return context;
}
