...
export function analyzeConnectionError(error: string): DiagnosticResult {
  ...
  if (err.includes("no such host") || err.includes("name resolution failed")) {
    return {
      ...
      suggestion:
        "Check for typos in the hostname. Ensure the DNS is configured correctly within the container network.",
      severity: "critical",
    };
  }
  ...
  if (
    ...
    (err.includes("dial tcp") && !err.includes("lookup"))
  ) {
    return {
      category: "network",
  ...
  if (err.includes("timeout") || err.includes("deadline exceeded")) {
    return {
      ...
      suggestion:
        "Check firewall settings or network latency. The service might be hung or overloaded.",
      severity: "warning",
    };
  }
  ...
  if (err.includes("404") || err.includes("not found")) {
    return {
      ...
      title: "Not Found (404)",
      description:
        "The requested path or resource does not exist on the upstream server.",
      suggestion:
        "Check the URL path in your configuration. The base URL might be correct, but the endpoint might be wrong.",
      severity: "warning",
    };
  }
  ...
  if (err.includes("500") || err.includes("internal server error")) {
    return {
      ...
      description:
        "The upstream server encountered an error while processing the request.",
      suggestion:
        "Check the logs of the upstream service. It might be crashing or misconfigured.",
      severity: "critical",
    };
  }
  ...
  if (
    ...
    return {
      ...
      description:
        "The upstream server is down or unable to handle the request.",
      suggestion:
        "The service might be restarting or overloaded. Check if the upstream process is running healthy.",
      severity: "critical",
    };
  }
  ...
  if (
    ...
    return {
      ...
      suggestion:
        "Verify your API key or Bearer token. Check if the token has expired.",
      severity: "critical",
    };
  }
  ...
  if (err.includes("403") || err.includes("forbidden")) {
    return {
      ...
      suggestion:
        "Check if the token has the necessary scopes or permissions for this resource.",
      severity: "critical",
    };
  }
  ...
  if (
    ...
    (err.includes("https") && err.includes("ws://"))
  ) {
    return {
      category: "configuration",
      title: "Protocol Mismatch (Mixed Content)",
      description:
        "Attempting to connect to an insecure WebSocket (ws://) from a secure page (https://).",
      suggestion:
        "Browsers block this for security. Use 'wss://' (Secure WebSocket) or serve your dashboard over HTTP (not recommended).",
      severity: "critical",
    };
  }
  ...
  if (
    ...
    return {
      ...
      suggestion:
        "If using self-signed certificates, ensure the CA is trusted. Check if you are trying to connect via HTTPS to an HTTP port (or vice versa).",
      severity: "critical",
    };
  }
  ...
  if (err.includes("eof") || err.includes("closed connection")) {
    return {
      ...
      suggestion:
        "The server might have crashed or rejected the connection immediately. Check server logs.",
      severity: "warning",
    };
  }
  ...
  if (
    ...
    return {
      ...
      description:
        "The server does not have permission to access a requested file or resource.",
      suggestion:
