...
describe("sanitizeServiceConfig", () => {
  ...
  it("should redact secrets when mode is redact", () => {
    ...
    expect(sanitized.commandLineService?.env?.["DB_PASSWORD"]).toBe(
      "<REDACTED>",
    );
    expect(sanitized.commandLineService?.env?.["AUTH_TOKEN"]).toBe(
      "<REDACTED>",
    );
    expect(sanitized.commandLineService?.env?.["PUBLIC_VAR"]).toBe(
      "public-value",
    );
  });
  ...
  it("should keep secrets when mode is unsafe", () => {
    ...
    expect(sanitized.commandLineService?.env?.["API_KEY"]).toBe(
      "super-secret-key",
    );
    expect(sanitized.commandLineService?.env?.["DB_PASSWORD"]).toBe(
  ...
  it("should handle missing env", () => {
    const serviceNoEnv = {
      ...mockService,
      commandLineService: { command: "cmd" },
    };
