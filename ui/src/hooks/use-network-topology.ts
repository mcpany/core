...
export function useNetworkTopology() {
  ...
  const { latestTopology, refreshTopology: refreshContextTopology } =
    useTopology();
        ...
        const addNode = (tNode: TopologyNode, parentId?: string) => {
          ...
          if (parentId) {
            newEdges.push({
              ...
              label: tNode.metrics
                ? `${tNode.metrics.qps?.toFixed(1) || 0} RPS`
                : undefined,
          ...
          if (tNode.children && tNode.children.length > 0) {
            const limit =
              tNode.type === "NODE_TYPE_CORE" ||
              tNode.type === "NODE_TYPE_SERVICE"
                ? 100
                : 100; // Disable limit for now to see all
            tNode.children
              .slice(0, limit)
              .forEach((child) => addNode(child, tNode.id));
          }
        };
