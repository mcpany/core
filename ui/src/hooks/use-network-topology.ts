...
export function useNetworkTopology() {
    ...
    const processGraph = useCallback((graph: Graph) => {
        try {
            ...
            // ⚡ BOLT: Optimized structure comparison to avoid expensive string join/hash.
            // Randomized Selection from Top 5 High-Impact Targets

            // Sort new elements by ID to ensure stable layout and consistent comparison
            ...
            if (!structureChanged && nodesRef.current.length > 0) {
                ...
                // Note: nodesRef.current might be unsorted depending on setNodes behavior, but we use a Map lookup.
