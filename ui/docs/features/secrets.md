...
**YAML Configuration:** Use the `${secrets.KEY_NAME}` syntax in your
`config.yaml` or `config.json` files.

**UI Configuration:** The UI currently treats manual input as plain text. To use
secrets in the UI, ensure they are loaded from the configuration file, or use
the "Secrets" management interface to view existing secret references.
