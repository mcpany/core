...
import { seedToolsService, cleanupToolsService, seedUser, cleanupUser } from './e2e/test-data';
...
test.describe('Tool Exploration', () => {
    ...
    test.afterEach(async ({ request }) => {
        await cleanupToolsService(request);
    ...
    test('should list available tools from real backend', async ({ page }) => {
        ...
        for (let i = 0; i < 10; i++) {
            try {
                ...
                await expect(page.getByText('process_payment_tools').first()).toBeVisible({ timeout: 5000 });
                found = true;
                break;
            } catch (e) {
        ...
        await expect(page.getByText('process_payment_tools').first()).toBeVisible({ timeout: 10000 });
        ...
        try {
            await expect(page.getByText(/echo_tool_test/).first()).toBeVisible({ timeout: 20000 });
        } catch (e) {
            console.log('Echo tool not found. Page content:', await page.content());
            throw e;
        }
        await expect(page.getByText('Echoes back input (tools test)').first()).toBeVisible({ timeout: 20000 });
    });
    ...
    test('should allow inspecting a tool', async ({ page }) => {
        ...
        const toolRow = page.locator('tr').filter({ hasText: /echo_tool_test/ });
        await toolRow.getByRole('button', { name: 'Inspect' }).click();

        await expect(page.getByText('Echoes back input (tools test)').first()).toBeVisible();
