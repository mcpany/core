...
test.describe('Tool Exploration', () => {
    ...
    test('should list available tools from real backend', async ({ page }) => {
        ...
        for (let i = 0; i < 20; i++) {
            try {
                // Check for weather-service first as it is more stable in CI than external Payment Gateway
                ...
                await page.waitForTimeout(4000); // Increased wait time significantly
            }
        }

        // Verify weather tool is visible (this is our primary readiness check)
    ...
    test('should execute a tool and show results', async ({ page }) => {
        ...
        await textArea.fill('{"city": "Paris"}');
