...
test.describe('Tool Exploration', () => {
    ...
    test.beforeEach(async ({ request, page }) => {
        // Use a unique ID suffix to avoid conflict with parallel tests using svc_01
        await seedServices(request, "_tools_test");
    ...
    test.afterEach(async ({ request }) => {
        await cleanupServices(request, "_tools_test");
    ...
    test('should list available tools from real backend', async ({ page }) => {
        ...
        for (let i = 0; i < 20; i++) {
            try {
                ...
                // Use regex to be robust against namespacing (e.g. "Payment Gateway.process_payment")
