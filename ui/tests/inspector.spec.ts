...
test.describe('Inspector Page', () => {
  test('should allow seeding a trace from backend and viewing it', async ({ page }) => {
    ...
    // The seeded trace's root span name should appear in the inspector table.
    // The backend `generateMockAuditEntries()` sets the root tool name to "orchestrator-task".
