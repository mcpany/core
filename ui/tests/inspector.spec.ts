...
test.describe("Inspector Page", () => {
  test("should allow seeding a trace from backend and viewing it", async ({
    page,
  }) => {
    ...
    let wsSendFunc: ((data: string) => void) | null = null;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await page.routeWebSocket("**/api/v1/ws/traces", (ws: any) => {
      wsSendFunc = (data: string) => ws.send(data);
    });
    ...
    // connection.
    if (wsSendFunc) {
      (wsSendFunc as (data: string) => void)(JSON.stringify(MOCK_TRACE));
    }
