...
test.describe('Skills Management', () => {
    ...
    test('should load skills page with empty state', async ({ page }) => {
        ...
        await expect(page.getByText('No skills found')).toBeVisible({ timeout: 10000 });
        await expect(page.getByRole('button', { name: 'Create Skill' })).toBeVisible();
    });

    test('should delete a skill via custom alert dialog', async ({ page, request }) => {
        ...
        await expect(page.getByText(testSkillName).first()).toBeVisible({ timeout: 10000 });

        // Click the delete button on the card
        const trashButton = page.locator('.backdrop-blur-sm').filter({ hasText: testSkillName }).locator('.text-destructive');
        await trashButton.click();

        // Verify custom alert dialog appears
        await expect(page.getByRole('alertdialog')).toBeVisible();
        await expect(page.getByRole('heading', { name: 'Delete Skill' })).toBeVisible();
        await expect(page.getByText(`Are you sure you want to delete the skill "${testSkillName}"?`)).toBeVisible();

        // Confirm deletion
        await page.getByRole('button', { name: 'Delete' }).click();

        // Verify success toast and skill removal
        await expect(page.getByText('Skill deleted')).toBeVisible({ timeout: 5000 });
        await expect(page.getByText(testSkillName)).not.toBeVisible({ timeout: 5000 });
    });
});
