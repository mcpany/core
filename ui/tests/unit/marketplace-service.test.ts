...
- [cloudflare/mcp-server-cloudflare](https://github.com/cloudflare/mcp-server-cloudflare) 🎖️ 📇 ☁️ - Integration with Cloudflare...
    ...
    it('should filter out unsafe URLs', async () => {
      const mockMarkdown = `
...
- [Unsafe](javascript:alert(1)) - Unsafe
      ...
      expect(servers[0].name).toBe('Safe');
    });
  });

  describe('importCollection', () => {
    it('should reject unsafe URLs', async () => {
      await expect(marketplaceService.importCollection('javascript:alert(1)')).rejects.toThrow('Invalid URL');
    });

    it('should accept safe URLs', async () => {
      const result = await marketplaceService.importCollection('https://example.com/collection.json');
