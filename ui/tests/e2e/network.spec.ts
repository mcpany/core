...
test.describe("Network Topology", () => {
  ...
  test("should display network topology nodes", async ({ page }) => {
    ...
    await expect(
      page.locator(".text-lg", { hasText: "Network Graph" }),
    ).toBeVisible();
    ...
    await expect(
      page
        .locator(".react-flow")
        .getByText(/MCP Any|Core/i)
        .first(),
    ).toBeVisible({ timeout: 10000 });
    await expect(
      page.locator(".react-flow").getByText("Payment Gateway").first(),
    ).toBeVisible();
    await expect(
      page.locator(".react-flow").getByText("User Service").first(),
    ).toBeVisible();
  ...
  test("should filter nodes", async ({ page }) => {
    ...
    if (await filterBtn.isVisible()) {
      ...
      const serviceToggle = page
        ...
        .filter({ hasText: /Services|Nodes/i });
      if ((await serviceToggle.count()) > 0) {
        ...
        console.log(
          "Filter options not found, skipping specific filter interaction",
        );
      }
    } else {
