...
import { seedUser, cleanupUser, seedComplexTool } from './test-data';

test.describe('Tool Wizard E2E', () => {
    test.describe.configure({ mode: 'serial' });

    test.beforeEach(async ({ request, page }) => {
        await seedComplexTool(request);
    ...
    test('Should fill out tool wizard and execute', async ({ page }) => {
        ...
        // Filter by the specific service seeded dynamically
        await expect(page.locator('tr', { hasText: 'Complex Tool Service Dynamic' })).toBeVisible();

        // Open Inspector
        await page.locator('tr', { hasText: 'Complex Tool Service Dynamic' }).getByRole('button', { name: 'Inspect' }).click();
