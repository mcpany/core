...
export const seedComplexTool = async (requestContext?: APIRequestContext) => {
    const context = requestContext || await request.newContext({ baseURL: BASE_URL });
    const service = {
        id: "complex-tool-service-dynamic",
        name: "Complex Tool Service Dynamic",
        version: "v1.0.0",
        command_line_service: {
            ...
            tools: [
                {
                    name: "create_user",
                    description: "Create a user with detailed attributes",
                    input_schema: {
                        type: "object",
                        properties: {
                    ...
                    call_id: "create_user_call"
                }
            ],
            calls: {
                create_user_call: {
                    args: ["User: {{username}} Age: {{age}} Role: {{role}} Active: {{isActive}} Tags: {{tags}} Address: {{address}}"]
                }
            }
        }
    };
    try {
        ...
        if (!res.ok()) {
             console.log(`Failed to seed complex service: ${res.status()} ${await res.text()}`);
        }
    } catch (e) {
        console.log(`Failed to seed complex service: ${e}`);
    }
};
