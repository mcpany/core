...
export const seedToolsTestServices = async (requestContext?: APIRequestContext) => {
    ...
    const services = [
        {
            ...
            name: `Payment Gateway Tools ${suffix}`,
            version: "v1.2.0",
            http_service: {
                address: "https://stripe.com",
                tools: [
                    { name: `process_payment_tool_${suffix}`, description: "Process a payment" }
                ]
            }
        },
        {
            id: `svc_tool_echo_${suffix}`,
            name: `Echo Service Tools ${suffix}`,
            version: "v1.0",
            command_line_service: {
                command: "/bin/echo",
                tools: [
                    {
                        name: `echo_tool_unique_${suffix}`,
                        description: "Echoes back input",
                        inputSchema: { type: "object" },
                        call_id: "echo_call"
                    }
    ...
    return { suffix, services };
};

export const cleanupToolsTestServices = async (requestContext: APIRequestContext | undefined, suffix: string) => {
    const context = requestContext || await request.newContext({ baseURL: BASE_URL });
    try {
        // Use Names for deletion as per API behavior (or check if ID works, but previous code used Name)
        // Ensure we match the exact names generated in seed
        await context.delete(`/api/v1/services/Payment Gateway Tools ${suffix}`, { headers: HEADERS });
        await context.delete(`/api/v1/services/Echo Service Tools ${suffix}`, { headers: HEADERS });
    } catch (e) {
