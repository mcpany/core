...
export const seedServices = async (requestContext?: APIRequestContext) => {
    ...
    const services = [
        ...
        },
        {
            ...
            version: "v1.0",
            http_service: {
                address: "http://127.0.0.1:50051", // Dummy address, using IP to be explicit
                tools: [
        ...
        // Add a service with calculator for existing test compatibility if desired
        {
            ...
            version: "v1.0",
            http_service: {
                // Use a non-loopback private IP (from docker compose service) to potentially bypass loopback check if enabled
                // But since it's a dummy, we just want it to resolve.
                // In CI, 'ui-http-echo-server' exists.
