...
test.describe("Services Feature", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route(
      (url) => url.pathname.includes("/api/v1/services"),
      async (route) => {
        const method = route.request().method();
        const urlStr = route.request().url();
        if (method === "GET" && urlStr.includes("%20Gateway")) {
          return await route.fulfill({ json: services[0] });
        }
        if (method === "GET") {
  ...
  test("should list services, allow toggle, and manage services", async ({
    page,
  }) => {
  ...
  test("should render schema visualizer in service tools dialog", async ({
    page,
  }) => {
    const paymentRow = page
      .locator("tr")
    ...
    await paymentRow.getByRole("link").click();

    // Tools are now in the General tab by default
    await page.getByRole("tab", { name: /Tools/ }).click();
    await expect(
      page.getByText("Tools", { exact: true }).first(),
    ).toBeVisible();

    // Should render service detail content
    await expect(page.locator("main").first()).toContainText("process_payment");

    // Click View Schema button
    await page.locator('button[title="View Schema"]').first().click();
    ...
    await expect(
      page.getByRole("dialog").getByRole("columnheader", { name: "Property" }),
    ).toBeVisible();
    await expect(
      page.getByRole("dialog").getByRole("columnheader", { name: "Type" }),
    ).toBeVisible();

    // Should see the properties we defined
    await expect(
      page.getByRole("dialog").getByText("amount", { exact: false }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("dialog").getByText("currency", { exact: false }).first(),
    ).toBeVisible();
    await expect(
      page
        .getByRole("dialog")
        .locator("td")
        .filter({ hasText: "Payment amount in cents" })
        .first(),
    ).toBeVisible();
  });
