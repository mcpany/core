...
test.describe("Global Search", () => {
  ...
  test("should open command palette by clicking the search button", async ({
    page,
  }) => {
  ...
  test("should open command palette via shortcut and search dynamic content", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("option", { name: /Dashboard/i }).first(),
    ).toBeVisible();

    // Also verify we can find the "Services" category or items
    await searchInput.fill("Service");
    await expect(
      page.getByRole("option", { name: /Services/i }).first(),
    ).toBeVisible();
