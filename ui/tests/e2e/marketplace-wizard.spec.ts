...
test.describe('Marketplace Wizard and Service Lifecycle', () => {
  ...
  test('Complete CUJ: Create Config -> Instantiate -> Manage', async ({ page }) => {
    ...
    // 4. Step 2: Configuration (Schema Form)
    await expect(page.getByRole('heading', { name: 'Configuration' })).toBeVisible();

    // Check for parameter input existence and edit it using label
    // Schema form renders labeled inputs
    ...
    // Note: Schema form does not support adding arbitrary parameters via "Add Parameter" button
    // So we skip that part for Postgres template.
