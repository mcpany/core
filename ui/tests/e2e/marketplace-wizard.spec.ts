...
test.describe("Marketplace Wizard and Service Lifecycle", () => {
  ...
  test("Complete CUJ: Create Config -> Instantiate -> Manage", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("dialog", { name: "Create Upstream Service" }),
    ).toBeVisible();
    ...
    await expect(paramInput).toHaveValue(
    ...
    await page
      .locator('input[placeholder="https://api.example.com/webhook"]')
      .first()
    ...
    await expect(
      page.getByRole("option", { name: "Test Credential" }),
    ...
    await expect(
      page.getByText("Connection verification successful"),
    ...
    await expect(
      page.getByText("postgresql://test:test@localhost:5432/testdb"),
    ).toBeVisible();
    ...
    // 9. Instantiate
    await expect(
      page.getByRole("button", { name: "Instantiate" }).first(),
    ).toBeVisible();
    await page.getByRole("button", { name: "Instantiate" }).first().click();

    await expect(
      page.getByRole("dialog", { name: "Instantiate Service" }),
    ).toBeVisible();
