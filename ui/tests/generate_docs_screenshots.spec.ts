...
test.describe('Generate Detailed Docs Screenshots', () => {
  ...
  test('Services Screenshots', async ({ page }) => {
    ...
    // Use exact: false for label matching to be robust
    // SKIPPED: This selector is flaky in CI environment (timeout)
    // await page.getByLabel('Address / URL', { exact: false }).waitFor({ state: 'visible', timeout: 10000 });

    // Diff Feature Screenshot - SKIPPED (Flaky in CI)
  ...
  test('Credentials Screenshots', async ({ page }) => {
    ...
    // Verification Screenshot (Test Connection) - SKIPPED (Flaky in CI)
