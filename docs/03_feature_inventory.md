...
### Proposed Additions
- **JIT Capability Negotiation Middleware**: (P0) Allows agents to request and receive granular permission escalations during a session without full state reset.
- **Ephemeral Tool Registry (In-Memory)**: (P0) A high-performance, transient registry for task-scoped tools generated by agent swarms.
- **Fair-Share MCP Scheduler**: (P1) Multiplexer for shared MCP servers (stdio/http) to prevent resource starvation in multi-agent environments.

### Priority Shifts
- **A2A Stateful Residency**: Promoted from **P0** to **Core Pillar**. It is becoming the bedrock of the Federated Agent Mesh.
- **Lazy-MCP Tool Search**: Re-affirmed as **P0** to handle the influx of ephemeral tools.

### Deprecations / Monitoring
- **Blocking Config Reloads**: Monitor for replacement by the Ephemeral Registry for all non-persistent tool changes.
