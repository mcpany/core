...
### Proposed Additions
- **Memory-Injection Shield (MIS)**: (P0) Authoritative semantic validation service for detecting and blocking "Belief-Corruption" payloads in long-term memory.
- **Native Sandbox Adapter (NSA)**: (P0) Unified interface for kernel-level tool isolation using macOS Seatbelt, Windows Sandboxing, and Linux-based mechanisms.
- **Identity-Bound Memory Attribution**: (P0) Identity middleware cryptographically tagging all memory fragments with their origin to neutralize sleeper-agent attacks.
- **Semantic Consistency Heartbeats**: (P1) Lifecycle monitoring service periodically verifying an agent's belief state against the mission-root manifest.

### Priority Shifts
- **Blackboard Versioning Hub**: (Re-affirmed P0) Elevated with mandatory support for **Identity-Bound** attribution.
- **Structural Metadata Sanitizer**: (Re-affirmed P0) Designated as a critical dependency for **MIS** to prevent schema-based memory poisoning.
