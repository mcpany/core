...
### Proposed Additions
- **Streaming Reason-Chain Attestation (SRCA) Provider**: (P0) Core security service facilitating real-time, hardware-bound (TPM) signatures for every reasoning fragment as it is generated by the LLM.
- **Context-Binding Identity (CBI) Middleware**: (P0) Security middleware that cryptographically binds identity tokens to specific reasoning fragments and hardware-attested Attention Anchors, neutralizing ToxicSkills.
- **Provenance Interop Bridge (PIB)**: (P1) Translation layer for verifying and propagating Gemini-style `x-gemini-provenance` headers across heterogeneous framework boundaries.
- **Attention-Density Sharding (ADS) Manager**: (P1) Resource management service for mailbox shards that prioritizes security-critical intent fragments during context-window congestion.

### Priority Shifts
- **Reasoning Provenance Validator**: (Re-affirmed P0) Now elevated with the requirement for mandatory **SRCA** integration for real-time validation.
- **Context-File Integrity Attestation (CFIA)**: (Re-affirmed P0) Designated as a critical dependency for providing the **Attention Anchors** required by the CBI middleware.
