...
### Proposed Additions
- **Skill Attestation Registry (SAR) Client**: (P0) A client for fetching and verifying tool behavioral contracts against a global registry.
- **Sidecar Execution Engine**: (P0) Pluggable runtime (WASM/Docker-lite) for isolating tool execution from the host gateway.
- **Intent-Scoped Policy Engine**: (P1) Middleware that evaluates tool calls against a "High-Level Intent" token generated by parent agents.

### Priority Shifts
- **Safe-by-Default Hardening**: Re-affirmed as **P0**. Urgent due to CVE-2026-25253 (Leaking local gateway tokens).
- **Provenance-First Discovery**: Promoted from **P1** to **P0**. Essential for preventing "Skill-Squatting" attacks.

### Deprecations / Monitoring
- **Direct Local Execution**: Flagging for future deprecation in favor of Sidecar Execution for all third-party (unattested) tools.
