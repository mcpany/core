...
- **Recursive Depth-Limit Enforcer (RDLE)**: (P0) Security middleware that
  recursively tracks and enforces hardware-attested handoff limits across
  deep swarms.
- **Reasoning-Path Watermarking Provider**: (P1) Identity service implementing
  HAIL-compliant watermarking for every reasoning fragment produced within a
  mission.

### Priority Shifts
- **Active Intent Alignment (AIA) Broker**: (Re-affirmed P0) Now elevated with
  the requirement for mandatory **ACR-compliant** revocation signals.
- **Recursive Accountability Tracker (RAT)**: (Re-affirmed P0) Designated as the
  primary infrastructure for the new **RDLE** depth-limit enforcement.
