...
### Proposed Additions
- **Intent-Aware Routing Channels**: (P0) Dedicated, high-reliability communication paths for sub-agent coordination to prevent message drift.
- **Proactive Sandbox Hardening**: (P0) Mandatory read-only protections for critical project-local configuration paths, enforced even if files are missing at startup (CVE-2026-25725).
- **Agentic PR Security Scanner**: (P1) Integrated static analysis middleware to detect common vulnerabilities in code generated by agents before repository ingestion.

### Priority Shifts
- **Relational PoI Validator**: Re-affirmed as **P0**. Critical for preventing Intent-Chain hijacking.
- **Ghost Shell Hook Profiler**: Re-affirmed as **P0**. Essential for behavioral attestation of configuration hooks.
