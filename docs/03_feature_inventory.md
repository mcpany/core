...
### Proposed Additions
- **Cross-Node Lease Orchestrator (CNLO)**: (P0) Authoritative lease synchronization service that mirrors TPM-signed mission capabilities across distributed physical nodes to neutralize attestation silos.
- **Epistemic Quorum Gating (EQG) Middleware**: (P1) Security extension that monitors hardware-attested uncertainty headers and escalates tool calls based on real-time reasoning confidence scores.
- **Instruction-Anchor Reinforcement (IAR) Service**: (P0) Advanced cognitive security service that dynamically injects attention-reinforcement tokens to protect mission-root anchors from eviction.

### Priority Shifts
- **Fast-Path Mesh Resumption**: (Re-affirmed P0) Elevated to v2 with mandatory support for **Mission-Lineage Tokens** to neutralize recursive tunnel handshake storms.
- **Reasoning Confidence Scoring (RCS) Gateway**: (Re-affirmed P0) Designated as the primary signal provider for the new **EQG** gating logic.
