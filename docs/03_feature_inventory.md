...
### Proposed Additions
- **Identity-Aware Tool Policy Engine**: (P0) Scopes tool permissions to specific agent identities/roles within a swarm to prevent cross-agent privilege escalation.
- **Containerized Skill Execution (Ephemeral Sandboxing)**: (P0) Native support for running MCP servers inside ephemeral Docker/Podman containers to isolate them from the host filesystem.
- **Swarm Peer-Review Protocol**: (P1) A multi-agent attestation flow where high-risk actions require cryptographic consent from multiple "Peer Agents" before execution.

### Priority Shifts
- **Detached Sandbox for Automated Hooks**: Promoted to **P0**. Essential for mitigating RCE via poisoned skills and project-local hooks.
- **Supply Chain Integrity Guard**: Re-affirmed as **P0**. Critical for validating community-contributed "Skills."
