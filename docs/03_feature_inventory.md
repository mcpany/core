...
### Proposed Additions
- **Tool-Specific Egress Firewall**: (P0) Network interception layer for all MCP tool calls to prevent SSRF and unauthorized data exfiltration.
- **MCP Attestation Registry (Local)**: (P0) Local database of cryptographic hashes for all allowed MCP servers and tool definitions.
- **Binary Sidecar Manager**: (P1) Secure management of ephemeral binary assets generated by tools (PDFs, Audio), mapped to agent sessions.

### Priority Shifts
- **Project Configuration Security Guard**: Re-affirmed as **P0** as it is the first line of defense against the "ClawHavoc" style local exploits.
- **Provenance-First Discovery**: Promoted from **P1** to **P0** due to the massive ClawHub compromise.

### Deprecations / Monitoring
- **Unsandboxed Network Access for Tools**: All tools without a declared network manifest will be moved to a "Deny-All" network policy by default.
