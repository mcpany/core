...
logic. It utilizes the Multi-Modal Behavioral Attestation (MMBA) signatures
to perform "Semantic Importance Scoring," ensuring that context fragments
from high-trust teammates are preserved during aggressive sharding.
**Impact:** Dramatically improves the stability of horizontal meshes by
ensuring critical teammate state is not accidentally "ghosted" during
high-entropy reasoning phases.
...
**Impact:** Prevents "Reasoning Hijacking" where a subagent attempts to
inject its own logic into the parent's reasoning stream by making every
fragment non-repudiable and lineage-aware.
...
that is cryptographically bound to the mission-root manifest, rather than
