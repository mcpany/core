...
### Update: 2026-03-12 - Defending Against Cross-Site WebSocket Hijacking (CSWSH)
**Context**: CVE-2026-25253 (OpenClaw) highlighted how agents listening on `localhost` can be hijacked via malicious URL parameters in the browser.
**Architecture Adjustment**:
* **Origin-Locked WebSocket Proxy**: The `File Proxy Middleware` (Section 4) now enforces strict `Origin` header validation for all WebSocket connections initiated by or directed to an agent. Any connection with a non-allowlisted origin is immediately terminated.
* **CSWSH Pattern Scanning**: The `Config Validator` now scans project-local settings for known "redirect-to-websocket" patterns or insecure local-host bindings that could facilitate hijacking.
* **MFA for Local Control**: Any browser-based interaction with the agent's control interface (even on `localhost`) now requires a one-time "Attestation Token" generated by the MCP Any CLI, preventing "One-Click" hijacking.
