...
## 1. Context and Scope
The OpenClaw security crisis (CVE-2026-25253) has demonstrated that the "Local Trust" modelwhere localhost connections are implicitly trustedis a critical vulnerability. Malicious websites can use JavaScript to initiate cross-site requests (CSWSH/CSRF) to local agent gateways, stealing authentication tokens and achieving RCE. MCP Any must implement a mandatory Same-Origin Policy (SOP) enforcement layer for all its local listeners.
