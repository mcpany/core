...
leading to "Delegation Escape" where subagents can recruit unauthorized
peers. The Recursive Depth-Limit Enforcer (RDLE) ensures that all
delegations are cryptographically bound to the mission-root manifest,
providing non-bypassable recursive depth control.
...
* **System Flow:** UACO Bid -> RDLE Validator -> Mission-Root Manifest Check
  -> Delegation Grant.
* **APIs / Interfaces:** `POST /api/v1/uaco/bid/validate` requiring a
  signed mission-root manifest and depth-token.
...
* **Observability:** All blocked delegations are logged for "Depth-
  Exhaustion" analysis.
