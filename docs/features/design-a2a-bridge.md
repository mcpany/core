...
*   **2026-02-27:** Integrating Agent Identity Provider (AIDP).
    *   **Context**: Today's market sync revealed that anonymous A2A handoffs are the primary vector for "Identity Hijacking" in federated meshes.
    *   **Architecture Adjustment**: Every A2A handoff now *requires* an AID token generated by the AIDP. The `A2ABridgeMiddleware` will automatically inject these tokens into outbound A2A headers.
    *   **Security Impact**: Ensures that the receiving agent can cryptographically verify the origin and "intent-scope" of the calling agent, mitigating rogue agent impersonation.
