...
* **2026-06-30:** Evolving to the **SCS-Native Adapter Hub**. This update implements support for the GA'd **Sovereign Context Sidecar (SCS) v1.0** standard. It mandates **"Intent-Bound Retrieval,"** where context sidecars will only serve fragments that match a cryptographically signed "Active Intent Token" from the verified mission root, neutralizing the risk of cross-session context smuggling.
