...
### Update: 2026-07-25 - Nested Lease Attestation (NLA)
**Context:** Today's market sync revealed "Lease-Chain Exhaustion" events in deep swarms, necessitating a standard for recursive delegation that maintains mission root sovereignty.
**Architecture Adjustment:**
* Introducing NLA (Nested Lease Attestation) middleware.
* Every sub-lease (HLML) generated by a subagent must carry a "Lineage Proof" that it is a mathematical subset of its parent lease.
* The MLE Gateway now performs recursive scope validation for all nested capability claims, ensuring they cannot exceed the primary mission manifest (HAMM).
**Security Impact:** Prevents "Lease-Chain Hijacking" where a specialist subagent could attempt to spawn a sibling with more permissions than it was originally granted.
