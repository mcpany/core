...
*   **2026-03-02:** **Update: Origin-Aware Execution (CORV)**
    - **Context**: The OpenClaw hijacking vulnerability revealed that "Local-Only" binding is not enough if a malicious website can trigger local tool calls via ambient browser context.
    - **Architecture Adjustment**: Implementing **Cross-Origin Request Verification (CORV)**. Every tool execution request must now include a cryptographically signed `OriginToken` generated by the trusted host application (e.g., the IDE or the developer's shell).
    - **Security Impact**: Prevents "Ambient Hijacking" by ensuring that only explicitly authorized local applications can trigger tool execution via MCP Any.
