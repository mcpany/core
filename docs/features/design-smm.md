...
### Update: 2026-06-18 - Thwarting Context Ghosting via ACP-Bound Stylometry
**Context:** Today's market sync revealed the "T2T Context Ghosting" vulnerability where specialist teammates inject unauthorized instructions into shared mailboxes.
**Architecture Adjustment:** * Integrating stylometric verification with the **ACP-Native Messaging Bridge**.
* Mandating that all translated ACP messages carry a "Stylometric Attestation" header.
**Security Impact:** Prevents subagents from using ACP as a "laundry" for shadowed intents, ensuring that translated instructions remain behaviorally anchored to their original parent.
