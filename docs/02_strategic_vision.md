...
### Focus: Local Gateway Isolation & Cross-Origin Security
**Context**: The "OpenClaw WebSocket Hijack" incident has revealed that local-only binding is insufficient if browser-based attacks can bridge the gap via cross-origin requests. Simultaneously, the rise of "Shadow AI" necessitates better administrative visibility into agent deployments.
**Strategic Pivot**:
- **Cross-Origin Lockdown**: MCP Any will implement strict WebSocket Origin enforcement and mandatory "Pre-Flight Attestation" for all local connections, preventing browser-initiated hijacking.
- **Inventory & Governance (Anti-Shadow AI)**: Introducing a "Discovery Beacon" that allows authorized organizational scanners to inventory local MCP Any instances and their granted capabilities.
- **Vetted Plugin Mesh**: Transitioning the marketplace model from "User-Rated" to "Attestation-First," where code provenance is cryptographically verified before execution.
