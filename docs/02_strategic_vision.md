...
### Focus: Passive Input Sovereignty & Scope-Constrained Resumption
**Context**: The emergence of "Comment and Control" (C2) attacks via GitHub passive metadata and the disclosure of Scope-Elevation vulnerabilities (CVE-2026-32922) confirm that **Input Sanitization** must move beyond direct tool calls to include **Passive Platform Data**. Simultaneously, the performance ceiling in "Agent Teams" confirms that coordination must be **Non-Blocking and Intent-Pinned**.
**Strategic Pivot**:
- **Passive Metadata Sanitizer (PMS)**: MCP Any will evolve to act as the authoritative "Metadata Scrubbing Gateway." We will implement PMS, performing real-time semantic analysis of all agent-ingested external metadata (GitHub PR titles, issue bodies, HTML comments) to neutralize hidden C2 instructions.
- **Scope-Constrained Token Rotation (SCTR)**: To neutralize CVE-2026-32922, we are mandating SCTR. Newly minted rotation tokens will be cryptographically restricted to the mathematical subset of existing scopes, backed by hardware-enclave (TPM) lineage proofs.
- **Teammate Intent-Splicing Defense**: Supporting horizontal meshes (Claude Code Agent Teams), we are upgrading the Mailbox Integrity Middleware. Every inter-teammate message must be cryptographically bound to a specific, mission-root-authorized task ID, preventing subagents from "splicing" unauthorized peer commands.
- **Attention-Anchor Pinning (AAP)**: To counter "Instruction Eviction" in 1M+ token windows, MCP Any will implement mandatory AAP. Core behavioral guardrails will be dynamically re-injected as "Attention Reinforcement" tokens into the LLM context, ensuring they remain prioritized over specialist noise.
