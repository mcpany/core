...
### Focus: Ephemeral Mesh Identity & Output-Linked Attestation
**Context**: The disclosure of the "Shadow-Tunnel" exploit in OpenClaw and "Lease-Squatting" in Claude Code confirms that session-bound hardware attestation is no longer a sufficient security boundary. We must move to **Ephemeral Mesh Identity (EMI)**—rotating identity tokens per tool-call—to eliminate the replay window for P2P tunnels. Simultaneously, the discovery of "Z-Proof Poisoning" (context-grafting) demands that privacy-preserving proofs move from "Path-Linked" to **Output-Linked Attestation**, ensuring that ZK-proofs are cryptographically bound to the semantic output of the agent's action.
**Strategic Pivot**:
- **Ephemeral Mesh Identity (EMI) Provider**: MCP Any will evolve to act as a high-frequency identity mint. We will implement EMI, issuing hardware-attested tokens that are valid for a single tool-call and bound to a monotonic counter, neutralizing tunnel hijacking and session reuse.
- **Entropy-Aware Budgeting (EAB)**: To neutralize "Lease-Squatting" during self-correction, we are introducing EAB. This layer will monitor the semantic progress vs. reasoning entropy of an agent; sessions that fail to converge while holding high-privilege hardware leases will be forcefully reclaimed.
- **Output-Linked Proof (OLP) Hub**: We are upgrading the PPA Hub to support OLP. Every ZK-proof generated for reasoning integrity must now include a cryptographic hash of the resulting tool output, preventing "Context-Grafting" where valid proofs are spliced onto unauthorized results.
- **Per-Call Monotonic Handshaking**: We are mandating monotonic anchoring for all inter-node coordination fragments. Every message in the P2P mesh will be cryptographically linked to its predecessor and the hardware-attested mission root, ensuring absolute continuity.
