...
### Focus: Active Attention Governance (AAG) & Identity Sovereignty
**Context**: The emergence of "Mailbox Shadowing" (CVE-2026-61011) and the rise of "Attention-Window Flooding" attacks (Gemini CLI v0.42.0) reveal that the security of a swarm now depends on the **absolute sovereignty of the focus** and the **deterministic rotation of identity**. As subagents become more parallel and persistent, infrastructure must move beyond simple isolation to active **Attention Locking** and **Hardware-Attested Identity Rotation (HAIR)**.
**Strategic Pivot**:
- **Hardware-Attested Attention (HAA) Bridge**: MCP Any will evolve to act as the authoritative "Attention Guard." We will implement the HAA Bridge, utilizing hardware-bound (TPM) headers to cryptographically "lock" mission-critical intent fragments at the LLM attention layer, neutralizing "Attention-Window Flooding" by rogue subagents.
- **Hardware-Attested Identity Rotation (HAIR)**: To counter "Mailbox Shadowing" in horizontal swarms, we are mandating HAIR. Every inter-teammate task claim must be signed with a hardware-attested, session-bound identity token that rotates periodically, ensuring that subagents cannot inherit unauthorized mailbox access from previous handoffs.
- **Temporal Shard Jitter (TSJ) Injection**: Supporting OpenClaw v3.1.3, we are upgrading the Entangled State Broker (ESB) to support TSJ Injection. This layer will inject hardware-attested timing jitter into state synchronization for non-primary mission-root requests, ensuring absolute temporal isolation and neutralizing cache-timing side-channels.
- **Cross-Mission Budget Continuity (CMBC)**: To support long-running missions, we are evolving the Reasoning-Budget Firewall (RBF) to include CMBC. This allows reasoning-effort and token budgets to persist and be reconciled across multiple mission phases and framework-neutral handoffs, ensuring continuous economic accountability.
