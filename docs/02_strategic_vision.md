...
## Strategic Evolution: [2026-03-27]
### Focus: Process-Control Sanitization & Mesh Lineage Observability
**Context**: The disclosure of OpenClaw's "Shell Fallback" (CVE-2026-32000) and environment variable injection (CVE-2026-22177) proves that the boundary between configuration and execution remains porous. Simultaneously, Claude Code's move toward structured hook events confirms that "Mesh Observability" is the primary operational hurdle for production swarms. We must now protect the **process environment** itself and provide **structured traceability** for every spawned subagent.
**Strategic Pivot**:
- **Mandatory Non-Shell Spawning**: MCP Any will strictly prohibit shell-fallback mechanisms in its execution adapters. All subprocesses must be launched via direct `exec` with no intermediate shell, ensuring that `EINVAL` or `ENOENT` failures do not trigger an un-sanitized shell execution path.
- **Process-Control Environment Sanitization**: We are introducing a "Process Boundary Guard." This middleware will automatically scrub process-control variables (e.g., `NODE_OPTIONS`, `LD_PRELOAD`, `PYTHONPATH`) from all project-local configurations, neutralizing "Squatting" attacks that target the agent runtime.
- **Structured Mesh Lineage**: To support swarm observability, we are mandating structured `agent_id` and `mission_id` headers for all inter-framework coordination. MCP Any will act as the authoritative "Lineage Broker," providing a standardized, machine-readable trace of the reasoning mesh.
- **Swarm Compliance Auditing**: MCP Any will evolve to act as a "Compliance Auditor" for the agentic supply chain. We will implement real-time framework versioning checks and SBOM generation for all connected swarms, proactively flagging framework-level vulnerabilities before they are exploited.
