...
### Focus: Teammate Identity Sovereignty & Memory Integrity
**Context**: The disclosure of teammate identity gaps in Claude Code (Issue #24505) and the rise of "Inbox-based Social Engineering" confirm that transport isolation is insufficient for horizontal swarms. Simultaneously, Lakera AI's research on "Memory Injection" (Sleeper Agents) reveals that the long-term state of an agent is now a primary attack vector.
**Strategic Pivot**:
- **Teammate-Aware Safety Hooks (TASH)**: MCP Any will evolve to mandate the inclusion of hardware-attested teammate identity in all safety hook payloads. This allows for differentiated policies (e.g., Researcher vs. Implementer) and prevents the "uniform hook" vulnerability.
- **Inbox Integrity Attestation (IIA)**: To neutralize coordination hijacking, we are mandating IIA. Every inter-teammate coordination message must be cryptographically signed and validated against the sender's mission-root authority before reaching the recipient's mailbox.
- **Memory Integrity Verification (MIV)**: Supporting the stability of long-running agents, MCP Any will implement MIV. This service will perform periodic, hardware-attested scans of agent memory shards to detect and purge "Sleeper" instructions or poisoned belief systems.
- **Terminal Workspace Sovereignty (TWS)**: Aligning with the Gemini CLI evolution, MCP Any will provide the infrastructure for persistent, stateful terminal agent sessions, ensuring environment integrity and command lineage across long-duration human-agent collaborations.
