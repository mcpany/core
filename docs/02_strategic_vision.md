...
### Focus: Configuration-as-Execution Sovereignty & Supply-Chain Attestation
**Context**: The emergence of "Repository-Local RCE" (CVE-2026-24887) in Claude Code and the mass-distribution of malicious skills via ClawHub confirm that the "Universal Agent Bus" must now move beyond simple tool gating. We must now protect the **integrity of the configuration itself** and provide **multi-agent quorum-based attestation** for all dynamic skill grafts. As agents increasingly operate in collaborative coding environments, the security frontier is now the **Project Settings** and the **Tool Registry**.
**Strategic Pivot**:
- **Hardware-Locked Configuration Anchor (HLCA)**: MCP Any will evolve to act as the authoritative "Config Anchor." We will implement HLCA, cryptographically binding project-local settings (like `.claude/settings.json`) to a TPM-signed user session, ensuring that repository-committed files cannot silently bypass security dialogs or execute unauthorized hooks.
- **Multi-Signature Skill Attestation (MSSA)**: Supporting the "Audit-before-Execution" model, we are mandating MSSA for all tools imported from public registries. Dynamic tool grafting will require cryptographically bound approval tokens from both the agent framework and a verified third-party security auditor, neutralizing "Rug-Pull" supply chain attacks.
- **Deterministic Boot Manifests**: Expanding the attestation gateway to provide signed "Environment Integrity Manifests." These manifests will serve as a mandatory prerequisite for agent boot, providing a deterministic proof of the project-local state, including non-existence proofs for restricted configuration hooks.
- **Zero-Trust Browser Origin Validation (SOP)**: In response to CVE-2026-0628, we are mandating strict Same-Origin Policy (SOP) enforcement for all local MCP listeners, ensuring that browser-based sidebar components cannot be hijacked to bridge into the local filesystem or peripherals.
