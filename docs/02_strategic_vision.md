...
### Focus: Progressive Capability Discovery & Mesh-Resident Teammate Messaging
**Context**: The maturation of Claude Code's "Agent Teams" and Gemini CLI's "Progressive Skill Disclosure" confirm that the "Universal Agent Bus" must move beyond monolithic discovery and hierarchical messaging. As swarms scale to dozens of parallel teammates, the 100ms+ "Orchestrator Tax" for inter-agent communication and the token saturation from thousands of idle tool schemas have become the primary bottlenecks.
**Strategic Pivot**:
- **Progressive Discovery Broker (PDB)**: MCP Any will evolve to support "Metadata-First Discovery." We will implement the PDB, which only surfaces minimal tool metadata to the primary agent initially. Detailed schemas are lazily fetched and injected only when the mission root identifies a high-probability intent match, neutralizing discovery context bloat.
- **Mesh-Resident Teammate Messaging (MRTM)**: To resolve the "Cognitive Stall" in horizontal meshes, we are introducing MRTM. This service provides a low-latency, peer-to-peer messaging tier that allows teammates to exchange mailbox fragments directly without routing through the primary orchestrator, anchored by hardware-attested mesh identity.
- **Multi-Platform Messenger Adapter (MPMA)**: Supporting the OpenClaw "Universal Inbox" vision, MCP Any will act as a unified bridge for P2P coordination across WhatsApp, Slack, and Discord. This ensures that agent mission context persists and remains synchronized even when the communication channel rotates.
- **Zero-Trust Capability Disclosure**: We are mandating "Handshake-before-Detail" for all discovery requests. Full tool schemas remain cryptographically invisible until a teammate completes a verified, mission-bound handshake, preventing "Pre-Flight Shadow Mapping" by rogue subagents.
