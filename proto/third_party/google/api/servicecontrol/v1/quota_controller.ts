...
export const QuotaControllerServiceName = "google.api.servicecontrol.v1.QuotaController";
export class QuotaControllerClientImpl implements QuotaController {
  private readonly rpc: Rpc;
  private readonly service: string;
  constructor(rpc: Rpc, opts?: { service?: string }) {
    this.service = opts?.service || QuotaControllerServiceName;
    this.rpc = rpc;
    this.AllocateQuota = this.AllocateQuota.bind(this);
  }
  AllocateQuota(request: AllocateQuotaRequest): Promise<AllocateQuotaResponse> {
    const data = AllocateQuotaRequest.encode(request).finish();
    const promise = this.rpc.request(this.service, "AllocateQuota", data);
    return promise.then((data) => AllocateQuotaResponse.decode(new BinaryReader(data)));
  }
}
