...
func TestUpstreamService_GitHub(t *testing.T) {
	if os.Getenv("GITHUB_API_TOKEN") == "" || os.Getenv("GITHUB_OWNER") == "" || os.Getenv("GITHUB_REPO") == "" {
		t.Skip("Skipping GitHub test because GITHUB_API_TOKEN, GITHUB_OWNER, or GITHUB_REPO is not set")
	}
	...
	// --- 3. Call Tool ---
	args := json.RawMessage(`{"owner": "` + os.Getenv("GITHUB_OWNER") + `", "repo": "` + os.Getenv("GITHUB_REPO") + `"}`)
	...
	err = json.Unmarshal([]byte(textContent.Text), &githubResponse)
	require.NoError(t, err, "Failed to unmarshal JSON response")

	require.Greater(t, len(githubResponse), 0, "Expected at least one issue")
