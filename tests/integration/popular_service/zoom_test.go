...
func TestZoom(t *testing.T) {
	...
	testCase := &framework.E2ETestCase{
		...
		BuildUpstream: upstream.BuildZoomMockServer,
		GenerateUpstreamConfig: func(upstreamEndpoint string) string {
			configPath := filepath.Join(root, "examples", "popular_services", "zoom", "config.yaml")
			content, err := os.ReadFile(configPath)
			require.NoError(t, err)

			return strings.Replace(string(content), "https://api.zoom.us", upstreamEndpoint, 1)
		},
		InvokeAIClient: func(t *testing.T, mcpanyEndpoint string) {
			...
			toolName := "zoom.create_user"
			arguments := json.RawMessage(`{"action": "create", "user_info": {"email": "test.user@example.com", "type": 1, "first_name": "Test", "last_name": "User"}}`)
			...
			require.NoError(t, err)
			require.Equal(t, "Test", result["first_name"])
			require.Equal(t, "User", result["last_name"])
			require.Equal(t, "test.user@example.com", result["email"])
			require.Equal(t, "user_12345", result["id"])
		},
	}
