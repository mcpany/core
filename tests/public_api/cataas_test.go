...
func TestUpstreamService_Cataas(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), integration.TestWaitTimeLong)
	defer cancel()

	t.Log("INFO: Starting E2E Test Scenario for Cataas Server...")
	t.Parallel()

	// --- 1. Start MCPANY Server ---
	mcpxTestServerInfo := integration.StartMCPANYServer(t, "E2ECataasServerTest")
	defer mcpxTestServerInfo.CleanupFunc()

	// --- 2. Register Cataas Server with MCPANY ---
	const serviceID = "e2e_cataas"
	serviceEndpoint := "https://cataas.com"
	t.Logf("INFO: Registering '%s' with MCPANY at endpoint %s...", serviceID, serviceEndpoint)
	registrationGRPCClient := mcpxTestServerInfo.RegistrationClient

	httpCall := configv1.HttpCallDefinition_builder{
		EndpointPath: proto.String("/cat"),
		Schema: configv1.ToolSchema_builder{
			Name: proto.String("getCat"),
		}.Build(),
	...
	sanitizedToolName, _ := util.SanitizeToolName("getCat")
	...
	require.NoError(t, err, "Error calling getCat tool")
	require.NotNil(t, res, "Nil response from getCat tool")
	...
	require.NotEmpty(t, textContent.Text, "The response should not be empty")
	t.Logf("SUCCESS: Received cat response")

	t.Log("INFO: E2E Test Scenario for Cataas Server Completed Successfully!")
}
